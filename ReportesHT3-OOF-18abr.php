...
if (trim($lstDivision) == "" ) {
	...
	$sql=$sql." and d.id_division =" . $lstDivision ;
}
$sql=$sql." order by  u.apellidos " ;
$cursor = mssql_query($sql);

if ( ($elIDDivision != $lstDivision) AND (trim($lstDivision) != "" ) ) {
	$elNomDirector = "";
}
...
$excel="";
$excel.="Unidad\t";
$excel.="Usuario que factura al proyecto\t";
$excel.="Categoria\t";
$excel.="Proyecto\t";
$excel.="Horas facturadas al mes\t";
$excel.="Total horas facturadas\n";
...
<table width="60%"  border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
	...
	<? 
	//Muestra la lista Divisiones a GRM, PBM, Fabio Sanchez, Andres Marulanda y Lina Arroyave Y cAMILO MARULANDA
	if (($laUnidad == 12974) OR ($laUnidad == 15712) OR ($laUnidad == 13829) OR ($laUnidad == 14234) OR ($laUnidad == 900582) OR ($laUnidad == 14384)) {
	?>
  <tr>
    <td align="right" class="TituloTabla">Divisi&oacute;n</td>
    <td class="TxtTabla">
	<?
	//Trae el listado de Divisiones Activas en la compañía
	$sqlDiv="select * ";
	$sqlDiv=$sqlDiv." from HojaDeTiempo.dbo.divisiones  ";
	$sqlDiv=$sqlDiv." where estadoDiv = 'A' ";
	$sqlDiv=$sqlDiv." order by nombre ";
	$cursorDiv = mssql_query($sqlDiv);
	
	if (trim($lstDivision) == "") {
		$lstDivision = $elIDDivision ;
	}
	?>
	&nbsp;
	<select name="lstDivision" class="CajaTexto" id="lstDivision">
	<? while ($regDiv=mssql_fetch_array($cursorDiv)) {
			if ($lstDivision == $regDiv[id_division] ) {
				$selLstDiv = "selected";
				$elNomDivision = strtoupper($regDiv[nombre]) ;
			}
			else {
				$selLstDiv = "";
			}
	 ...
	 ?>
	  <option value="<? echo $regDiv[id_division]; ?>" <? echo $selLstDiv; ?> ><?  echo strtoupper($regDiv[nombre]);  ?></option>
	  <? } ?>
	  </select>	  </td>
    <td align="right" class="TituloTabla">&nbsp;</td>
  ...
  <tr>
    ...
    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
	  ...
	  <tr class="TxtTabla">
        ...
        <td><? echo $qw1=ucwords(strtolower($reg[apellidos] . " " . $reg[nombre] )); 
		
		?> </td>
        <td width="2%"><?php
        	echo $qwe=ucwords( strtolower( $reg[categoria] ) ); # . " " . $reg[nombre] )); 
			;
		?></td>
        <td width="50%" align="center">
		<?
		 $consul="select H.unidad,U.apellidos,U.nombre nombre1,U.id_categoria,P.nombre nombre2, sum(H.horas_registradas) horasFact
		 from horas H, Proyectos P, Usuarios U 
		 where H.id_proyecto = P.id_proyecto 
		 and U.unidad=H.unidad
		 and H.unidad = ".$reg[unidad]."
		 and month(H.fecha) = ".$pMes." 
		 and year(H.fecha) = ".$pAno."
		 group by H.id_proyecto, P.nombre, P.cargo_defecto, P.codigo,h.unidad,U.nombre,U.apellidos,U.id_categoria";
		 $consulta = mssql_query($consul);
		while ($regQry1=mssql_fetch_array($consulta)) {
				 $excel.=$regQry1[unidad]."\t";
				 $excel.=$regQry1[apellidos]." ";
				 $excel.=$regQry1[nombre1]."\t";
				 $excel.=$regQry1[id_categoria]."\t";
				 $excel.=$regQry1[nombre2]."\t";
				 $excel.=$regQry1[horasFact]."\t";
		$consul2="select sum(H.horas_registradas) horasFact2 from HojaDeTiempo.dbo.horas H, HojaDeTiempo.dbo.Proyectos P 
						where H.id_proyecto = P.id_proyecto and H.unidad = ".$reg[unidad]." and month(H.fecha) = ".$pMes." and year(H.fecha) = ".$pAno;
				$sumator = mssql_fetch_array( mssql_query( $consul2) );
				$excel.=$sumator[horasFact2]."\n"; 	 
    ...
    <td align="left" bgcolor="#FFFFFF" class="TxtTabla">
	  <table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
          ...
          <tr>
            <td align="left">
			<? 
			echo $qwer= "[".$regQry[codigo].".".$regQry[cargo_defecto]."] ".ucwords(strtolower($regQry[nombre])); #  H.cargo_defecto, H.codigo,?> <?  ?></td>
            <td width="20%" align="right"><? echo $qwert=$regQry[horasFact]; ?></td>            
            <?php 
	  ...
	  <? } ?>
		...
		<table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td class="TxtTabla">
            <form name="form1" method="post" action="rptXlsFactura01.php">
