	...
	while ( ( $reg < $cantAct ) &&($error!="no"))
	{
		...
		if ( (${$act}=='I') &&($error!="no") )
		{
			...
			else
			{
				...
				$sqlDel = "UPDATE  HojaDeTiempo.dbo.ParticipantesActividad SET estado='I'
						   Where id_actividad = ".${$opc}." and unidad = ".$und." AND id_proyecto = ".$cualProyecto;
				$qry = mssql_query( $sqlDel );
				if( !$qry )
					$error = "no";
...
//	$error = "no";
	if( $error == "" ){
		...
		echo "<script> 
				alert('Error el proceso con exito.');
			  </script>";		
	}	
 
echo "<script> 
				window.close();
				MM_openBrWindow('htPlanProyectos02.php?cualProyecto=".$cualProyecto."&participante=0&aa=".$aa."','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');
			  </script>";
