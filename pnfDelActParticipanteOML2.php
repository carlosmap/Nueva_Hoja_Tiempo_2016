	...
	while( $reg < $cantUsuarios ){
		...
		if( ${$opc} == 1 ){
			*/
			...
			$sqlDel = "Delete From ParticipantesActividad Where id_actividad = ".$actividad." AND id_proyecto = ".$idProyecto." AND unidad = ".$unidad;
#		}		
		#echo $sqlDel."<br />";		
#		$reg++;
#	}
...
?>
	...
	<form name="form1" id="form1" method="post" action="">
        ...
        <td bgcolor="#FFFFFF">
            ...
            <?
                $sqlActResponsable = "SELECT distinct
                                      ...
                                      FROM
										...
										, HojaDeTiempo.dbo.Categorias cat, HojaDeTiempo.dbo.Departamentos dep, HojaDeTiempo.dbo.Divisiones div
										, HojaDeTiempo.dbo.Programadores prog									
                                      WHERE
            ...
            <table width="100%"  border="0" cellspacing="1" cellpadding="0">
                <tr class="TituloTabla2">
                  <td colspan="4" valign="top">Esta seguro de querer eliminar esta actividad?</td>
                </tr>
                <?
                    $sqlAct = "SELECT act.id_actividad idAct, act.nombre nActividad, act.macroactividad, res.estado		
                               ...
                               WHERE 
                                ...
                                AND	res.id_proyecto = ".$cualProyecto." AND usu.unidad = ".$idAct." AND act.id_actividad = ".$idActi; 
                ...
                <tr class="TxtTabla">
                    ...
                    <input type="hidden" name="actividad" value="<?= $idActi ?>" /> 
                    <input type="hidden" name="unidad" value="<?= $idAct ?>" />
                    <input type="hidden" name="idProyecto" value="<?= $cualProyecto ?>" />
                    </td>
                </tr>
            </table>
            <table width="100%"  border="0" cellspacing="0" cellpadding="0">
...
<input name="Submit" type="submit" class="Boton" onclick="envia2()" value="Eliminar" /></td>
              </tr>
          </table>
            <table width="100%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
