...
$cantProyectos = 0 ;
$sql="SELECT DISTINCT P.* , D.nombre nomDirector, D.apellidos apeDirector, C.nombre nomCoordina, C.apellidos apeCoordina,  ";
$sql=$sql." D.email mailDirector, C.email mailCoordina  ";
$sql=$sql." FROM HojaDeTiempo.dbo.proyectos P, HojaDeTiempo.dbo.Usuarios D, HojaDeTiempo.dbo.Usuarios C, HojaDeTiempo.dbo.Divisiones div ";
$sql=$sql." WHERE P.id_director = D.unidad  ";
$sql=$sql." AND P.id_coordinador *= C.unidad ";
if (trim($pEmp) != "") {
	$sql=$sql." AND P.idEmpresa =" . $pEmp ;
  ...
  <tr>
    ...
    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
      ...
      <tr class="TituloTabla2">
        ...
        <td width="10%">Director/Coordinador</td>
        ...
        <td width="7%">Fecha <br />
          de creaci&oacute;n </td>
        <td width="5%">Cantidad de Personas que facturaron </td>
        <td width="10%">Horas registradas</td>
	  ...
	  <tr valign="top" class="TxtTabla">
	    <td width="3%"><? echo ucwords(strtolower($reg[codigo])) . "." . ucwords(strtolower($reg[cargo_defecto]))  ; ?><br />
		...
		<?
			...
			$sqlPersonas = "SELECT COUNT(*) Personas
							FROM (
							SELECT DISTINCT unidad
							FROM Horas hr, Proyectos Pro 
							WHERE hr.id_proyecto = Pro.id_proyecto AND 
							Pro.id_proyecto = ".$reg[id_proyecto]."  AND MONTH(hr.fecha) = ".$pMes." AND YEAR(hr.fecha)=  ".$pAno . ") A ";
			#echo $sqlPersonas."<br />"; 
		...
		?></td>
        <td width="10%"><table width="100%"  border="0" cellspacing="1" cellpadding="1" bgcolor="#FFFFFF" >
          <tr class="TituloTabla2" bgcolor="#FFFFFF">
            <td width="50%">CT</td>
          ...
          <tr align="right" bgcolor='#FFFFFF'  class='TxtTabla'>
            <td width="50%"><?= $rowHrFacturado[clase_tiempo] ?></td>
            <td width="50%"><?=  $rowHrFacturado[hrReg] ?>
            </td>
          </tr>
          <?		}
		?>
        ...
        <?
			...
			$sqlFacturado = "SELECT DISTINCT 
							 ...
							 FROM ( 
								Select 
								H.unidad, U.nombre, U.apellidos, D.nombre nomDpto, D.id_departamento, 
								Z.nombre nomDiv, Z.id_division, H.id_proyecto, P.nombre nomProyecto, 
								H.clase_tiempo, sum(H.horas_registradas) hFacturadas, A.salarioBase, ((A.salarioBase/185)*sum(H.horas_registradas)) vlrFacturado, P.codigo, 
								P.cargo_defecto 
								from 
								Horas H, Proyectos P, Asignaciones A, Usuarios U , Departamentos D, Divisiones Z 
								where 
								H.id_proyecto = P.id_proyecto and H.id_proyecto = A.id_proyecto and H.id_actividad = A.id_actividad and H.unidad = A.unidad and 
								H.clase_tiempo = A.clase_tiempo and H.localizacion = A.localizacion and H.cargo = (P.codigo + A.cargo) and 
								MONTH(H.fecha)= MONTH(A.fecha_inicial) and YEAR(H.fecha)= YEAR(A.fecha_inicial) and H.unidad = U.unidad and 
								H.id_proyecto = ".$reg[id_proyecto]." AND MONTH(H.fecha)= ".$pMes." and YEAR(H.fecha)= ".$pAno." and 
								U.id_departamento = D.id_departamento and D.id_division = Z.id_division  
								GROUP BY 
									H.unidad, U.nombre, U.apellidos, H.id_proyecto, P.nombre, H.clase_tiempo, A.salarioBase, P.codigo, P.cargo_defecto, D.nombre, 
									D.id_departamento, Z.nombre, Z.id_division 
							 ) A 
