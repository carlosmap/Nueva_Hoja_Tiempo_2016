...
//$recarga = "" cuando visualiza la página por primera vez
...
//Para identificar la unidad del primer usuario que aparezca en la lista cuando el formulario no ha sido recargado
if (trim($recarga) == "") {
	$sqlU="Select * from Usuarios   "  ;
	$sqlU=$sqlU." where retirado is null ";
	$sqlU=$sqlU." and not exists ";
	$sqlU=$sqlU." (select distinct unidad from ProgSumaGlobalUsu ";
	$sqlU=$sqlU." where id_proyecto = " . $cualProyecto;
	$sqlU=$sqlU." and unidadProgramador= " . $laUnidad ;
	$sqlU=$sqlU." and unidad = Usuarios.unidad ";
	$sqlU=$sqlU." ) ";
	$sqlU=$sqlU." order by apellidos ";
	$cursorU = mssql_query($sqlU);
	if($regU=mssql_fetch_array($cursorU)) {
		$pUsuario = $regU[unidad] ;
	}
}
	...
	while ($s <= $cantItems) {
		...
		*/
		...
		if(${$elhProg} > 0) {		
			...
			$query = "INSERT INTO ProgSumaGlobalUsu(id_proyecto, unidadProgramador, unidad, mes, vigencia,   " ;
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="TituloUsuario">Programación de Proyectos - Empleados que participan </td>
  </tr>
</table>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td colspan="2" class="TxtTabla"><select name="pUsuario" class="CajaTexto" id="pUsuario" onChange="envia1()" >
      <?
		...
		$sql2=$sql2." (select distinct unidad from ProgSumaGlobalUsu ";
		$sql2=$sql2." where id_proyecto = " . $cualProyecto;
		$sql2=$sql2." and unidadProgramador= " . $laUnidad ;
  ...
  <tr>
    ...
    <td align="right" class="TxtTabla"><input name="Submit2" type="button" class="Boton" onClick="MM_openBrWindow('verProgUsu.php?cualUnidad=<? echo $pUsuario; ?>','winPUsu','scrollbars=yes,resizable=yes,width=700,height=400')" value="Ver Programaci&oacute;n"></td>
  </tr>
</table>
      <table width="100%"  border="0" cellspacing="0" cellpadding="0">
        <tr>
      ...
      <table width="100%"  border="0" cellspacing="1" cellpadding="0">
        ...
        <tr class="TxtTabla">
          ...
          <td width="30%" align="center" onClick="MM_openBrWindow('verProgUsu.php','vPusu','scrollbars=yes,resizable=yes,width=600,height=400')">
		  <?
		  	$pHorasLab=0;
		  ...
		  ?>
		  <input name="hLab<? echo $e; ?>" type="text" class="CajaTexto" id="hLab<? echo $e; ?>" value="<? echo $pHorasLab; ?>" size="10" readonly>		  </td>
