...
,c.nombre
,a.clase_tiempo
 FROM [Horas] a, Usuarios b,Departamentos c,UsuariosSalario d,Proyectos e, categorias f  where (";
...
if($pUnidad<>"")
{
  ...
  and f.id_categoria=b.id_categoria
 ...
 //  echo $sql00;
  ...
  <tr>
    ...
    <? $sqlproyecto="select * from Proyectos where especial=1"; 
	  ...
	  ?>
        <option value="<? echo $lstRegProy[id_proyecto]; ?>" <? echo $selProyecto; ?> >.::<? echo $lstRegProy[nombre];echo"[";   echo$lstRegProy[codigo]; echo"."; echo$lstRegProy[cargo_defecto];   echo"]"; ?>::.</option>
	  <? }?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF">
      ...
      <tr class="TxtTabla">
        <td width="8%">
		<? echo $reg00[unidad];  $excel.=$reg00[unidad]."\t"; ?>
        ...
        <td width="3%"><? echo $reg00[nombrec]; $excel.=$reg00[nombrec]."\t"; ?>
		
		</td>
        <td width="10%"><? echo  ucwords(strtolower($reg00[nombre])); $excel.=$reg00[nombre]."\t"; ?>
		
		</td>
        <td width="25%"><? echo $project="[" . $reg00[codigo] . "."  . $reg00[cargo_defecto] . "] " . $reg00[nombrep];   $excel.=$project."\t";  ?>

		</td>
        <td width="5%" align="right"><? echo $reg00[clase_tiempo]; $excel.=$reg00[clase_tiempo]."\t"; ?>
	...
	//echo $sumSql;
				...
				echo $querySum[horasFact]; $excel.= $querySum[horasFact]."\t"; ?>
		<? 
...
year(fecha) <= ".$pAno."  and unidad=".$reg00[unidad]." and  month(fecha) <= ".$pMes;
	...
	//echo $sumSql;
		...
		if (trim($querySal[salario]) != "") {
			...
			echo $salfact=number_format($vlrFacturado, 2, ",", ".");
