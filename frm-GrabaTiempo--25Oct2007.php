...
<tr>
            ...
            <? if (($laUnidad == 129740000) OR ($laUnidad == 20400000)) { ?>
			<tr>
              <td class="menu"><img src="img/images/mnuFlecha.gif" width="12" height="13" /></td>
...
<TR>
<TD width="30%" valign="top" class="TituloTabla">Actividad-Clase Tiempo-Localizaci&oacute;n</TD>
	...
	if($resultado=mssql_query($sql)){
		if ($actividad=="" or $actividad=="Seleccione una actividad"){
		   ...
		   while ($filas=mssql_fetch_array($resultado)){
				...
				if($filas[id_actividad]."-".$filas[clase_tiempo]."-".$filas[localizacion]==$actividad){
					//$elnombredelaactividad = ucwords(strtolower($filas[nombre]))."-".$filas[clase_tiempo];
					$elnombredelaactividad = ucwords(strtolower($filas[nombre]))."-".$filas[clase_tiempo]."-".$filas[localizacion];
				}
		   }
...
/*		do {
		...
		while ($filas=mssql_fetch_array($resultado)){
		 	//echo "<OPTION VALUE='".$filas[id_actividad]."-".$filas[clase_tiempo]."'>".ucwords(strtolower($filas[nombre])).'-'.$filas[clase_tiempo].'</OPTION>';
			echo "<OPTION VALUE='".$filas[id_actividad]."-".$filas[clase_tiempo]."-".$filas[localizacion]."'>".ucwords(strtolower($filas[nombre])).'-'.$filas[clase_tiempo]."-".$filas[localizacion].'</OPTION>';
		}   
...
//	WHERE (id_proyecto = '$nomproyecto') AND (id_actividad = '$actividad') AND (horas.unidad = '$laUnidad') AND (horas.clase_tiempo='$clasetiempo')";
	$sql="select sum(horas_registradas) as hr
	FROM horas
	WHERE (id_proyecto = '$nomproyecto') AND (id_actividad = '$actividad') AND (horas.unidad = '$laUnidad') AND (horas.clase_tiempo='$clasetiempo') AND (localizacion = $localizacion)";
...
/*	echo "<script>alert('El total se horas de la fecha seleccionada es ".$totalHorasDia."')</script>";*/
