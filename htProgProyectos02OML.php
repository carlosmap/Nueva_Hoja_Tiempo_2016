  ...
  <tr>
	...
	<? } ?>
      ...
      <tr>
        ...
        <td align="left" class="TxtTabla">
        <?
			$perfiles = array( 'Programador', 'Ordenador de gastos', 'Coordinador', 'Director' );
			$regPer = 0;
		?>
        <select name="pr" class="CajaTexto" id="pr" onchange="document.form1.submit();" >
		  <option value="" >:::Todos los Pefiles:::</option>
          <? 
			while ( $regPer < 4) { 	
				if ( $pr == $regPer ) 
					$selPer = "selected";
					...
					$selPer = "";
		  ?>
          		<option value="<?= $regPer; ?>" <?= $selPer; ?> ><?= ucwords( strtolower( $perfiles[$regPer] ) ) ?></option>
          <? 
		  		$regPer++;
		  	} 
		  ?>
      ...
      <tr>
        ...
        <?
			...
			if( trim( $lt ) != "" ){
				if( $lc != "" )
					...
					$lote = "actPrincipal";
				$sqlActResponsable .= " AND act.dependeDe in ( select id_actividad from  HojaDeTiempo.dbo.Actividades 
																	where dependeDe = ".$lt." and id_proyecto = ".$cualProyecto." and actPrincipal = ".$lote." )";#.$lote.")";
			}
			if( trim( $pfDivision ) != "" )
				$sqlActResponsable .= " AND div.id_division = ".$pfDivision;
				
			if( trim( $miDpto ) != "" )
				$sqlActResponsable .= " AND dep.id_departamento = ".$miDpto;
				
			if( trim( $pFiltro ) != "" )
				$sqlActResponsable .= " AND cat.id_categoria = ".$pFiltro;
			
			$sqlActResponsable .= " AND cat.id_categoria = usu.id_categoria AND usu.id_departamento = dep.id_departamento
									AND dep.id_division = div.id_division AND act.id_actividad = res.id_actividad 
									AND	res.id_proyecto =".$cualProyecto;
			
			if( trim( $lc ) != "" )
				$sqlActResponsable .= " AND act.id_actividad in ( select id_actividad from HojaDeTiempo.dbo.Actividades WHERE 
										actPrincipal = ".$lc." AND id_proyecto = ".$cualProyecto." )";

			if( trim( $pr ) != "" )
				switch( $pr ){
					case 1:
						#	PROGRAMADOR
						$sqlActResponsable .= " AND ( prog.id_proyecto = res.id_proyecto AND prog.unidad = res.unidad )";
					break;
				}				
        ...
        <table width="100%"  border="0" cellspacing="1" cellpadding="0">
          ...
          <tr class="TxtTabla">
          	<td width="8%" valign="top"><?= $row[unidad] ?></td>
            <td width="20%" valign="top"><?= $row[NombresApellidos] ?></td>
            <td width="5%" valign="top"><?= $row[Categoria] ?></td>
            <td width="10%" valign="top"><?= $row[nDivision] ?></td>
            <td width="10%" valign="top"><?= $row[nDepartamento] ?></td>
            <td valign="top">
			
				<?
					$sqlAct = "SELECT act.nombre nActividad, act.macroactividad	
							   FROM HojaDeTiempo.dbo.ParticipantesActividad res, HojaDeTiempo.dbo.Actividades act, HojaDeTiempo.dbo.Usuarios usu	
							   WHERE 
							   	usu.unidad = res.unidad	AND act.id_actividad = res.id_actividad AND act.id_proyecto = res.id_proyecto AND res.estado = 1 
								AND	res.id_proyecto = ".$cualProyecto." AND usu.unidad = ".$row[unidad]; 
					$qryAct = mssql_query( $sqlAct );
					#echo $sqlAct."<br />";
					while( $rw = mssql_fetch_array( $qryAct ) ){										 
						echo "<b>[ ".$rw[macroactividad]." ]</b> ".$rw[nActividad]."<br />";
					}
				?>
            </td>
            <td width="1%" colspan="2">
            <img onclick="MM_openBrWindow('pnfdelAutParticipanteOML.php?cualProyecto=<? echo $cualProyecto ; ?>&idAct=<?= $row[unidad] ?>', 'addAAT', 'scrollbars=yes,resizable=yes,width=620,height=400')" src="imagenes/actualizar2.gif" alt="" style="cursor: hand;" />
            </td>
          </tr>
          <?	}	?>
          ...
          <tr class="TxtTabla">
            ...
            <td>01-A</td>
            <td>[división Gergen]</td>
            <td>&nbsp;</td>
            <td><strong>LC1 GERENCIA DEL PROYECTO</strong><br />
            LT1.1 ACTIVIDADES DE SOPORTE A LA GERENCIA DEL PROYECTO</td>
            <td>&nbsp;</td>
          </tr>
          <tr class="TxtTabla">
            <td>4417</td>
            <td>Hector Alfredo L&oacute;pez</td>
            <td>02-A</td>
            <td> [divisi&oacute;n Gergen] </td>
            <td>Calidad</td>
            <td>Calidad</td>
            <td>&nbsp;</td>
          </tr>
          ...
          <tr class="TxtTabla">
            <td>12372</td>
            <td>Hernando Caicedo</td>
            <td>02-A</td>
            <td>[divisi&oacute;n Gergen] </td>
            <td> Sogamoso </td>
            <td>Manuales e informes <br />
            Documentos obra civil</td>
            <td>&nbsp;</td>
          </tr>
          <tr class="TxtTabla">
            <td>17206</td>
            <td>Fernando Garz&oacute;n</td>
            <td>04</td>
            <td>Geotecnia</td>
            <td>Geologia</td>
          ...
          <tr class="TxtTabla">
            <td>11973</td>
            <td>Jose Luis Sierra</td>
            <td>04</td>
            <td>Planeamiento</td>
            <td>Hidrolog&iacute;a</td>
            <td>Climat. Hidro - Sedim <br />
            Hidrol., hidr&aacute;ulica y socav.<br />
            Hidrol., hidr&aacute;ulica y socav.<br />
            Hidr&aacute;ulica y sanitaria </td>
            <td>&nbsp;</td>
          </tr>
          <tr class="TxtTabla">
            <td>11383</td>
          ...
          <tr class="TxtTabla">
            <td>12974</td>
            <td>Gonzalo Rodr&iacute;guez </td>
            <td>03</td>
            <td>[divisi&oacute;n Gergen] </td>
          ...
          <tr class="TxtTabla">
            ...
            <td>[divisi&oacute;n Gergen] </td>
            <td> Subgerencia </td>
            <td>LT1.2 ADMINISTRACI&Oacute;N DE ASESOR&Iacute;AS, CONSULTOR&Iacute;AS E INTERVENTOR&Iacute;AS</td>
            <td>&nbsp;</td>
          </tr>
          <tr class="TxtTabla">
            <td>15252</td>
            ...
            <td>Vias-transp</td>
            <td> Vias-topogr </td>
            <td><strong>LC2 INFRAESTRUCTURA<br /></strong>
            LT2.2 V&Iacute;AS SECUNDARIAS Y PUENTE TEMPORAL, PLAZOLETAS Y PORTALES - ADECUACI&Oacute;N CANTERAS Y DEP&Ograve;SITOS <br />
            Dise&ntilde;o geom&eacute;trico</td>
            <td>&nbsp;</td>
          </tr>
          <tr class="TxtTabla">
            <td>&nbsp;</td>
          ...
          <tr class="TxtTabla">
            <td width="8%">15712</td>
            <td width="20%">Patricia Bar&oacute;n Manrique </td>
            <td width="5%">05</td>
            <td width="10%">[divisi&oacute;n Gergen] </td>
            <td width="10%">Inform&aacute;tica</td>
            <td>&nbsp;</td>
        ...
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td align="right" class="TxtTabla"><input name="Submit" type="submit" class="Boton" onclick="MM_openBrWindow('pnfaddAutParticipanteOML.php?cualProyecto=<? echo $cualProyecto ; ?>','addAAT','scrollbars=yes,resizable=yes,width=500,height=400')" value="Ingresar participantes al proyecto" /></td>
          </tr>
        </table>
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr>
