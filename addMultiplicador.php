...
if ($lPorcentaje != "") {
	...
	if ($vReg1=mssql_fetch_array($vCursor1)) {
		if ($vReg1[Cuantos] > 0) {
			echo ("<script>alert('La descripción del multiplicador de viático ya existe. " . $vReg1[NomHorario] . " ');</script>");
			echo ("<script>window.close()</script>");	
			exit;
		}
	}

	//Realiza la inserción del multiplicador en la tabla FraccionesV
	//IDfraccion, Porcentaje, Descripcion
	$query = "INSERT INTO  FraccionesV (Porcentaje, Descripcion) " ;
	$query = $query." VALUES( " . $lPorcentaje . " , " ;
	$query = $query . " '". $lDescripcion . "'  ";
	$query = $query . " ) ";
	  ...
	  <form action="" method="post" name="Form1" onSubmit="MM_validateForm('lPorcentaje','','RinRange1:100','lDescripcion','','R');return document.MM_returnValue" >
	    <table width="100%"  border="0" cellspacing="1" cellpadding="0">
          <tr>
            ...
            <td class="TxtTabla"><input name="lPorcentaje" type="text" class="CajaTexto" id="lPorcentaje" size="10">              <span class="TituloTabla">
              <input name="miProyecto" type="hidden" value="<? echo $cualProyecto; ?>">
</span></td>
          </tr>
          <tr>
            <td width="25%" class="TituloTabla">Descripci&oacute;n</td>
            <td class="TxtTabla"><input name="lDescripcion" type="text" class="CajaTexto" id="lDescripcion" size="100"></td>
          </tr>
        </table>
	    <table width="100%"  border="0" cellspacing="1" cellpadding="0">
  <tr>
