...
*/
#	U.fechaIngreso, U.fechaRetiro, U.retirado, 
$sql="Select  U.unidad, U.nombre, U.apellidos, U.id_departamento, U.id_categoria, ";
$sql=$sql." C.nombre nomCategoria, D.nombre nomDpto, A.vigencia, A.mes, A.fechaEnvio,  " ;
...
if ($pMes == "") {
	...
	#	= 4 AND = 2012
	#$sql= $sql. " AND MONTH( U.fechaIngreso ) =  month(getdate()) ";
	#$sql= $sql. " AND YEAR( U.fechaIngreso) =  year(getdate())";
}
else {
	#	MODIFICADO OSCAR LOPEZ
	$sql= $sql. " AND A.mes = " . $pMes;
	$sql= $sql. " AND A.vigencia =  " . $pAno;
	#	AGREGADO
	#	= 4 AND = 2012
	#$sql= $sql. " AND MONTH( U.fechaIngreso ) < " . $pMes;
	#$sql= $sql. " AND YEAR( U.fechaIngreso) < " . $pAno;
}
/*
  ...
  <tr>
    ...
    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
      ...
      <tr class="TituloTabla2">
        ...
        <td width="10%" rowspan="2">Feca de ingreso y retiro</td>
	  ...
	  <tr class="TxtTabla">
        ...
        <b>Ingreso : </b><?= date("d/m/Y ", strtotime($reg[fechaIngreso])) ?><br />
        <?	if( $reg[fechaRetiro] != '' ){	?>
		<b>Retiro: </b><?= date("d/m/Y ", strtotime($reg[fechaRetiro])) ?>
        <?	}	?>
        </td>
        <td width="5%" align="center">
		<?
