...
if ($_SESSION["sesPerfilUsuario"] == "1")  {
	...
	$sqlTOP="
		...
		select *, ROW_NUMBER() OVER (ORDER BY  t.nombre ) AS RowNumber  from (		
			SELECT  P.*, D.nombre nombreD, D.apellidos apellidosD, C.nombre nombreC, C.apellidos apellidosC  ";
	...
	$sqlTOP="
		...
		select *, ROW_NUMBER() OVER (ORDER BY  t.nombre ) AS RowNumber  from (
					...
					SELECT 
					P.*, D.nombre nombreD, D.apellidos apellidosD, C.nombre nombreC, C.apellidos apellidosC FROM (
		
					 Select id_proyecto from HojaDeTiempo.dbo.Proyectos where id_director = ". $laUnidad." or id_coordinador = ". $laUnidad." and id_estado = 2 
					 UNION 
					 Select id_proyecto from HojaDeTiempo.dbo.Programadores where unidad = ". $laUnidad." 
					 UNION
					  select id_proyecto from GestiondeInformacionDigital.dbo.OrdenadorGasto where unidadOrdenador = ". $laUnidad." and id_proyecto is not null 
					  UNION 
					  select id_proyecto from HojaDeTiempo.dbo.actividades where id_encargado = ". $laUnidad." 
					  UNION 
					  select id_proyecto from HojaDeTiempo.dbo.ResponsablesActividad where unidad = ". $laUnidad."
					  
					   ) A, Proyectos P, Usuarios D, Usuarios C 
					  WHERE A.id_proyecto = P.id_proyecto AND P.id_director *= D.unidad 
...
$sqlTOP = $sqlTOP." 	)
		as t ) SELECT RowNumber,* FROM proy  WHERE RowNumber BETWEEN ".$inicio." and  ".$fin;
  ...
  <tr>
    ...
    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
      ...
      <tr class="TituloTabla2">
        <td width="1%">&nbsp;</td>
        <td width="3%">ID</td>
        <td width="20%">Proyectos</td>
        <td>C&oacute;digo</td>
        <td width="17%">Encargados</td>
        <td width="17%">Planeaci&oacute;n Vs Facturaci&oacute;n </td>
        <td width="5%">&nbsp;</td>
        <td width="1%">&nbsp;</td>
        <td width="1%">&nbsp;</td>
        <td width="1%">&nbsp;</td>
	  ...
	  ?>
	   <tr class="TxtTabla">
	    ...
	    <td width="3%" valign="top"><? echo  $reg[id_proyecto] ; ?></td>
        <td width="20%" valign="top"><? echo  ucwords(strtolower($reg[nombre])) ; ?></td>
        <td valign="top">
		<?
		//27Ene2009
...
//		}
        ...
        <td width="5%" bgcolor="#FFFFFF"   ><table width="100%"  border="0" cellspacing="1" cellpadding="0" >
...
<?
			$sql_valores_proy="
			select 
			(select valorProyecto from  proyectos where id_proyecto=". $reg[id_proyecto].") total_proyecto,
			(select SUM(valorReal) from AsignaValorDivision where id_proyecto=". $reg[id_proyecto].") asignado_divisiones_real,
			(select SUM(valorAsignado) from AsignaValorDivision where id_proyecto=". $reg[id_proyecto].") asignado_divisiones,
			(select SUM(valorPlaneado) from PlaneacionProyectos where id_proyecto=". $reg[id_proyecto].") planeado,
			(select SUM(valorFacturado) from FacturacionProyectos where id_proyecto=". $reg[id_proyecto].") facturado";
//echo $sql_valores_proy." ******";
			$cur_valores_proy=mssql_query($sql_valores_proy);
			$datos_valores_proy=mssql_fetch_array($cur_valores_proy);
?>
              <tr class="TxtTabla">
                <td width="50%">Total</td>
                <td align="right">$<? echo number_format($datos_valores_proy["total_proyecto"],0,",",".") ; ?></td>
              </tr>
              <tr class="TxtTabla">
                <td width="50%"> Real</td>
                <td align="right">
				<?
					...
					echo "$".number_format($datos_valores_proy["asignado_divisiones_real"],0,",",".");
				?>
				</td>
				</tr>
              <tr class="TxtTabla">
				...
				<?
					...
					echo "$".number_format($datos_valores_proy["asignado_divisiones"],0,",",".");
				?>
				</td>
				</tr>

				<tr>
                <td width="50%" class="TxtTabla">Planeado</td>
                <td align="right" class="TxtTabla">
				<?
					echo "$".number_format($datos_valores_proy["planeado"],0,",",".");
				?>
				</td>
              </tr>
				<tr>
                <td width="50%" class="TxtTabla">Facturado</td>
                <td align="right" class="TxtTabla">
				<?
					echo "$".number_format($datos_valores_proy["facturado"],0,",",".");
				?>
				</td>
              </tr>
              <tr class="TxtTabla">
        ...
        <td width="5%" valign="top">
			...
			<img src="grafica_valores_proyecto.php?val_proy=<?=$datos_valores_proy["total_proyecto"] ?>&val_fact=<?=$datos_valores_proy["facturado"]; ?>&val_planea=<?=$datos_valores_proy["planeado"]; ?>" width="224" height="97" />


		</td>
        <td width="1%"><img src="img/images/icoConfig.gif" alt="Configuraci&oacute;n" width="25" height="23" style="cursor: hand" onclick="MM_goToURL('parent','htPlanProyectoConfig.php?cualProyecto=<? echo $reg[id_proyecto]; ?>');return document.MM_returnValue" /></td>
        <td width="1%"><img src="img/images/icoGesPro.gif" alt="Gesti&oacute;n del proyecto" width="23" height="24" style="cursor: hand" onclick="MM_goToURL('parent','htPlanProyectos01.php?cualProyecto=<? echo $reg[id_proyecto]; ?>');return document.MM_returnValue" /></td>
