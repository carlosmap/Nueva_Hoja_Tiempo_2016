...
global $suma;
//si la variable autorizadoPor es diferente de blanco, entoces la solicitud de autorización viene de RevisarHojasdeTiempo.php
//Esto significa que la aprobación la esta haciendo una persona autorizada
if($autorizadoPor <> ""){
	$elQueAutoriza = $autorizadoPor;//viene de la página arriba citada
	$perfilQueAutoriza = $perfil;//viene de la página arriba citada
}
...
/*echo "<table>";
echo "<tr><td><B>CONVENCIONES:</B></td><td bgcolor=green>     </td><td>Tiempo aprobado</td></tr>";
echo "<tr><td></td><td bgcolor=red>     </td><td>Tiempo no aprobado</td></tr>";
echo "<tr><td></td><td bgcolor=blue>     </td><td>Tiempo aún no revisado</td></tr>";
echo "</table>";*/
...
<form action="hdetiempo-aprobDivision.php" name=reporte method="post">
...
$cadfecha="'$fiAprobacion' and '$ffAprobacion'";
...
$fechasistema="del periodo $cadfecha";
/******************************************************************************************************************/
/*
echo "<table>";
echo "<tr><td><b>MI APROBACIÓN:</b></td><td>Verde: SI</td><td>Rojo:NO</td>
	<td>Azul: Por revisar</td></tr>";
echo "<tr><td><b>OTRA APROBACIÓN:</b></td><td bgcolor='#D2D2D2'>NO APROB<td><TD bgcolor='#FFCC00'>APROBADO</TD></tr>";
echo "</table>";
*/
/***************************Dibuja el encabezado de la hoja de tiempo**********************************************/
echo "<table class = 'TxtTabla' border='1' bordercolor=black width=100%>";
echo "<tr>";
...
if($codProyecto==""){
	$sql="SELECT RTRIM(localizacion) + '-' + RTRIM(cargo) AS codigoproyecto,estadoAprobDivision, comentariosDivision,
	revisadoPorDivision, estadoAprobProyecto, comentariosProyecto,	revisadoPorProyecto, horas_registradas,
	clase_tiempo, resumen_trabajo, day(fecha) as dia, id_actividad, id_proyecto, fecha, localizacion, cargo
	FROM Horas
	WHERE     (fecha BETWEEN $cadfecha) AND (unidad = '$MiUnidad') AND (cargo <> 'acc0')
...
if($resultado=mssql_query($sql)){
	...
	while($filas){
		...
		and ($mactividad==$filas[id_actividad])){
			...
			//Despliega las horas para su aprobación si estas no han sido vistas, o tienen un no en el campo de aprobaciones
			//Decide que visualizar
			if($aprobacion[$indice] <> "SI"){
		...
		for($i=1;$i<=31;$i++){
			...
			if($horasdia[$i]==0){
				...
				if($aprobacion[$i]=="SI"){
					...
					$comentarioNoAprob = "POR EL MOMENTO NO HAY COMENTARIOS SOBRE LA APROBACIÓN DE SU TIEMPO";
				...
				*/
					...
					$cPro = $comenPro[$i];
					$cDto = $comenDto[$i];
					echo "<td bgcolor='$color' style=\"text-align:center;\"><a href=\"javascript:alert('$resumen');\">$horasdia[$i]</a></td>\r";
					$horasdia[$i]=" ";
			}
		}
		$color='';
		...
		if($DATOS==""){
			...
			echo "<td colspan=4><a href = \"javascript:alert('$cDiv');\">Cdivi</a>/<a href = \"javascript:alert('$cPro');\">Cproy</a>
			/<td>";
		}
	...
	echo "<td>$primas[0]</td>";
	echo "<td>$primas[1]</td>";
	echo "<td colspan='31'><center>$primas[2]</center></font></td>";
	echo "<td colspan='3'><table class='TxtTabla' border='1' bordercolor=black>
			<tr>
			<td colspan=3><center>DIAS</center></td></tr>
			<tr>
			<td>Viaticos</td>
			<td>Prima</td>
			<td>Aux. Alim</td>
	...
	echo "<td colspan='3'><table class='TxtTabla' border='1' bordercolor=black><tr><td colspan=3 width=150><center>DESCRIPCIÓN</center>
			</td></tr><tr><td>Proyecto:</td><td>Sitio:</td></table></td>";
	echo "</tr>";
...
$sql = "SELECT DISTINCT cargo AS Cargo FROM Horas
	WHERE (unidad = $MiUnidad) AND (fecha BETWEEN $cadfecha) order by cargo";
...
$sql="SELECT     distinct Horas.cargo, Proyectos.nombre AS NombreProyecto,  Horas.fecha,
	...
	WHERE     (Horas.unidad = $MiUnidad) AND (Horas.fecha BETWEEN $cadfecha) and horas.id_proyecto='$codProyecto'
...
?>
<table>
	<!--<tr><td><a href="#"><input name=atras type=button class="Boton" onClick="MM_goToURL('parent','RevisarHojasdeTiempo.php');return document.MM_returnValue" value="  Atras   "></a></td><td><a href="#"><input name=atras type=button class="Boton" onClick="MM_goToURL('parent','frm-GrabaTiempo.php');return document.MM_returnValue" value="  Inicio   "></a></td></tr>-->
</table>
<?php
echo "</table>";
?>
