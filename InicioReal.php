...
<FORM action=InicioReal.php method=post name="form2">
...
if ($ds) {
	...
	if ($ldapbind) {
		...
		if ($res=mssql_query($sql)) {
			...
			echo "<script>location.href=\"InicioReal.php\"</script>";
		}

			echo "<script>location.href=\"frm-GrabaTiempo.php?laUnidad=$laUnidad\"</script>";
		}else {
			//echo "<script>alert('Hoja de tiempo en mantenimiento durante 20 minutos ');</script>";
			echo "<script>alert('No es posible autenticar, favor revisar usuario y clave !');</script>";
			echo "<SCRIPT LANGUAGE=\"JavaScript\">";
			echo "location.href=\"InicioReal.php\"";
			echo "</SCRIPT>";
		}
	}
}
