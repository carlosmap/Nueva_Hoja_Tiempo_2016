...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    &nbsp;<input name="Submit4" type="submit" class="Boton" onclick="MM_callJS('history.back()')" value="Revisi&oacute;n Contratos" />
...
<?
	...
	<table width=100%  border=1 cellspacing=0 cellpadding=0>
	  <tr align=center class=TituloUsuario>
		<td width=5%>A</td>
		<td width=5%>B</td>
		<td width=5%>C</td>
		<td width=5%>LT</td>
...
//11Ene2011-casillas
//Encontrar la localización de acuerdo con el día anterior 
//**************************

//Verifica si es personal de planilla
$localizaVac="&nbsp;";
$localizaEnf="&nbsp;";
$localizaAcc="&nbsp;";
$localizaPer="&nbsp;";
...
$esPlanilla="No";
$sqlPlan="select usuarios.unidad , substring(Categorias.nombre,1,2) nomCategoria ";
$sqlPlan=$sqlPlan." from usuarios inner join Categorias on Usuarios.id_categoria = Categorias.id_categoria ";
$sqlPlan=$sqlPlan." where unidad =" . $verUnidadDe ;
$cursorPlan = mssql_query($sqlPlan);
if ($regPlan=mssql_fetch_array($cursorPlan)) {
	$esCategoria =  $regPlan[nomCategoria] ;
	if ($esCategoria > 50) {
		$esPlanilla="Si";
		$localizaVac="3";
	}
...
if ($esPlanilla="No") {
	//Vacaciones
	$miFechaVac = "";
	$sqlVac0="select MIN(fecha) laFecha from horas ";
	$sqlVac0=$sqlVac0." where unidad = " . $verUnidadDe ;
	$sqlVac0=$sqlVac0." and MONTH(fecha)= " . $MiMes ;
	$sqlVac0=$sqlVac0." and YEAR(fecha)= " . $MiAnno ;
	$sqlVac0=$sqlVac0." and id_proyecto = 56 "; //Vacaciones
	$cursorVac0 = mssql_query($sqlVac0);
	if ($regVac0=mssql_fetch_array($cursorVac0)) {
		$miFechaVac =  date("Y/n/d", strtotime($regVac0[laFecha])) ;
	}
	if ($miFechaVac != "") {
		$sqlVac1="Select * from ( ";
		$sqlVac1=$sqlVac1." select localizacion, MAX(fecha) laFecha from horas ";
		$sqlVac1=$sqlVac1." where unidad = " . $verUnidadDe ;
		$sqlVac1=$sqlVac1." and fecha < '".$miFechaVac."'";
		$sqlVac1=$sqlVac1." and id_proyecto not in (56, 60, 61, 62, 63, 64, 65) ";
		$sqlVac1=$sqlVac1." group by localizacion) A ";
		$sqlVac1=$sqlVac1." order by  laFecha desc	 ";
		$cursorVac1 = mssql_query($sqlVac1);
		if ($regVac1=mssql_fetch_array($cursorVac1)) {
			$localizaVac =  $regVac1[localizacion] ;
		}
	}

	//Enfermedades
	$miFechaEnf = "";
	$sqlEnf0="select MIN(fecha) laFecha from horas ";
	$sqlEnf0=$sqlEnf0." where unidad = " . $verUnidadDe ;
	$sqlEnf0=$sqlEnf0." and MONTH(fecha)= " . $MiMes ;
	$sqlEnf0=$sqlEnf0." and YEAR(fecha)= " . $MiAnno ;
	$sqlEnf0=$sqlEnf0." and id_proyecto = 60 "; //Enfermedades
	$cursorEnf0 = mssql_query($sqlEnf0);
	if ($regEnf0=mssql_fetch_array($cursorEnf0)) {
		$miFechaEnf =  date("Y/n/d", strtotime($regEnf0[laFecha])) ;
	}
	if ($miFechaEnf != "") {
		$sqlEnf1="Select * from ( ";
		$sqlEnf1=$sqlEnf1." select localizacion, MAX(fecha) laFecha from horas ";
		$sqlEnf1=$sqlEnf1." where unidad = " . $verUnidadDe ;
		$sqlEnf1=$sqlEnf1." and fecha < '".$miFechaEnf."'";
		$sqlEnf1=$sqlEnf1." and id_proyecto not in (56, 60, 61, 62, 63, 64, 65) ";
		$sqlEnf1=$sqlEnf1." group by localizacion) A ";
		$sqlEnf1=$sqlEnf1." order by  laFecha desc	 ";
		$cursorEnf1 = mssql_query($sqlEnf1);
		if ($regEnf1=mssql_fetch_array($cursorEnf1)) {
			$localizaEnf =  $regEnf1[localizacion] ;
		}
	}	

	//Accidentes
	$miFechaAcc = "";
	$sqlAcc0="select MIN(fecha) laFecha from horas ";
	$sqlAcc0=$sqlAcc0." where unidad = " . $verUnidadDe ;
	$sqlAcc0=$sqlAcc0." and MONTH(fecha)= " . $MiMes ;
	$sqlAcc0=$sqlAcc0." and YEAR(fecha)= " . $MiAnno ;
	$sqlAcc0=$sqlAcc0." and id_proyecto = 61 "; //Accidnetes de trabajo
	$cursorAcc0 = mssql_query($sqlAcc0);
	if ($regAcc0=mssql_fetch_array($cursorAcc0)) {
		$miFechaAcc =  date("Y/n/d", strtotime($regAcc0[laFecha])) ;
	}
	if ($miFechaAcc != "") {
		$sqlAcc1="Select * from ( ";
		$sqlAcc1=$sqlAcc1." select localizacion, MAX(fecha) laFecha from horas ";
		$sqlAcc1=$sqlAcc1." where unidad = " . $verUnidadDe ;
		$sqlAcc1=$sqlAcc1." and fecha < '".$miFechaAcc."'";
		$sqlAcc1=$sqlAcc1." and id_proyecto not in (56, 60, 61, 62, 63, 64, 65) ";
		$sqlAcc1=$sqlAcc1." group by localizacion) A ";
		$sqlAcc1=$sqlAcc1." order by  laFecha desc	 ";
		$cursorAcc1 = mssql_query($sqlAcc1);
		if ($regAcc1=mssql_fetch_array($cursorAcc1)) {
			$localizaAcc =  $regAcc1[localizacion] ;
		}
	}	

	//Permisos
	$miFechaPer = "";
	$sqlPer0="select MIN(fecha) laFecha from horas ";
	$sqlPer0=$sqlPer0." where unidad = " . $verUnidadDe ;
	$sqlPer0=$sqlPer0." and MONTH(fecha)= " . $MiMes ;
	$sqlPer0=$sqlPer0." and YEAR(fecha)= " . $MiAnno ;
	$sqlPer0=$sqlPer0." and id_proyecto = 62 "; //Permisos de trabajo
	$cursorPer0 = mssql_query($sqlPer0);
	if ($regPer0=mssql_fetch_array($cursorPer0)) {
		$miFechaPer =  date("Y/n/d", strtotime($regPer0[laFecha])) ;
	}
	if ($miFechaPer != "") {
		$sqlPer1="Select * from ( ";
		$sqlPer1=$sqlPer1." select localizacion, MAX(fecha) laFecha from horas ";
		$sqlPer1=$sqlPer1." where unidad = " . $verUnidadDe ;
		$sqlPer1=$sqlPer1." and fecha < '".$miFechaPer."'";
		$sqlPer1=$sqlPer1." and id_proyecto not in (56, 60, 61, 62, 63, 64, 65) ";
		$sqlPer1=$sqlPer1." group by localizacion) A ";
		$sqlPer1=$sqlPer1." order by  laFecha desc	 ";
		$cursorPer1 = mssql_query($sqlPer1);
		if ($regPer1=mssql_fetch_array($cursorPer1)) {
			$localizaPer =  $regPer1[localizacion] ;
		}
	}	

	//Licencias
	$miFechaLic = "";
	$sqlLic0="select MIN(fecha) laFecha from horas ";
	$sqlLic0=$sqlLic0." where unidad = " . $verUnidadDe ;
	$sqlLic0=$sqlLic0." and MONTH(fecha)= " . $MiMes ;
	$sqlLic0=$sqlLic0." and YEAR(fecha)= " . $MiAnno ;
	$sqlLic0=$sqlLic0." and id_proyecto = 63 "; //Licencias
	$cursorLic0 = mssql_query($sqlLic0);
	if ($regLic0=mssql_fetch_array($cursorLic0)) {
		$miFechaLic =  date("Y/n/d", strtotime($regLic0[laFecha])) ;
	}
	if ($miFechaLic != "") {
		$sqlLic1="Select * from ( ";
		$sqlLic1=$sqlLic1." select localizacion, MAX(fecha) laFecha from horas ";
		$sqlLic1=$sqlLic1." where unidad = " . $verUnidadDe ;
		$sqlLic1=$sqlLic1." and fecha < '".$miFechaLic."'";
		$sqlLic1=$sqlLic1." and id_proyecto not in (56, 60, 61, 62, 63, 64, 65) ";
		$sqlLic1=$sqlLic1." group by localizacion) A ";
		$sqlLic1=$sqlLic1." order by  laFecha desc	 ";
		$cursorLic1 = mssql_query($sqlLic1);
		if ($regLic1=mssql_fetch_array($cursorLic1)) {
			$localizaLic =  $regLic1[localizacion] ;
		}
	}	

	//Sanciones
	$miFechaSan = "";
	$sqlSan0="select MIN(fecha) laFecha from horas ";
	$sqlSan0=$sqlSan0." where unidad = " . $verUnidadDe ;
	$sqlSan0=$sqlSan0." and MONTH(fecha)= " . $MiMes ;
	$sqlSan0=$sqlSan0." and YEAR(fecha)= " . $MiAnno ;
	$sqlSan0=$sqlSan0." and id_proyecto = 64 "; //Sanciones
	$cursorSan0 = mssql_query($sqlSan0);
	if ($regSan0=mssql_fetch_array($cursorSan0)) {
		$miFechaSan =  date("Y/n/d", strtotime($regSan0[laFecha])) ;
	}
	if ($miFechaSan != "") {
		$sqlSan1="Select * from ( ";
		$sqlSan1=$sqlSan1." select localizacion, MAX(fecha) laFecha from horas ";
		$sqlSan1=$sqlSan1." where unidad = " . $verUnidadDe ;
		$sqlSan1=$sqlSan1." and fecha < '".$miFechaSan."'";
		$sqlSan1=$sqlSan1." and id_proyecto not in (56, 60, 61, 62, 63, 64, 65) ";
		$sqlSan1=$sqlSan1." group by localizacion) A ";
		$sqlSan1=$sqlSan1." order by  laFecha desc	 ";
		$cursorSan1 = mssql_query($sqlSan1);
		if ($regSan1=mssql_fetch_array($cursorSan1)) {
			$localizaSan =  $regSan1[localizacion] ;
		}
	}	

	//Ausencias
	$miFechaAus = "";
	$sqlAus0="select MIN(fecha) laFecha from horas ";
	$sqlAus0=$sqlAus0." where unidad = " . $verUnidadDe ;
	$sqlAus0=$sqlAus0." and MONTH(fecha)= " . $MiMes ;
	$sqlAus0=$sqlAus0." and YEAR(fecha)= " . $MiAnno ;
	$sqlAus0=$sqlAus0." and id_proyecto = 65 "; //Ausencias
	$cursorAus0 = mssql_query($sqlAus0);
	if ($regAus0=mssql_fetch_array($cursorAus0)) {
		$miFechaAus =  date("Y/n/d", strtotime($regAus0[laFecha])) ;
	}
	if ($miFechaAus != "") {
		$sqlAus1="Select * from ( ";
		$sqlAus1=$sqlAus1." select localizacion, MAX(fecha) laFecha from horas ";
		$sqlAus1=$sqlAus1." where unidad = " . $verUnidadDe ;
		$sqlAus1=$sqlAus1." and fecha < '".$miFechaAus."'";
		$sqlAus1=$sqlAus1." and id_proyecto not in (56, 60, 61, 62, 63, 64, 65) ";
		$sqlAus1=$sqlAus1." group by localizacion) A ";
		$sqlAus1=$sqlAus1." order by  laFecha desc	 ";
		$cursorAus1 = mssql_query($sqlAus1);
		if ($regAus1=mssql_fetch_array($cursorAus1)) {
			$localizaAus =  $regAus1[localizacion] ;
		}
	}	
...
*/
	$sql="SELECT estadoAprobDivision, comentariosDivision, revisadoPorDivision, estadoAprobProyecto, comentariosProyecto,
		revisadoPorProyecto, RTRIM(localizacion) + '-' + RTRIM(cargo) AS codigoproyecto,
		 ...
		 localizacion AS cualLocalizaEs, RTRIM(cargo) AS cualCargoEs
		 FROM Horas
		WHERE     (fecha BETWEEN $cadfecha) AND (unidad = '$verUnidadDe') AND (cargo <> 'acc0')
		AND (cargo <> 'aus0') AND (cargo <> 'enf0') AND (cargo <> 'lic0') AND (cargo <> 'per0')
         AND (cargo <> 'san0') AND (cargo <> 'vac0')ORDER BY codigoproyecto, clase_tiempo, id_actividad";
...
//11Ene2011-casillas-casillas
		$nLocalizaActiv = $filas[cualLocalizaEs];
		//$nCargoActiv = $filas[cualCargoEs];
		$nCargoActiv = substr($filas[cualCargoEs], 2, strlen($filas[cualCargoEs]));  
...
/*********************************************IMPRIME LAS HORAS FACTURADAS***********************************************/
		...
		while($filas){
			...
			while(($codproy==$filas[codigoproyecto]) and ($ttiempo==$filas[clase_tiempo]) and ($mactividad==$filas[id_actividad])){
					...
					*/
					$sqlAct = "SELECT Proyectos.nombre AS proyecto, Proyectos.codigo AS codigoDelProyecto, Actividades.nombre AS nombre, 
								Actividades.macroactividad AS macroactividad 
					...
					//11ENE2011-casillas
					$nElCodigoProyecto = $filasAct[codigoDelProyecto];
			...
			echo "<td nowrap><font face='Arial' size=1>
			<table width=100%  border=1 cellspacing=0 cellpadding=0>
			  <tr>
				<td width=5%>$nLocalizaActiv</td>
				<td width=5%>$nElCodigoProyecto</td>
				<td width=5%>$nCargoActiv</td>
				<td width=5%>[$act]</td>
				<td>$nProyecto</td>
			  </tr>
			</table>
...
//			echo "<td><center>$vacaciones[$i]</center></td>";
		...
		echo "<td colspan=4>$vacaciones[35]</td>";
		echo "</tr>";
...
//			echo "<td><center>$enfermedad[$i]</center></td>";
		...
		echo "<td colspan=4>$enfermedad[35]</td>";
		echo "</tr>";
...
//			echo "<td><center>$acciddetrabajo[$i]</center></td>";
		...
		echo "<td colspan=4>$acciddetrabajo[35]</td>";
		echo "</tr>";
...
//			echo "<td><center>$permisospacto[$i]</center></td>";
		...
		echo "<td colspan=4>$permisospacto[35]</td>";
		echo "</tr>";
...
//			echo "<td><center>$licencias[$i]</center></td>";
		...
		echo "<td colspan=4>$licencias[35]</td>";
		echo "</tr>";
...
//			echo "<td><center>$sanciones[$i]</center></td>";
		...
		echo "<td colspan=4>$sanciones[35]</td>";
		echo "</tr>";
...
//			echo "<td><center>$ausencias[$i]</center></td>";
		...
		echo "<td colspan=4>$ausencias[35]</td>";
		echo "</tr>";
...
<tr>
  <td colspan="3" class="TxtTabla"><B>Nota Devolución Contratos: </B><? echo $mensajeContratos; ?></td>
  </tr>
<? } ?>
</table>
<div id="Layer4">
