...
//CONSULTA QUE LA HOJA DE TIEMPO NO ESTE APROBADA OR EL JEFE 
$sql_vobo_HT=mssql_query("Select A.*, U.nombre, U.apellidos, U.retirado from VoBoFirmasHT A INNER JOIN usuarios U ON A.unidad=U.unidad
 where A.unidad = U.unidad AND A.mes =".$cualMes." AND A.vigencia = ".$cualVigencia." and A.unidad = ".$laUnidad." and A.validaJefe=1");

/*echo "Select A.*, U.nombre, U.apellidos, U.retirado from VoBoFirmasHT A INNER JOIN usuarios U ON A.unidad=U.unidad
 where A.unidad = U.unidad AND A.mes =".$cualMes." AND A.vigencia = ".$cualVigencia." and A.unidad = ".$laUnidad." and A.validaJefe=1 <br>".mssql_num_rows($sql_vobo_HT);
*/
if ( (mssql_num_rows($sql_vobo_HT))>=1 )
{
		echo "<script>alert('No es posible eliminar los viáticos debido a que, la hoja de tiempo del mes de ".$mes[$cualMes]." del ".$cualVigencia.", se encuentra aprobada.');</script>";
		exit();
}
