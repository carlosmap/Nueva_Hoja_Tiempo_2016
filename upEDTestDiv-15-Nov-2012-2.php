...
//echo $operacion;
		...
		$cursorTran1 = mssql_query(" BEGIN TRANSACTION ");
			$error="no";
			//consultamos la division de  origen, para el intercambio/movimiento
...
*/
		...
		if($operacion==1)
		{	
			//consultamos la division destino, para el intercambio/movimiento
...
//echo "-4 ".$sql_div_lt2."  --  ".mssql_get_last_message()." <br><br>";
			...
			//si en el lote de trabajo de destino, ya existe la division que se va a intercambiar
			if($cant_div>=1)
			{
				$error="si";
				$ban_encon="si";
...
//		echo "6.0 ".$sql_act."  --  ".mssql_get_last_message()." --  $macro_act_3 <br><br>";
		...
		echo "9 ". $sql_lt2."  -- $error ".mssql_get_last_message()."<br><br>";
						...
						//si no se encontraron registros, quiere indicar que el lote de control  no tiene divisiones
						//situacion 1.
...
//		$cursorTran2 = mssql_query(" ROLLBACK TRANSACTION ");
		...
		echo ("<script language='JavaScript' type='text/JavaScript'>alert('Operación realizada satisfactoriamente.');</script>"); 
	} 
	else {
		$cursorTran2 = mssql_query(" ROLLBACK TRANSACTION ");
		...
		echo ("<script language='JavaScript' type='text/JavaScript'>alert('Error durante la grabación');</script>");
	}
//	echo ("<script language='JavaScript' type='text/JavaScript'>window.close();MM_openBrWindow('htProgProyectos01.php?cualProyecto=".$cualProyecto."','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");
 ...
 ?>
		...
		<select name="LC" id="LC"   class="CajaTexto"  onChange="document.Form1.submit();">
            <?php
					...
					$sql_LC=$sql_LC."  order by cast(reverse(substring(reverse(macroactividad),1,charindex('C', reverse(macroactividad))-1)) as varchar) ";
...
<?php
//echo $sql_LC." ".mssql_get_last_message();
...
//	alert(document.getElementById("operacion").value);
	...
	if(document.Form1.operacion[0].checked)
	{
		//si la opercacion seleccionada es intercambio
			...
			{
				alert ('Seleccione un a división');
			}
/*		
