...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <?
		$sqlDiv = "Select id_proyecto from TMPActividadesHT2 Where id_proyecto = ".$cualProyecto;
		$qryDiv = mssql_fetch_array( mssql_query( $sqlDiv ) );
		if( $_SESSION['sesUnidadUsuario'] == $row[division] )
	?>
  <tr>
  ...
  <?
    $sql = "Select count(*) reg From TMPActividadesHT2 WHERE id_proyecto =  ".$cualProyecto." AND id_encargado = ".$_SESSION['sesUnidadUsuario'];	  #id_encargado 
    $qry = mssql_fetch_array( mssql_query( $sql ) );
	#echo "<tr><td colspan='2'>".$qry[reg]."<br />".$sql."</td></tr>";
    if( $qry[reg] == 0 ){
  ?>	
  <tr>
    <td class="TituloTabla">Lote de control</td>
    <td align="left" class="TxtTabla"><?
			$sqlLc = "select * from HojaDeTiempo.dbo.TMPActividadesHT2 Where id_proyecto = ".$cualProyecto." AND nivel = 1";
  ...
  <tr>
    ...
    <td align="left" class="TxtTabla"><?
			$sqlLt = "select * from HojaDeTiempo.dbo.TMPActividadesHT2 Where id_proyecto = ".$cualProyecto." AND nivel = 2 ";
			if( trim($lc) != "" )
  ...
  <tr>
    ...
    <td align="left" class="TxtTabla">
		<?
			if( $qry[reg] != 0 )
				$sqlDv = "select distinct macroactividad, id_division, nombre from HojaDeTiempo.dbo.TMPActividadesHT2 Where id_proyecto = ".$cualProyecto." AND nivel = 3 AND id_encargado = ".$_SESSION['sesUnidadUsuario'];
			else
				$sqlDv = "select distinct macroactividad, id_division, nombre from HojaDeTiempo.dbo.TMPActividadesHT2 Where id_proyecto = ".$cualProyecto." AND nivel = 3";
  ...
  <tr>
    ...
    <td class="TxtTabla">
      <?
		$sqlActividad = "SELECT macroactividad, id_actividad, nombre, id_proyecto, id_encargado FROM TMPActividadesHT2 
						 WHERE dependeDe in( Select id_actividad from TMPActividadesHT2 where id_division = ".$pfDivision." AND id_proyecto = ".$cualProyecto." ) 
