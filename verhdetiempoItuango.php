...
//Si se carga por primera vez la hoja de tiempo trae el mes y ano actual
if (trim($Flmes) == "") {
	$qSql=$qSql." where vigencia = year(getdate()) ";
	$qSql=$qSql." and mes = month(getdate()) ";
}
// si no, trae la información correspondiente al mes y ano seleccionados en la lista
else {
...
//tabla que hari parte del encabezado
...
//6Julio2007
//Consulta para verificar si la Hoja ya esti aprobada o no por el jefe seleccionado
$muestraBoton = 0;
$firmaJefeAprueba = "";
$ObsJefeAprueba = "";
$muestraContratos = 0;
...
$qSql2="Select vigencia, mes, unidad, unidadJefe, validaJefe, comentaJefe, validaContratos, unidadContratos, comentaContratos ";
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><input name="BotonReg" type="submit" class="Boton" id="BotonReg" onclick="MM_callJS('history.back()')" value="Listado de usuarios" />
    &nbsp;&nbsp;
<?
//El botón sólo aparec si la Hoja de tiempo no ha sido aprobada por el jefe
//if ($muestraBoton == 0) {

//13Oct2007
//Para que desactive el botón por si acaos luego quieren hacer el trimite desde aqui
if ($mostrar == "Soy yo") {
?>    <input name="BotonHT" type="submit" class="Boton" id="BotonHT" onclick="MM_openBrWindow('upAutorizaHTjefe.php?cualUnidad=<? echo $verUnidadDe; ?>&cualMes=<? echo $Flmes; ?>&cualAno=<? echo $Flano; ?>','vUPjef','scrollbars=yes,resizable=yes,width=400,height=250')" value="Tr&aacute;mite de Aprobaci&oacute;n" />
	<? } ?>
   	</td>
  </tr>
</table>
...
//	$vacaciones[1]="1";
	...
	$primas[2]="<center><b>DIAS DE VIITICOS, PRIMA DE LOCALIZACION, AUXILIO DE TRASLADO O AUXILIO ALIMENTACION</b></center>";
	for($i=3;$i<=37;$i++){
		$primas[$i]=" ";
	}
...
/*
//	echo "<table class='TxtTabla' border='1' bordercolor=black width=98%>";
	...
	echo "<td colspan='14' valign=top align=left><font face=arial size=2><b>".strtoupper($nomb)." ".strtoupper($apel)."    $verUnidadDe"."-"."$categoria";
...
//--Trae la información del usuario con nombre del departamento y nombre de la categoria
	...
	if ($encabReg=mssql_fetch_array($encabCursor)) {
		...
		echo "<table class='TxtTabla' border='1' cellpadding='0' cellspacing='0' bordercolor='#999999' >";
		echo "<tr>";
		echo "<td colspan='6'><img src='pics/LogoIngetecPNG2.png' width='150' heigth='75'></td>";
		echo "<td colspan='22'><font face=arial size=4><h3><center>HOJA DE TIEMPO<br>$fechasistema<br>$vHLab</center></h3></font></td>";
...
/*********************************************IMPRIME LAS HORAS FACTURADAS***********************************************/
		...
		while($filas){
			...
			$nombreCompletoF = ucwords ($filasAct[proyecto]);

			echo "<td nowrap><font face='Arial' size=1><img src='imagenes/mnuFlecha.gif' alt='$codproy [$act] $nombreCompletoF' width='12' height='13' />$codproy [$act] $nProyecto</font></td>";
			...
			//10Jul2007
			//Busca el registro en AprobacionFacHT para el mes y ano del registro y verifica si fue o no aprobado y quien o hizo
			$vBJefe="";
			$vBNombre="";
			$vBComenta="";
			$cualImagenvb = "";
...
//			echo "<td><center>$vacaciones[$i]</center></td>";
			...
			//6Dic2007
			//Busca el registro en AprobacionFacHT para el mes y ano del registro de vacaciones y verifica si fue o no aprobado y quien o hizo
			if ($i == 34) {
				$vBJefe="";
				$vBNombre="";
				$vBComenta="";
...
//			echo "<td><center>$enfermedad[$i]</center></td>";
			...
			//6Dic2007
			//Busca el registro en AprobacionFacHT para el mes y ano del registro de enfermedad y verifica si fue o no aprobado y quien o hizo
			if ($i == 34) {
				$vBJefe="";
				$vBNombre="";
				$vBComenta="";
...
//			echo "<td><center>$acciddetrabajo[$i]</center></td>";
			...
			//6Dic2007
			//Busca el registro en AprobacionFacHT para el mes y ano del registro de accidentes de trabajo y verifica si fue o no aprobado y quien o hizo
			if ($i == 34) {
				$vBJefe="";
				$vBNombre="";
				$vBComenta="";
...
//			echo "<td><center>$permisospacto[$i]</center></td>";
			...
			//6Dic2007
			//Busca el registro en AprobacionFacHT para el mes y ano del registro de permisospacto  y verifica si fue o no aprobado y quien o hizo
			if ($i == 34) {
				$vBJefe="";
				$vBNombre="";
				$vBComenta="";
...
//			echo "<td><center>$licencias[$i]</center></td>";
			//6Dic2007
			//Busca el registro en AprobacionFacHT para el mes y ano del registro de licencias  y verifica si fue o no aprobado y quien o hizo
			if ($i == 34) {
				$vBJefe="";
				$vBNombre="";
				$vBComenta="";
...
//			echo "<td><center>$sanciones[$i]</center></td>";
			...
			//6Dic2007
			//Busca el registro en AprobacionFacHT para el mes y ano del registro de sanciones  y verifica si fue o no aprobado y quien o hizo
			if ($i == 34) {
				$vBJefe="";
				$vBNombre="";
				$vBComenta="";
...
//			echo "<td><center>$ausencias[$i]</center></td>";
			//6Dic2007
			//Busca el registro en AprobacionFacHT para el mes y ano del registro de ausencias  y verifica si fue o no aprobado y quien o hizo
			if ($i == 34) {
				$vBJefe="";
				$vBNombre="";
				$vBComenta="";
...
*/
			//14Nov2007
			//Para mostrar los viiticos discriminados por actividad y visualizar la macroactividad			
			...
			if($resultado=mssql_query($sql)){
				...
				if(mssql_num_rows($resultado)>0){
					...
					$tipoViatico=$filas[IDTipoViatico]; //20140325
					while($filas) {
						$cualMacroActividad = $filas[macroactividad] ;
						$cantDias = 0;
						
						//20140325
						while(($idActividad==$filas[id_actividad]) AND ($codigo == $filas[codigo] ) AND ($tipoViatico == $filas[IDTipoViatico] ) ) // ****2014 Adicion de AND ($tipoViatico == $filas[IDTipoViatico] )
						   {    //20140325
								//Nuevo
/*
...
/******************************************************************************************************/
							  //20140325
							  /***** 2014 :  Se adiciona consulta para hallar Nombre viatico, proyecto y sitio *****************************/	

							   $tipoViatico = $filas[IDTipoViatico];
							   $idProyect =  $filas[id_proyecto];
							   $iddelSitio = $filas[IDSitio];
							   $sqlConsulta= "SELECT 
				   				  (select NomTipoViatico from tiposviatico where idtipoviatico = '$tipoViatico') AS nomTipoViatico,
								  (select nombre from proyectos where id_proyecto = $idProyect) AS nomdelProyecto,
								  (select NomSitio from sitiostrabajo where IDSitio = $iddelSitio and id_proyecto = $idProyect) AS nomdelSitio";
							   $cursorConsulta = mssql_query($sqlConsulta);			  
							   if($regConsulta = mssql_fetch_array($cursorConsulta))
							   {
									$nomTipoViatico=$regConsulta[nomTipoViatico];
									$nomdelProyecto = $regConsulta[nomdelProyecto];
									$nomdelSitio = $regConsulta[nomdelSitio];
							   }
							   /***********************************************************************************************************/
								...
								//Nuevo
								//20140325
								/******************2014 SE COMENTAREA PARA COLOCAR LOS RESULTADOS DE CADA TIPO DE VIATICO O AUXILIO UNICAMENTE EN LA UBICACIÓN 32 DEL ARREGLO ************/
								/*								
								...
								*/
								...
								echo "<tr>";
								for($i=-1;$i<=32;$i++){ //20140325  $i<=34 a  $i<=32
									...
									else
									{
										if($i == 32)
										{
											...
											echo "<td align='center'>Viiticos<br>$viaticos[$i]</td>";
											 $viaticos[$i]=" ";
										}
										elseif($i == 33)
										{
									...
									*/
									...
									{
										if($viaticos[$i]==NULL) $viaticos[$i]=" ";
										echo "<td align='center' colspan='3'>$nomTipoViatico<br>$viaticos[$i]</td>";
										 $viaticos[$i]=" ";
									}										
									else
									{
		...
		<br>10 Por compensar
		<br>11 Compensado
		<br>Viitico (Clase o Localidad)
		</td>
		<TD COLSPAN=4>6 am-10 pm<br>6 am-10 pm<br>0am-6am y 10pm-12pm<br>0am-6am y 10pm-12pm</TD>
		<TD COLSPAN=6> </TD>
		<TD COLSPAN=5 align=center> $cualImagen <br> $miUsuarioJefeFirma </TD>
		<TD colspan=5> </TD><TD align=center> $cualImagenCont <br> $miUsuarioContrato</TD><TD>
		 </TD></tr>";
...
<p class=titulo>Instrucciones para imprimir la hoja de tiempo</p>
	...
	6. Escoja la hoja tamano carta y coloquela en sentido apaisado.<br>
