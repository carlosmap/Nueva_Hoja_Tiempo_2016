...
$sql=$sql." where (nombre <> '' and nombre <> 'sd') ";
$sql=$sql." order by nombre ";
$cursorDiv = mssql_query($sql);
...
if ($HorasAsignadas != "") {
	...
	if ($HorasAsignadas == 0) {
		...
		if ($btnGraba == "Grabar y Continuar") {
			echo ("<script>MM_openBrWindow('ProgProyectosActiv.php?cualProyecto=$miProyecto&cualActividad=$miActividad','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");	
		}
		else {
...
if ((parseFloat(document.Form1.hombresMes.value) > 0) && (parseFloat(document.Form1.hombresMes.value) <= 1)) {
	if ((document.Form1.pClase.value == 1) || (document.Form1.pClase.value == 2)) {
		mHoras = parseFloat(document.Form1.horasMes.value) * parseFloat(document.Form1.hombresMes.value);
	}
	else {
		//Si clase de tiempo es mayor que 1 o 2 las horas del mes son 240
		mHoras = 240 * parseFloat(document.Form1.hombresMes.value);
	}
	document.Form1.HorasAsignadas.value = '';
}
else {
	alert("Hombres/Mes debe ser un valor mayor que 0 y menor o igual a 1");
	document.Form1.hombresMes.value = '';
	document.Form1.hombresMes.focus() ;
}
//alert (document.Form1.pClase.value);
//alert (document.Form1.hombresMes.value);
//document.Form1.recarga.value="1";	
//document.Form1.submit();
}
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td class="TxtTabla">
		<select name="pDivision" class="CajaTexto" onChange="envia1()" >
...
/*		$sql2="Select * from Usuarios  "  ;
		...
		$sql2="SELECT U.*  "  ;
		$sql2=$sql2." FROM usuarios U, departamentos D ";
...
//	echo $cualSalario;
	...
	?>
    <input name="esteSalario" type="hidden" id="esteSalario" value="<? echo $cualSalario ; ?>">	</td>
  </tr>
  <tr>
...
<?
  ...
  <tr>
	...
	?>
    <input name="horasMes" type="hidden" id="horasMes" value="<? echo $horasLaboralesB; ?>">	</td>
  </tr>
  <tr>
    <td class="TituloTabla">Hombres/Mes</td>
    <td class="TxtTabla"><input name="hombresMes" type="text" class="CajaTexto" id="hombresMes" onChange="envia3()"  onKeypress="if ( event.keyCode == 110 || event.keyCode < 45 || event.keyCode > 57  ) event.returnValue = false;"  value="<? echo $hombresMes; ?>" size="20"></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<input name="btnGraba" type="button" class="Boton" onClick="envia2()" value="Grabar y Continuar"   >
	<input name="btnGraba" type="button" class="Boton" id="btnGraba" onClick="envia2()" value="Grabar y Cerrar" >
	<? } ?>
	</td>
  </tr>
  <tr>
