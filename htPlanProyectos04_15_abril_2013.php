...
//echo $sql_proyecto_valor_total."<br>".mssql_get_last_message();
	...
	if(trim($val_proye)=="")
		$val_proye=0;
			...
			?>
          <tr class="TxtTabla">
            <td class="TituloTabla2" >Valor del proyecto </td>
			<td> $<? echo number_format($val_proye, "2", ",", "." )   ?></td>
			<? if ($z==1)
				{ ?>
        	    <td width="15%" align="right" rowspan="<? echo 2; ?>">  <img src='grafico.jpg' border='0' width="495" height="289" /> <? echo $image; ?> </td>
			<?  } 
			...
			?>
	          ...
	          <tr class="TxtTabla">
    	        <td class="TituloTabla2" >Valor del facturado</td>
				<td> $<? echo number_format($val_proye, "2", ",", "." )   ?></td>
			  </tr>
  	      </table>			
		</td>
        ...
        <table  width="100%"  border="0" cellspacing="1" cellpadding="0">
          <tr>
            <td  class="TituloUsuario" colspan="3" >Resumen del proyecto </td>
          </tr>
        </table>
		<table width="100%"  border="0" cellspacing="1" cellpadding="0">
          <tr>
          ...
          <tr class="TituloTabla2">
            ...
            <td width="15%">Valor Planeado </td>
            <td width="15%">Valor Facturado </td>
            <td width="15%">Valor planeado Vs Facturado por Divisi&oacute;n </td>
          </tr>
<?
	//CONSULTA QUE CONSOLIDA EL VALOR TOTAL ASIGNADO A CADA DIVISION, EN CADA PROYECTO
	...
	select nombre, SUM(valor) as valor_division FROM  HojaDeTiempo.dbo.Actividades WHERE id_proyecto = ".$cualProyecto." and nombre in (
	 SELECT distinct(nombre) FROM  HojaDeTiempo.dbo.Actividades WHERE id_proyecto = ".$cualProyecto." and nivel=3
	 ) and nivel=3 group by(nombre) ";
//echo $sql_division_valor_total."<br>".mssql_get_last_message();
	$cur_division_valor_total=mssql_query($sql_division_valor_total);
	$cant_filas=mssql_num_rows($cur_division_valor_total);
	$z=1;
	$total_valor_asignado_division=0;
	while($datos_division_valor_total=mssql_fetch_array($cur_division_valor_total))
	{

			?>
          <tr class="TxtTabla">
            <td><? echo strtoupper( $datos_division_valor_total["nombre"]); ?></td>
            <td width="15%" align="right">$<? echo number_format($datos_division_valor_total["valor_division"], "2", ",", "." )   ?></td>
            <td width="15%" align="right">$0 </td>		
			<? if ($z==1)
				{ ?>
        	    <td width="15%" align="right" rowspan="<? echo $cant_filas; ?>">  <img src='grafico.jpg' border='0' /> <? echo $image; ?> </td>
			<?  } 
				$total_valor_asignado_division+=$datos_division_valor_total["valor_division"];
			?>
          </tr>
<?
		$z++;
...
?>
			<tr class="TituloTabla">
				...
				<td  align="right" >$ <? echo number_format($total_valor_asignado_division, "2", ",", "." )  ?></td>
				<td  align="right">$ 0</td>
				<td></td>
			</tr>
