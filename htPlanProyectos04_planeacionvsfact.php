...
//FACTURACION SIN PLANEACION
			$sql_fact_sin_plane="select distinct(mes) mes from FacturacionProyectos where id_proyecto=".$cualProyecto." and id_actividad=".$datos_act_edt["id_actividad"]." and unidad=". $datos_parti["unidad"]." and vigencia=".$lstVigencia."  and esInterno='".$datos_parti["esInterno"]."'
and mes not in (select distinct(mes) from PlaneacionProyectos where id_proyecto=".$cualProyecto." and id_actividad=".$datos_act_edt["id_actividad"]." and unidad=". $datos_parti["unidad"]." and vigencia=".$lstVigencia."  and esInterno='".$datos_parti["esInterno"]."')";
			$cur_fact_sin_planea=mssql_query($sql_fact_sin_plane);

			$ban_fact_planea=0;
//			mssql_num_rows($cur_fact_sin_planea);
			if($datos_fact_sin_planea=mssql_fetch_array($cur_fact_sin_planea))
			{
				$ban_fact_planea=1;
			}
			if($ban_fact_planea==0)
			{

//FACTURACION QUE EXCEDE LA PLANEACION
				$sql_sobre_fact="	select * from (
					select *,
					(
						select SUM(hombresMes) total_planeacion from PlaneacionProyectos where id_proyecto=T1.id_proyecto and id_actividad=T1.id_actividad and unidad=T1.unidad and vigencia= T1.vigencia and esInterno=T1.esInterno and mes=T1.mes
						GROUP BY hombresMes,mes
					) total_planeacion
					 from (
						select distinct(mes) mes, SUM(hombresMesF) total_facturacion, id_actividad,id_proyecto,unidad,esInterno,vigencia from FacturacionProyectos where id_proyecto=".$cualProyecto." and id_actividad=".$datos_act_edt["id_actividad"]." and unidad=". $datos_parti["unidad"]." and vigencia=".$lstVigencia."  and esInterno='".$datos_parti["esInterno"]."'
						GROUP BY hombresMesF,mes, id_actividad,id_proyecto,unidad,esInterno,vigencia
					) T1 
					) T2
					where total_planeacion<total_facturacion ";
					$cur_fact_sin_planea=mssql_query($sql_sobre_fact);
...
?>
                        <td width="2%" rowspan="1" align="center" >
							 
<?
//echo $ban_fact_planea." --- ".$ban_fact_planea2." <br>";
						if($ban_fact_planea==1)
							echo " <img src='imagenes/alertaFactu.gif' title='Se registro facturación sin pnaeación'   width='15' height='16' /> ";

						else if($ban_fact_planea2==1)
							echo " <img src='imagenes/alertaPlane.gif' title='La facturación excede la planeación'   width='15' height='16' /> ";
						else
?>						
...
//echo " LC ".$Lote_control." LT ".$Lote_trabajo." DIV ".$Division." ACT ".$Actividad." <br>";
							...
							{
								//SI SE ESTA CONSULTANDO LAS ACTIVIDADES DE UNA DIVISION, SE CARGA LA VARIABLE DE Actividad, PARA ENVIARLA POR URL, A LA PAGINA DE DETALLE
								if($datos_act_edt["nivel"]==4) //SI ES LA ACTIVIDAD, DE NIVEL 4
								{														
									$Actividad=$datos_act_edt["id_actividad"];								
								}
...
//echo  mssql_num_rows($cur_parti)."**** <br>" ;
//echo $conts." ---- ".mssql_num_rows($cur_parti)."<br>";
							//SI SE TRHAE MAS DE UN PARTICIPANTE, EN LA CONSULTA, SE MUESTRA LA FRANJA GRSI QUE DIVIDE LOS PARTICIPANTES EN LA ACTIVIDAD
							if ( ( ( (int) mssql_num_rows($cur_parti) )>1 )and($conts< ( (int) mssql_num_rows($cur_parti))))
							{
//echo "Ingreso <br>";
			?>
								<tr>
										<td colspan="18" class="TituloTabla2"  > </td>
								</tr>
			<?php
							}
							$conts++;
...
?>
                      <tr class="TituloUsuario">
                        <td colspan="19" align="left" class="TituloUsuario"> </td>
                      </tr>
<?
				}
	}
