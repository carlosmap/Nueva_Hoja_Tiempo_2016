...
$mensaje= array('Cantidad planeados ','Cantidad facturados','Planeados con facturación','Facturación sin planeación','Planeados que no facturaron','Usuarios que sobrepasan lo planeado','Usuarios que estan por debajo de lo planeado');
		...
		//(CONSULTA 1) CANTIDAD DE USUARIOS DE LA DIVISION, QUE HAN SIDO PLANEADOS EN LOS PROYECTOS
		...
		where vigencia=".$vigencia." and PlaneacionProyectos.mes=".$pMes." and PlaneacionProyectos.esInterno='I' and Divisiones.id_division=".$pDivision;
...
//echo $sql_usu_plane." <br>";
		...
		// (CONSULTA 2) USUARIOS DE LA DIVISION, QUE FACTURARON EN ALGUN PROYECTOS
        ...
        inner join Departamentos on Usuarios.id_departamento=Departamentos.id_departamento
        inner join Divisiones on Departamentos.id_division=Divisiones.id_division
        where FacturacionProyectos.id_proyecto not in (61,65,71,60,63,62,64,56) and FacturacionProyectos.vigencia=".$vigencia." and FacturacionProyectos.mes=".$pMes." and FacturacionProyectos.esInterno='I'  and Divisiones.id_division=".$pDivision;
		if(trim($cualProyecto)!="")
		{
...
//echo $sql_factu." <br>";
		...
		// (CONSULTA 3) USUARIOS DE LA DIVISION, QUE FUERON PLANEADOS EN ALGUN PROYECTO Y TIENEN FACTURACION
		...
		// (CONSULTA 4) USUARIOS DE LA DIVISION, QUE GENERARON FACTURACION EN ALGUNA DE LAS ACTIVIDADES DE UN PROYECTO Y NO TENIA PLANEACION
		...
		inner join Departamentos on Usuarios.id_departamento=Departamentos.id_departamento
		inner join Divisiones on Departamentos.id_division=Divisiones.id_division
		where FacturacionProyectos.id_proyecto not in (61,65,71,60,63,62,64,56) and vigencia=".$vigencia." and mes=".$pMes." and FacturacionProyectos.esInterno='I' and Divisiones.id_division=".$pDivision." and FacturacionProyectos.unidad not in (
...
//echo "<br>".$sql_fac_sin_plan."---- <br>";
		...
		//(CONSULTA 5) USUARIOS DE LA DIVISION, QUE FUERON PLANEADOS EN LAS ACTIVIDADES DE UN PROYECTO, Y NO HAN FACTURADO
        ...
        inner join Departamentos on Usuarios.id_departamento=Departamentos.id_departamento
        inner join Divisiones on Departamentos.id_division=Divisiones.id_division
        where vigencia=".$vigencia."  and PlaneacionProyectos.mes=".$pMes." and PlaneacionProyectos.esInterno='I'  and Divisiones.id_division=".$pDivision." and PlaneacionProyectos.unidad not in (
...
?>
          <tr>
            ...
            <td class="TxtTabla"><?
			$qSql1="Select * from Departamentos where estadoDpto='A' and id_departamento=".$pDepto ;
			$qCursor1 = mssql_query($qSql1);
			$qReg1=mssql_fetch_array($qCursor1);
			echo ucwords(strtolower($qReg1[nombre]));
?>			</td>
...
-->
<?
	//CONSULTA LOS PROYECTOS, EN LOS CUALES SE HA PLANEADO O HAN FACTURADO, LOS USUARIOS DE LA DIVISION
	$sql_proys="SELECT * from (
				---PROYECTOS EN LOS QUE LOS USUARIOS DE LA DIVISION, HAN SIDO PLANEADOS
			(
				select  distinct Proyectos.nombre, Proyectos.id_proyecto, Proyectos.cargo_defecto, Proyectos.codigo from PlaneacionProyectos 
			...
			union
			---PROYECTOS EN LOS QUE LOS USUARIOS DE LA DIVISION, HAN FACTURADO
			(
			select  distinct Proyectos.nombre, Proyectos.id_proyecto, Proyectos.cargo_defecto, Proyectos.codigo from FacturacionProyectos 
...
?>
	<select name="cualProyecto" class="CajaTexto" id="cualProyecto">
		<option value="" >Todos</option>
<?
	while($datos_proys=mssql_fetch_array($cur_proy))
	{
		$sel="";
		if($cualProyecto==$datos_proys["id_proyecto"])
			$sel="selected";
		echo "<option value='".$datos_proys["id_proyecto"]."' ".$sel." >[".$datos_proys["codigo"].".".$datos_proys["cargo_defecto"]."] ".$datos_proys["nombre"]."</option>";
	}
?>
	          </select></td>
			</tr>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
        ...
        <table width="100%"  border="0"  bgcolor="#FFFFFF" >
          <tr class="TituloUsuario">
            ...
            <td width="2%"></td>
            <td width="63%"><table width="100%" border="0" cellpadding="0"  bgcolor="#FFFFFF" >
              <tr class="TituloUsuario">
                <td width="37%" ></td>
...
//--distinct PlaneacionProyectos.unidad, Usuarios.nombre, Usuarios.apellidos,Usuarios.fechaRetiro,Usuarios.retirado 
						...
						if($i==1)
						{
							...
							inner join Departamentos on Usuarios.id_departamento=Departamentos.id_departamento
							inner join Divisiones on Departamentos.id_division=Divisiones.id_division 
							where vigencia=".$vigencia." and FacturacionProyectos.mes=".$pMes." and FacturacionProyectos.esInterno='I' and Divisiones.id_division=".$pDivision;
							if(trim($cualProyecto)!="")
							{
...
//									}
									...
									$sql=$sql."and Divisiones.id_division=".$pDivision."
...
//echo $sql;
						...
						if($i==3)
						{
							...
							and Divisiones.id_division=".$pDivision." and FacturacionProyectos.unidad not in ( 
								select distinct(PlaneacionProyectos.unidad) unidad from PlaneacionProyectos 
								 inner join Usuarios USU on PlaneacionProyectos.unidad=Usuarios.unidad
								 inner join Departamentos DEP on Usuarios.id_departamento=DEP.id_departamento 
...
//echo $sql;
						...
						{                
							if($i==0)
							{
								...
								inner join Departamentos on Usuarios.id_departamento=Departamentos.id_departamento 
								inner join Divisiones on Departamentos.id_division=Divisiones.id_division
								where vigencia=".$vigencia." and mes=".$pMes." and Divisiones.id_division=".$pDivision." AND FacturacionProyectos.id_proyecto=".$datos_fac["id_proyecto"]."
								and FacturacionProyectos.unidad=".$datos_usu["unidad"] ."  
							...
							if($i==1)
							{
								...
								inner join Departamentos on Usuarios.id_departamento=Departamentos.id_departamento 
								inner join Divisiones on Departamentos.id_division=Divisiones.id_division
								where vigencia=".$vigencia." and mes=".$pMes." and Divisiones.id_division=".$pDivision." AND PlaneacionProyectos.id_proyecto=".$datos_fac["id_proyecto"]."
								and PlaneacionProyectos.unidad=".$datos_usu["unidad"] ."  
...
//echo mssql_get_last_message()." --- ".$sql_fact;
                ...
                ?>
                      <tr >
                        <td width="37%" class="TxtTabla"><?=$datos_fac["nombre"] ?></td>
					 ...
					 ?></td>
                        <td width="10%" class="TxtTabla"><?=$datos_fac["total_facturado_h_mes"] ?></td>
                        <td width="20%" class="TxtTabla"><?
...
/*
							...
							if($i==1)
							{
								if( ( (int) $datos_fac["total_planeacion_h_mes"] )>1 )
								{
									echo '<img title="Valor real de la divisi&oacute;n en todos los proyectos." src="/img/arrAlta.gif"> ';
								}
...
?>							
                ...
                ?>
                    ...
                    <td width="1%"  class="TxtTabla"><img src="img/images/ver.gif" width="16" height="16"  onclick="MM_openBrWindow('htFacturacion_via_fac.php?pMes=<?=$pMes ?>&amp;pAno=<?=$vigencia ?>&amp;unidad_u=<?=$datos_usu["unidad"] ?>','winAddFV','scrollbars=yes,resizable=yes,width=1400,height=400')" /></td>
                  </tr>
