...
//Trae la información del encabezado de la Programación de suma global para identificar el proyecto 
//y el nombre del programado.
$sqlSG="SELECT S.*, P.nombre, U.nombre nomPrograma, U.apellidos apePrograma ";
$sqlSG=$sqlSG." FROM ProgSumaGlobal S, Proyectos P, Usuarios U ";
$sqlSG=$sqlSG." WHERE S.id_proyecto = P.id_proyecto ";
$sqlSG=$sqlSG." AND S.unidadProgramador = U.unidad ";
$sqlSG=$sqlSG." AND S.id_proyecto = " . $cualProyecto ;
$sqlSG=$sqlSG." AND S.unidadProgramador =" . $cualUnidad ;
$cursorSG = mssql_query($sqlSG);
if ($regSG=mssql_fetch_array($cursorSG)) {	 
	$pnombre = ucwords(strtolower($regSG[nombre]));
	$pProgramador = ucwords(strtolower($regSG[nomPrograma])) . " " . ucwords(strtolower($regSG[apePrograma]));
}
...
if ($Justificacion != "") {
	...
	//Realiza la actualización en de libera = 0 en la suma global del proyecto y programador seleccionado
	$sqlUp="UPDATE ProgSumaGlobal SET liberar = '0' ";
	$sqlUp=$sqlUp." WHERE id_proyecto = " . $miProyecto ;
	$sqlUp=$sqlUp." AND unidadProgramador = " . $miUnidadProg ;
	$cursorUp = mssql_query($sqlUp);
	
	//Realiza la inserción de la bitácora para la Suma Global
	$sqlAd="INSERT INTO BitacoraSumaGlobal (id_proyecto, unidadProgramador, ";
	...
	echo ("<script>window.close();MM_openBrWindow('ProgProyectosGeneralSG.php?cualProyecto=$miProyecto&cualUnidad=$miUnidadProg','winProgProyectos','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");	
}
...
<html>
<head>
<title>Programaci&oacute;n de Proyectos - Suma Global</title>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="TituloUsuario">Programaci&oacute;n de proyectos - Suma Global </td>
  </tr>
</table>
