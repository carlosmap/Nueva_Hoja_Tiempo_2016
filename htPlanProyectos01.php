...
/*
	//aplicamos el estilo distintivo a la actividad seleccionada
	document.getElementById("fila"+id).style.background='#ababab';
	document.getElementById("fila"+id).style.color="white";

	//CAMBIA EL ESTILO EN LA TABLA vlAsig y vlDis, de las divisiones
	if(document.getElementById("fila_"+id)!=null)
	{
		document.getElementById("fila_"+id).style.background='#ababab';
		document.getElementById("fila_"+id).style.color="white";

		document.getElementById("fila1_"+id).style.background='#ababab';
		document.getElementById("fila1_"+id).style.color='white';
	}

    if(val==0)
        val=id;  //si es la primera vez que se  hace click en un icono, se almacena el id del elemento seleccionado
    else
    {
		//si se ha hecho clik sobre un icono con anteioridad, se deja el estilo original, utilizada en la clase
        document.getElementById("fila"+val).style.background='#E9E9E9';
		document.getElementById("fila"+val).style.color="#333366";

		if(document.getElementById("fila_"+val)!=null)
		{
			document.getElementById("fila_"+val).style.background='#E9E9E9';
			document.getElementById("fila_"+val).style.color="#333366";

			document.getElementById("fila1_"+val).style.background='#E9E9E9';
			document.getElementById("fila1_"+val).style.color="#333366";
		}
	...
	<form name="form1" id="form1" method="post" action="">
	  ...
	  <tr valign="top" class="TxtTabla">
		...
		<table width="100%"  border="1" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td width="40%" class="TituloTabla">Valor Real del proyecto: </td>
	  ...
	  <? } ?>
		...
		<?
		//28May2014
		//Visualiza el área de valores para el director, coordinador u ordenador de gasto.
		if ($verBoton=="SI") {
		?>
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr>
		...
		<? } //Cierra if ($verBoton=="SI") para mostrar área de divisiones?>
        <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td class="TxtTabla">&nbsp;</td>
          </tr>
      ...
      <tr>
        <td bgcolor="#FFFFFF"><table width="100%"  border="0" cellspacing="1" cellpadding="0">
          <tr class="TituloTabla2">
		   ...
		   ?>
          <tr class="TxtTabla" id="fila<? echo $reg02[id_actividad]; ?>" >
			...
			<? if (trim($reg02[nivel]) == "1") { ?>
			<a href="<?=$reg02[id_actividad] ?>" id="<?=$reg02[id_actividad] ?>"></a>
			<a href="#<?=$reg02[id_actividad] ?>"><img src="img/images/icoNew.gif" alt="Ingresar Lote de trabajo" width="16" height="15" border="0" onclick="MM_openBrWindow('addHtLoteTrabajo.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[id_actividad] ; ?>','winHH','scrollbars=yes,resizable=yes,width=900,height=400',<?=$reg02[id_actividad] ?>)" /></a>
			<? } ?>
			<? if (trim($reg02[nivel]) == "2") { ?>
			<a href="<?=$reg02[id_actividad] ?>" id="<?=$reg02[id_actividad] ?>" ></a>
			<a href="#<?=$reg02[id_actividad] ?>"><img src="img/images/icoNew.gif" alt="Ingresar División" width="16" height="15" border="0" onclick="MM_openBrWindow('addHtDivision.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[dependeDe] ; ?>&cualLT=<? echo $reg02[id_actividad] ; ?>','winHH','scrollbars=yes,resizable=yes,width=900,height=400',<?=$reg02[id_actividad] ?>)" /></a>
			<? } ?>
			<?
			//27May2014
			//PBM
			//Se quita este if para que el director, coordinador u ordenador de gasto No pueda crear actividades segun indicación de GRM
			 //if (trim($reg02[nivel]) == "3") { 
/*
			<a href="<?=$reg02[id_actividad] ?>" id="<?=$reg02[id_actividad] ?>" ></a>
			<a href="#<?=$reg02[id_actividad] ?>"><img src="img/images/icoNew.gif" alt="Ingresar Actividades" width="16" height="15" border="0" onclick="MM_openBrWindow('addHtActividades.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[actPrincipal] ; ?>&cualLT=<? echo $reg02[dependeDe] ; ?>&cualDiv=<? echo $reg02[id_actividad] ?>&cualIDdiv=<? echo $reg02[id_division] ?>','winHH','scrollbars=yes,resizable=yes,width=1150,height=400',<?=$reg02[id_actividad] ?>)" /></a>
*/			 
			 ?>
			<? //} ?>
			<?  } //if activaBTN) == "SI ?>
			<? } // if verBoton ?>			
			<?
//			echo "ID=" . $reg02[id_encargado] . "<br>";
//			echo "unidad=" . $laUnidad . "<br>";
			?>
			<? 
			//27May2014
			//PBM
			//Por instrucción de GRM se activará el botón de crear actividades para el responsable asignado a una división.
			//Segun GRM Sólo el responsable asignado a la División podrá crear actividades y valores, pero no podrá hacerlo el Director, Coordinador u Ordenador de gasto.
			//if ( ($reg02[id_encargado] == $laUnidad) AND (trim($reg02[nivel]) == "3") AND ($verBoton == "NO") ) { 

			//Para que el Director, coordinador u ordenador de gasto pueda ver el nivel tres si es responsable
			if ( ( ($reg02[id_encargado] == $laUnidad) AND (trim($reg02[nivel]) == "3") AND ($verBoton == "NO") ) OR ( ($verBoton == "SI") AND ($reg02[id_encargado] == $laUnidad) ) ) { 
				//Para habilitar el responsable de la División para permitir que borre y elimine actividades
				$idEncargadoNiv03 = $reg02[id_encargado];
			...
			if (($reg02[nivel] == 3)) {
				...
				echo " <tr id='fila_".$reg02[id_actividad]."'   class='TxtTabla' >";
				echo ' 	<td align="left" height="30%" ><b>Asignado</b></td>';
				echo " 	<td align='right'>";
				if ($reg04=mssql_fetch_array($cursor04)) {
				...
				echo " <tr id='fila1_".$reg02[id_actividad]."'  class='TxtTabla' >";
				echo " 	<td align='left' height='30%' ><b>Disponible</b></td>";
				echo " 	<td align='right'>";
...
//echo "<br><br> $val_div-$vla_act_div"; settype();
			...
			<? if (trim($activaBTN) == "SI") { ?>
			<? if (trim($reg02[nivel]) == "1") { ?>
<a href="#<?=$reg02[id_actividad] ?>" >
            <img src="img/images/imgFiles.gif" title="Editar Estructura" width="14" height="14" onclick="MM_openBrWindow('upEDTestLC.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[id_actividad] ; ?>','winHH','scrollbars=yes,resizable=yes,width=650,height=350',<?=$reg02[id_actividad] ?>)" border="0" /></a>
            <? } ?>
			<? if (trim($reg02[nivel]) == "2") { ?>
<a href="#<?=$reg02[id_actividad] ?>">
            <img src="img/images/imgFiles.gif" alt="Editar Estructura" width="14" height="14" onclick="MM_openBrWindow('upEDTestLT.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[actPrincipal] ; ?>&cualLT=<? echo $reg02[id_actividad] ; ?>','winHH','scrollbars=yes,resizable=yes,width=650,height=350',<?=$reg02[id_actividad] ?>)" border="0"  /></a>
            <? } ?>
			<? if (trim($reg02[nivel]) == "3") { ?>
<a href="#<?=$reg02[id_actividad] ?>">
            <img src="img/images/imgFiles.gif" alt="Editar Estructura" width="14" height="14" onclick="MM_openBrWindow('upEDTestDiv.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[actPrincipal] ; ?>&cualLT=<? echo $reg02[dependeDe] ; ?>&cualDIvision=<? echo $reg02[id_actividad] ?>','winHH','scrollbars=yes,resizable=yes,width=650,height=350',<?=$reg02[id_actividad] ?>)" border="0"  /></a>
            <? } ?>
            <? if (trim($reg02[nivel]) == "4") { ?>
<a href="#<?=$reg02[id_actividad] ?>">
			<img src="img/images/imgFiles.gif" alt="Editar Estructura" width="14" height="14" onclick="MM_openBrWindow('upEDTest.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[actPrincipal] ; ?>&cualDiv=<? echo $reg02[dependeDe] ; ?>&cualACtividad=<? echo $reg02[id_actividad] ?>','winHH','scrollbars=yes,resizable=yes,width=650,height=350',<?=$reg02[id_actividad] ?>)" border="0"  /> </a>
			<? } ?>   
			<?  } //if activaBTN) == "SI ?>
			<? } //if verBoton ?>         
            <td width="1%">
			...
			<? if (trim($reg02[nivel]) == "1") { ?>
<a href="#<?=$reg02[id_actividad] ?>">
            <img src="img/images/actualizar.gif" width="19" height="17" onclick="MM_openBrWindow('upEDTregLC.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[id_actividad] ; ?>','winHH','scrollbars=yes,resizable=yes,width=650,height=350',<?=$reg02[id_actividad] ?>)" border="0"  /> </a>
            <? } ?>
			<? if (trim($reg02[nivel]) == "2") { ?>
<a href="#<?=$reg02[id_actividad] ?>">
            <img src="img/images/actualizar.gif" width="19" height="17" onclick="MM_openBrWindow('upEDTregLT.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[actPrincipal] ; ?>&cualLT=<? echo $reg02[id_actividad] ; ?>','winDedt','scrollbars=yes,resizable=yes,width=650,height=350',<?=$reg02[id_actividad] ?>)" border="0"  /></a>
            <? } ?>
			<? if (trim($reg02[nivel]) == "3") { 			?>
<a href="#<?=$reg02[id_actividad] ?>">
            <img src="img/images/actualizar.gif" width="19" height="17" onclick="MM_openBrWindow('upEDTregDiv.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[actPrincipal] ; ?>&cualLT=<? echo $reg02[dependeDe] ; ?>&cualDIvision=<? echo $reg02[id_actividad] ?>','winHH','scrollbars=yes,resizable=yes,width=650,height=350',<?=$reg02[id_actividad] ?>)" border="0"  /></a>
            <? } ?>
			<? 
//			if (trim($reg02[nivel]) == "4") { ?>
<?
/*
			<a href="<?=$reg02[id_actividad] ?>" id="<?=$reg02[id_actividad] ?>" ></a>
<a href="#<?=$reg02[id_actividad] ?>">
			<img src="img/images/actualizar.gif" width="19" height="17" onclick="MM_openBrWindow('','','')" border="0"  /></a>
*/
?>			
			<? //} ?>
			<?  } //if activaBTN) == "SI ?>
			...
			//27May2014
			//PBM
			//Por instrucción de GRM se activará el botón de editat actividades para el responsable asignado a una división.
			//Segun GRM Sólo el responsable asignado a la División podrá editar actividades y valores, pero no podrá hacerlo el Director, Coordinador u Ordenador de gasto.

			//if (trim($reg02[nivel]) == "4") { 
			if ( ($idEncargadoNiv03 == $laUnidad) AND (trim($reg02[nivel]) == "4") AND ($verBoton == "NO") ) { 
			?>

			<a href="<?=$reg02[id_actividad] ?>" id="<?=$reg02[id_actividad] ?>" ></a>
<a href="#<?=$reg02[id_actividad] ?>">
			<img src="img/images/actualizar.gif" width="19" height="17" onclick="MM_openBrWindow('upEDTreg.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[actPrincipal] ; ?>&cualDiv=<? echo $reg02[dependeDe] ; ?>&cualACtividad=<? echo $reg02[id_actividad] ?>','winHH','scrollbars=yes,resizable=yes,width=650,height=350',<?=$reg02[id_actividad] ?>)" border="0"  /></a>
			<? } ?>	
			
            <td width="1%">
			<?
			//28May2014
			//Verifica si se puede eliminar o no la actividad segun la existencia de planeación y/o facturación
			$verIconoEliminar =  "SI" ;
			$sql07="SELECT cuantosHayPlan, cuantosHayFact ";
			$sql07=$sql07." FROM  ";
			$sql07=$sql07." ( ";
			$sql07=$sql07." SELECT COUNT(*) cuantosHayPlan ";
			$sql07=$sql07." FROM PlaneacionProyectos ";
			$sql07=$sql07." where id_proyecto = " . $cualProyecto;
			$sql07=$sql07." and id_actividad = " . $reg02[id_actividad] ;
			$sql07=$sql07." ) A, ";
			$sql07=$sql07." ( ";
			$sql07=$sql07." SELECT COUNT(*) cuantosHayFact ";
			...
			if ($reg07=mssql_fetch_array($cursor07)) {
				if ( ($reg07[cuantos] > 0) OR ($reg07[cuantos]) > 0) {
					$verIconoEliminar =  "NO" ;
				}
			}
			...
			<? if (trim($reg02[nivel]) == "1") { ?>
<a href="#<?=$reg02[id_actividad] ?>">
            <img src="img/images/Del.gif" width="14" height="13" onclick="MM_openBrWindow('delEDTregLC.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[id_actividad] ; ?>','winHH','scrollbars=yes,resizable=yes,width=800,height=400',<?=$reg02[id_actividad] ?>)"  border="0"  /> </a>
            <? } ?>
			<? if (trim($reg02[nivel]) == "2") { ?>
<a href="#<?=$reg02[id_actividad] ?>">
            <img src="img/images/Del.gif" width="14" height="13" onclick="MM_openBrWindow('delEDTregLT.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[actPrincipal] ; ?>&cualLT=<? echo $reg02[id_actividad] ; ?>','winHH','scrollbars=yes,resizable=yes,width=800,height=400',<?=$reg02[id_actividad] ?>)"   border="0" /> </a>
            <? } ?>
			<? if (trim($reg02[nivel]) == "3") { ?>
<a href="#<?=$reg02[id_actividad] ?>">
            <img src="img/images/Del.gif" width="14" height="13" onclick="MM_openBrWindow('delEDTregDiv.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[actPrincipal] ; ?>&cualLT=<? echo $reg02[dependeDe] ; ?>&cualDIvision=<? echo $reg02[id_actividad] ?>','winHH','scrollbars=yes,resizable=yes,width=800,height=400',<?=$reg02[id_actividad] ?>)"  border="0" /> </a>
            <? } ?>
            <? //if (trim($reg02[nivel]) == "4") { ?>
<?
...
/*
<a href="#<?=$reg02[id_actividad] ?>">
			<img src="img/images/Del.gif" width="14" height="13" onclick="MM_openBrWindow('delEDTreg.php?cualProyecto=<? echo $cualProyecto; ?>&cualLC=<? echo $reg02[actPrincipal] ; ?>&cualDiv=<? echo $reg02[dependeDe] ; ?>&cualACtividad=<? echo $reg02[id_actividad] ?>','winHH','scrollbars=yes,resizable=yes,width=800,height=400',<?=$reg02[id_actividad] ?>)"   border="0" /> </a>
*/			
?>
			
			<? //} ?>
			<? } // if de VerEliminar ?>
			<? } // if de verIconoEliminar ?>
			<?  } //if activaBTN) == "SI ?>
			<? } // if verBoton ?>
...
<a href="#<?=$reg02[id_actividad] ?>">
			...
			<? } 
			} //Cierra if verEliminar
			} //Cierra if verIconoEliminar
			?>
			</td>
          </tr>
		  <? } //Cierra While ?>
...
<?
	//permite ejecutar el ancla, cuando la pagina se ha recargado, al momento de ser redireccionada, desde otra 

	if(trim($aa)!="")
	{

		echo '<script type="text/javascript" language="javascript">';
		echo ' document.location.href = "http://www.ingetec.com.co/NuevaHojaTiempo/htPlanProyectos01.php?cualProyecto='.$cualProyecto.'#'.$aa.'";
			</script>
			';
	}
