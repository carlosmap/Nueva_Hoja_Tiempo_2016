...
//SI RECARGA EXISTE ES POR QUE SE HA RECARGADO LA PAGINA, Y ENTONCES SE CARGAN LAS VARIABLES ENVIADAS COMO PARAMETROS, CON LO QUE SE HAYA SELECCIONADO EN LOS SELECTS DE CONSULTA, ESTO CON EL FIN DE ACTUALIZAR EL VALOR ACTIVIDAD, SELECCIONADA
if((isset($recarga)))
{
	$cualLC=$LC;
	$cualLT=$LT;
...
//echo "valor asignaco a la division ".$valor_division;

//consulta el valor total de las actividades asociadas a la division
//if (trim($recarga)=="")
{
	...
	$valor_disponible=$valor_division-$valor_actividades_division;
}
//echo " $sql_valor_actividades_division <br><br> valor de las actividades de la division ".$valor_actividades_division."<br>";
...
if(trim($recarga) == "2"){
		...
		//CONSULTA LA EL LC,LT Y DIV EN LA ERP
		$sql_div="select erpLC,erpLT,erpDiv from Actividades where id_proyecto=".$cualProyecto."  and nivel=3 and  id_actividad=".$DI;
		$datos_div=mssql_fetch_array(mssql_query($sql_div));

		$erp_LC=$datos_div["erpLC"];
		$erp_LT=$datos_div["erpLT"];
		$erp_Div=$datos_div["erpDiv"];


		//CONSULTA LA CANTIDAD DE ACTIVIDADES, QUE ESTAN ASOCIADAS A LA DIVISION, CON LOS MISMOS DATOS DE LA ERP
		$sql_max_act="select MAX(erpAct) act_total from Actividades where id_proyecto=".$cualProyecto." and nivel=4 and erpDiv='".$erp_Div."'  and erpLC='".$erp_LC."' and  erpLT='".$erp_LT."' ";
		$datos_max_act=mssql_fetch_array(mssql_query($sql_max_act));


		$can_act=( (int) $datos_max_act["act_total"] ) + $pCantReg;
		$num_actividad=( (int) $datos_max_act["act_total"] ) +1;
		//SOLO SE PERMITIRA REGISTRAR CUANDO LA SUMATORIA DE LAS ACTIVIDADES + LA CANTIDAD DE REGISTROS A GRABAR, SEAN MENOR O IGUAL A 99
		if($can_act<=99)
		{
				...
				while ($s <= $pCantReg) {
					...
					if( ((int) $num_actividad )<10)
						$num_actividad ='0'.$num_actividad ;
			...
			//echo ${$elpJefe}."  ---<br>";
					...
					$sqlIn1=$sqlIn1."dependeDe, actPrincipal, tipoActividad, nivelesActiv, nivel,id_division, erpLC, erpLT, erpDiv, erpAct ,usuarioCrea,fechaCrea  ";
					...
					$sqlIn1 = $sqlIn1 . " '4' ";
					$sqlIn1 = $sqlIn1 . " ,".$divi_selec.", '".$erp_LC."' ,'".$erp_LT."' ,'".$erp_Div."' ,'".$num_actividad."',".$_SESSION["sesUnidadUsuario"].",getdate()  ";
					if(trim(${$valor})!="")
					{
							$sqlIn1=$sqlIn1."," . ${$valor};			
					}
...
//echo $sqlIn1."<br>";
		...
		//					echo mssql_get_last_message()." **** ".$sql_partici."<br>***".mssql_num_rows($cursorIn_part);
			...
			//echo $sql_acti_val."<br>".mssql_get_last_message()."<br>"; 	
			...
			//echo $sqlIn1."<br>".mssql_get_last_message()."<br>"; 				
				...
				echo ("<script>window.close();MM_openBrWindow('htPlanProyectos01.php?cualProyecto=".$cualProyecto."&aa=".$cualDiv."','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");

		}
		else
		{
					echo ("<script>alert('No se puede registrar mas de 99 actividades en esta división.');</script>"); 					
		}
...
//echo $erp_Div." --- ".$erp_LT." --- ".$erp_LC."<br>".$sql_div;

		$sql_max_act="select MAX(erpAct) act_total from Actividades where id_proyecto=".$cualProyecto." and nivel=4 and erpDiv='".$erp_Div."'  and erpLC='".$erp_LC."' and  erpLT='".$erp_LT."' ";
		$datos_max_act=mssql_fetch_array(mssql_query($sql_max_act));

		if($datos_max_act["act_total"]==99)
		{
				echo ("<script>alert('No se puede registrar mas de 99 actividades en esta división.');</script>"); 
...
//VALIDA SI EL VALOR DE LAS ACTIVIDADES ES IGUAL O INFERIOR A EL VALOR DE LA DIVISION, SI ES MAYOR NO PERMITE LA GRABACION
/*
	$s = 1;
	...
	{
		$valor = "valor" . $s;
		$valor_total_act+=${$valor};
		$s++;
	}
*/
// echo  " valor total actividades ingresadas".$valor_total_act. " ----- valor actividades division " .$valor_actividades_division."<br> valor asignado division: ";  echo $valor_division; 
?>
<script language="JavaScript" type="text/JavaScript">
<!--
...
CantCampos=5+(4*document.Form1.pCantReg.value);
var total_act_div_ingresadas=0;
var total_acti_division=<? echo $valor_actividades_division; ?>;

//Valida que el campo valor no esté vacio
for (i=8;i<=CantCampos;i+=4) {
...
//+document.Form1.pLCi.value
	...
	//VALIDA EL RESPONSABLE DE ACTIVIDAD
	if (document.Form1.elements[i-1].value == '') {
		v2='n';
		msg2 = 'El responsable de la actividad es obligatorio.'; //'Asigne un valor a cada una de las divisiones. \n';
//+document.Form1.pLCi.value
	}
	else //ALMACENA EL VALOR TOTAL DE LAS ACTIVIDADES INGRESADAS
	{
		total_act_div_ingresadas+=parseInt(document.Form1.elements[i].value);
	}
}
...
for (i=6;i<=CantCampos;i+=4) {
	if (document.Form1.elements[i].value == '') {
		v2='n';
		msg2 = 'El nombre de cada uno de las actividades es obligatorio. \n';
	}
}
...
<? //if(trim($recarga) == "2") ?>
		//SUMA EL VALOR DE LAS ACTIVIDADES A ADICIONAR Y EL VALOR DE LAS ACTIVIDADES ALMACENADAS
		//COMPARA SI LA SUMA DE LAS ACTIVIDADES, NO SUPERA EL VALOR ASIGNADO A LA DIVISION
		var total=parseInt(total_acti_division)+parseInt(total_act_div_ingresadas);
		if(total><? echo $valor_division; ?>)
		{
			msg2='El valor de las actividades supera el valor asignado a la división, por favor verifique el valor asignado en cada actividad.';
// total ingresadas '+total_act_div_ingresadas+ ' total suma (ingre+regis): '+total;
			v2='n';
		}
<? ?>
...
//echo $sql_max_act." <br><br>".mssql_get_last_message();
...
?>
		 <tr>
          <td width="20%" class="TituloTabla">Valor disponible en la divisi&oacute;n </td>
          <td class="TxtTabla"><input name="val_dis" type="text" class="CajaTexto" id="val_dis" value="<? echo $valor_disponible; ?>" size="10" readonly onKeyPress="return acceptNum(event)"></td>
        </tr>
      ...
      <tr class="TxtTabla">
		...
		$sql2="select U.*
				...
				--inner join Divisiones as div on dep.id_division=div.id_division
				 where  -- div.id_division='".$divi_selec."'  and
				 retirado is null
