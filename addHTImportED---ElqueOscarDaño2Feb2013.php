			...
			if($band==0){
				...
				if( $primero == 1 ){
					...
					for( $v = $start_v; $v <= $end_v; $v++ ){
						...
						$inc++;
						#	Mirar asignacion
						#$valor = ltrim( rtrim( get_cell("B".$v, $objPHPExcel) ) );
				...
				if( $mal == 0 and $depVer == 0 ){
						...
						for( $h = $start_h; ord($h) <= ord( $end_h ); pp($h) ){
							...
							if( $cellValue !== null ){
								...
								if( $h == "B" or $h == "C"){
									...
									if( $nivel == 3 and $h == "C" ){
										$sqlDiv = "SELECT id_division FROM Divisiones WHERE nombre LIKE '".$cellValue."'";
					...
					if( $error == "no"){
						...
						echo "<script type='text/javascript'> alert('No se pudo guardar la EDT debido a que ya hay una creada en el sistema para este proyecto.');</script>";
					}
				}
				else{
					echo "<script type='text/javascript'> 
							alert('Hay un error en el archivo que se quiere subir. Verifiqué cada uno de los campos y corríjala.');
						  </script>";
    ...
    <div align='center'>
        <table width='80%' border='0' cellpadding='0' cellspacing='1'  bgcolor='#FFFFFF'>
            <tr class='TituloTabla' >
              <th width="8%" colspan="5" align="left">Citerios para importar los datos </th>
            </tr>
            <tr >
            <th width="8%" align="left" class='TituloTabla'>CodActividad</th>
            <th width="8%" colspan="4" align="left" class='TxtTabla'>Debe ser un consecutivo.</th>
            </tr>
            <tr >
            <th width="8%" align="left" class='TituloTabla'>Identificador</th>
            <th width="8%" colspan="4" align="left" class='TxtTabla'>No debe tener espacios en blanco<br />
              Para 
              los separadores debe utilizar un punto(.)</th>
            </tr>
            <tr >
            <th width="8%" align="left" class='TituloTabla'>Nombre</th>
            <th width="8%" colspan="4" align="left" class='TxtTabla'>No puede ir vacío</th>
            </tr>
            <tr >
            <th width="8%" align="left" class='TituloTabla'>DependeDe</th>
            <th width="8%" colspan="4" align="left" class='TxtTabla'>
            Si es un lote de control debe ir con 0.<br />
            Si es un lote de trabajo debe traer el CodActividad del lote de control al que corresponde.<br />
            Si es una división debe traer el CodActividad del lote de trabajo al que pertenece.<br />
            Si es una actividad debe traer el CodActividad de la divisi&oacute;n a la que pertenece.
            </th>
            </tr>            
            <tr class='TituloTabla' >
                <td width="8%">CodActividad</td><td width="8%">Identificador</td><td width="50%">Nombre</td><td width="5%">DependeDe</td><td width="20%">Error</td>
            </tr>
				<?php	for( $v = $start_v; $v <= $end_v; $v++ ){	?>                
            <tr class='TxtTabla'>
                <?php
					for( $h = $start_h; ord($h) <= ord( $end_h ); pp($h) ){
					$cellValue = get_cell($h.$v, $objPHPExcel);
                ...
                ?>
                <td width="8%" valign="top"><?= $mensaje ?></td>
			</tr>
				<?	}	?>
		</table>
