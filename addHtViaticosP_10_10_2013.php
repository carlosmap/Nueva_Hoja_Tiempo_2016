...
<tr>
  ...
  <?php
	...
	while($reg = mssql_fetch_array($ap_s)){
		echo "<option value= $reg[IDsitio]>".str_replace('ó','&oacute;',str_replace('ñ','&ntilde;',$reg[NomSitio]))." ";
	}

	if($sitioTrabajo != ""){
		//Decide cual opción dejar seleccionada
		$sql_sitios="select * from sitiostrabajo where idsitio=$sitioTrabajo";
		$ap_s = mssql_query($sql_sitios);
		$reg = mssql_fetch_array($ap_s);
...
//{
	...
	while($reg = mssql_fetch_array($ap)){
		echo "<option value=$reg[IDTipoViatico]>".str_replace('ó','&oacute;',str_replace('ñ','&ntilde;',$reg[NomTipoViatico]))."";
	}
...
<td>
<input name="fechaInicialViatico" id="fechaInicialViatico" type="text" size= 30 class="CajaTexto" onKeyPress="return acceptNum(event)"  onBlur="cambiarDisplay('row2'); valFecha(this); "   onChange="cambiarDisplay('row2')"  value="<? echo $fechaInicialViatico; ?>"  >
...
<td>
<input name="fechaFinalViatico" id="fechaFinalViatico" type="text" size=30 class="CajaTexto"  onKeyPress="return acceptNum(event)"  onBlur="cambiarDisplay('row2'); valFecha(this);"  onChange="cambiarDisplay('row2')" value="<? echo $fechaFinalViatico; ?>"  >
...
<tr >
  <td colspan="2" >
<!-- NOTA: El &uacute;ltimo d&iacute;a de vi&aacute;ticos se ingresa de manera independiente -->
</td>
  </tr>
