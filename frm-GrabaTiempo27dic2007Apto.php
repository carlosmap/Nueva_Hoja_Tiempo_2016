...
if ($esProyEsp == "NO") {
	...
	$sql="SELECT     Asignaciones.id_actividad, Actividades.nombre, Asignaciones.clase_tiempo, Asignaciones.localizacion, Asignaciones.cargo
	...
	/*		do {
			...
			while ($filas=mssql_fetch_array($resultado)){
				//echo "<OPTION VALUE='".$filas[id_actividad]."-".$filas[clase_tiempo]."'>".ucwords(strtolower($filas[nombre])).'-'.$filas[clase_tiempo].'</OPTION>';
				echo "<OPTION VALUE='".$filas[id_actividad]."-".$filas[clase_tiempo]."-".$filas[localizacion]."-".$filas[cargo]."'>".ucwords(strtolower($filas[nombre])).'-'.$filas[clase_tiempo]."-".$filas[localizacion]."-".$filas[cargo].'</OPTION>';
			}   
...
//	WHERE (id_proyecto = '$nomproyecto') AND (id_actividad = '$actividad') AND (horas.unidad = '$laUnidad') AND (horas.clase_tiempo='$clasetiempo')";
	...
	AND (horas.unidad = '$laUnidad') AND (horas.clase_tiempo='$clasetiempo') AND (horas.localizacion = $localizacion)
	AND (horas.cargo = $cargo)
