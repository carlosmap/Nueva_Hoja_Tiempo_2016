...
if($recarga==1)
{
		...
		$sql3 = " select * from TrabajadoresExternos T
				  inner join ParticipantesExternos P on T.consecutivo=P.consecutivo and P.id_proyecto=".$cualProyecto." and P.id_actividad=".$id_ac;
...
//		$mail = "mail" . $i;
		...
		*/
		if(($$aplicaUsuario == 1)and($okGuardar == "Si"))
		{
			...
			$sql_planea="select * from  PlaneacionProyectos  where id_proyecto=".$cualProyecto." and id_actividad=".$id_ac." and unidad=".$$usuario. " and  esInterno='E' ";
			...
			if(mssql_num_rows($cur_planea)==0)
			{
				...
				$sqlDel = "DELETE FROM  HojaDeTiempo.dbo.ParticipantesExternos  
						   Where id_actividad = ".$id_ac." and consecutivo = ".$$usuario." AND id_proyecto = ".$cualProyecto;
...
//echo "<br>".$sqlDel;
	...
	//				$sqlIn1 = " DELETE FROM HojaDeTiempo.dbo.ParticipantesActividad  WHERE id_proyecto=" . $cualProyecto ." and unidad=".$$usuario." and id_actividad=".$id_ac;
					$sqlIn1 = " UPDATE HojaDeTiempo.dbo.ParticipantesExternos SET estado='I'  WHERE id_proyecto=" . $cualProyecto ." and consecutivo=".$$usuario." and id_actividad=".$id_ac;			
...
?>
  ...
  <tr>
    <td><table width="100%"  cellspacing="1">
	  ...
	  ?>
	  <tr class="TxtTabla">
        <td><? echo ucwords(strtolower($reg3['apellidos'] . " " . $reg3['nombre'])); ?>
          <input name="usuario<? echo $i; ?>" type="hidden" id="usuario<? echo $i; ?>" value="<? echo $reg3['consecutivo']; ?>">
