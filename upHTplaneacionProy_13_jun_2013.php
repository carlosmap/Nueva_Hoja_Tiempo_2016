...
if(trim($recarga) == "2"){
	...
	$pTema = '<table width="100%" border="0">
	  ...
	  <tr class="Estilo2">
		...
		<td  >Planeacion de actividades</td>
	  </tr>
	  <tr class="Estilo2"  >
		<td width="20%">Fecha de planeación</td>
		<td >'.gmdate ("n/d/Y").'</td>
	  </tr>
	  <tr class="Estilo2">
		<td width="20%">Usuario que planea:</td>
	  ...
	  <tr>
		...
		$cur_proy=mssql_query('select nombre,macroactividad,id_encargado  from Actividades where id_proyecto='.$cualProyecto.' and id_actividad='.$cualAct);
		if($dato_proy=mssql_fetch_array($cur_proy))
		{
			$usu_correo[0]=$datos_dir_cor["id_encargado"];
			$nomLoteAct1=$dato_proy["nombre"];
			$macroLoteAct1=$dato_proy["macroactividad"];
		}
...
//echo $sqlIn1." *** <br>".mssql_get_last_message()."<br><br>";
				...
				if($Z==$d)
				{	
						$pTema = $pTema.'<td width="1%">&nbsp;'. ${$eltxtPlan}.'&nbsp;</td>';	
...
//		$pTema = $pTema.'</tr></table></td>';
		...
		$pTema = $pTema.'<td width="1%">&nbsp;'.${$salario_u}.'&nbsp;</td>';
		$pTema = $pTema.'<td width="1%">&nbsp;'.${$valor_fac_u}.'&nbsp;</td></tr>';
		$s = $s + 1;
	} //Cierra While
