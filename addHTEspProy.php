...
if ($recarga == "2") {
	...
	if  (trim($cursor) != "") {
		...
		$quienEspecifica="";
		$qry="SELECT * FROM Usuarios WHERE unidad = " . $laUnidad;
		$cursorQry = mssql_query($qry);
		if ($regQry=mssql_fetch_array($cursorQry)) {
			$quienEspecifica= $regQry[apellidos] . " " . $regQry[nombre];
		}
		...
		$pTema = $pTema . "		<td>".$quienEspecifica."</td>";
		$pTema = $pTema . "	</tr>";
		$pTema = $pTema . "	<tr class='Estilo2'>";
		$pTema = $pTema . "		<td>&nbsp;</td>";
		$pTema = $pTema . "		<td>&nbsp;</td>";
		...
		if ($Valor != $vlSC) {
			...
			$pTema = $pTema . "		<td>El valor definido en la solicitud de código ($".number_format($vlSC, 0, ",", ".").") y el valor del proyecto aquí definido no son equivalentes. Por favor verifique.</td>";
			$pTema = $pTema . "	</tr>";
			$pTema = $pTema . "	<tr class='Estilo2'>";
			$pTema = $pTema . "		<td>&nbsp;</td>";
			$pTema = $pTema . "		<td>&nbsp;</td>";
...
//		}
	...
	if(document.Form1.Valor.value<parseFloat(<? echo $mitotDivReal; ?>))
	{
		v5='n';
		msg5='El valor del proyecto, no puede ser inferior, a la sumatoria del valor real de las divisiones.';
	}
