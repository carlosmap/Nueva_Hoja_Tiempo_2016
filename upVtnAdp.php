...
if($recarga==2)
{
	...
	if($opc==2)
	{
		...
		$sqlFchIni=$sqlFchIni.' AND vigencia = '.$vigencia;
		$sqlFchIni=$sqlFchIni.' AND mes = '.$mes;
		...
		#/*#	Verifica para cuando esta dentro de un rango de fechas 	vigencia, mes
		if($error==1)
		{
			$sqlFchEnt='SELECT * FROM AdpHT ';
			$sqlFchEnt=$sqlFchEnt.'WHERE id_proyecto = '.$cualProyecto;
			$sqlFchEnt=$sqlFchEnt.' AND unidad = '.$laUnidad;
			$sqlFchEnt=$sqlFchEnt.' AND vigencia = '.$vigencia;
			$sqlFchEnt=$sqlFchEnt.' AND mes = '.$mes;
			$sqlFchEnt=$sqlFchEnt." AND idAdp != ".$idAdp;
			#	*****
			/*
			$sqlFchEnt=$sqlFchEnt." AND DAY(fechaInicio) >= ".date( 'd', strtotime($fInicia));
			$sqlFchEnt=$sqlFchEnt." AND MONTH(fechaInicio) = ".date( 'm', strtotime($fInicia));
			$sqlFchEnt=$sqlFchEnt." AND YEAR(fechaInicio) = ".date( 'Y', strtotime($fInicia));
			*/
			$sqlFchEnt2=$sqlFchEnt." AND ( DAY(fechafinal) >= ".date( 'd', strtotime($fInicia));
			$sqlFchEnt2=$sqlFchEnt2." AND MONTH(fechafinal) = ".date( 'm', strtotime($fInicia));
			$sqlFchEnt2=$sqlFchEnt2." AND YEAR(fechafinal) = ".date( 'Y', strtotime($fInicia));
			
			$sqlFchEnt2=$sqlFchEnt2." AND DAY(fechafinal) <= ".date( 'd', strtotime($fFinal));
			$sqlFchEnt2=$sqlFchEnt2." AND MONTH(fechafinal) = ".date( 'm', strtotime($fFinal));
			$sqlFchEnt2=$sqlFchEnt2." AND YEAR(fechafinal) = ".date( 'Y', strtotime($fFinal)).")";
			#	*****
			#echo $sqlFchEnt2.'<br />';
			$qryFchEnt=mssql_query($sqlFchEnt2);
			$error=1;
			$msg = 'La fecha ingresada esta dentro de los rangos.\n';
			$cant = mssql_num_rows($qryFchEnt);
			...
			#*/
			$sqlFchEn2=$sqlFchEnt." ORDER BY fechaInicio ";
			$qryFchEnt2=mssql_query($sqlFchEn2);
			#echo $sqlFchEn2.'<br />';
			$error=1;
			$msg = 'La fecha ingresada esta dentro de los rangos.\n';
			while($rwFchEnt2=mssql_fetch_array($qryFchEnt2))
			{
				$fIni = date( 'd', strtotime( $rwFchEnt2['fechaInicio'] ) );
				$fcF = date( 'd', strtotime( $fFinal ) );
				#if($fcF<$fIni)
				if($fcF<$fIni&&$cant==0)
				{
					$error=0;
		...
		#*/
		#	*****
		#	Verifica que sea menor que todas
		if($error==1)
		{
			$sqlFchIni='SELECT MIN(fechaInicio) fch  FROM AdpHT ';
			$sqlFchIni=$sqlFchIni.'WHERE id_proyecto = '.$cualProyecto;
			$sqlFchIni=$sqlFchIni.' AND unidad = '.$laUnidad;
			$sqlFchIni=$sqlFchIni.' AND vigencia = '.$vigencia;
			$sqlFchIni=$sqlFchIni.' AND mes = '.$mes;
			...
			{
				$fIni = date( 'd', strtotime( $rwFchIni['fch'] ) );
				
				$fcF = date( 'd', strtotime( $fFinal ) );
				$fcI = date( 'd', strtotime( $fInicia ) );
				if($fcI<$fIni&&$fcF<$fIni)
				{
					$error=0;
				}
			}
		...
		if($error==0)
		{
			...
			{			
				...
				$sqlUpdAdp = $sqlUpdAdp . "fechaMod='".date('Y-m-d')."', ";
				$sqlUpdAdp = $sqlUpdAdp . "usuarioMod=".$_SESSION['sesUnidadUsuario'];
...
?>
        ...
        <tr>
          <td colspan="2"  class="TxtTabla"><!-- readonly-->
            <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#FFFFFF">
              ...
              <tr class="TituloTabla2">
                ...
                <?	if($opc==2)
					{			
				?>
                <img src="imagenes/cal.gif" alt="" width="16" height="16" border="0" />
                <?
							}
                        ?>
                </a></td>
