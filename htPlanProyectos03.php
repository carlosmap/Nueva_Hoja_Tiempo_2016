...
<?php
			$cur_val_fact=mssql_query("select sum(valorFacturado) as total_fac from FacturacionProyectos where id_proyecto =".$reg02[id_proyecto]." and id_actividad=".$reg02[id_actividad]);
			if($datos_val_fact=mssql_fetch_array($cur_val_fact))			
				$vla_fact_acti=$datos_val_fact["total_fac"];
?>	
            <td width="8%" align="center"><? if(trim($vla_fact_acti)!=""){ echo number_format($vla_fact_acti, "2", ",", "." ); } ?></td>
            <td width="1%" class="TituloTabla"> </td>       
