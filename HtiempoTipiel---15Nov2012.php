...
$sql="SELECT RTRIM(localizacion) + '-' + RTRIM(cargo) AS codigoproyecto, cargo,
		 ...
		 FROM Horas
		WHERE     (fecha BETWEEN $cadfecha) AND (unidad = '$laUnidad') and id_proyecto = 1752  and (cargo='480' or cargo='4810' ) order by dia";
...
$sqlViat=" SELECT  distinct   Horas.resumen_trabajo, ViaticosHoras.horallegada, ViaticosHoras.horasalida, Horas.unidad, Viaticos.sitio, Horas.fecha
            ...
            Horas.fecha = ViaticosHoras.fecha
			WHERE (Horas.fecha BETWEEN $cadfecha) AND (ViaticosHoras.unidad = '$laUnidad') AND (ViaticosHoras.id_proyecto = 1752) order by Horas.fecha";
