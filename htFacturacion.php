...
//Para verificar si se trata de proyecto especial
...
$esProyectoNovedad="NO";

//No se podrá eliminar toda la hoja de tiempo si al menos uno de los proyectos ya tiene VoBo
//Se suma 1 a la siguiente variable cada vez que haya un proyecto con VoBo.
$cuantosVoBoProy=0;

//24Jul2013
//PBM
//--Trae la planeación de una persona para un mes y año seleccionados
$sql01="SELECT A.id_proyecto, A.unidad, A.vigencia, A.mes, SUM(A.hombresMes) totHombresMes, SUM(A.horasMes) totHorasMes, B.nombre, B.codigo, B.cargo_defecto, B.idTipoCobro, C.nomTipoCobro ";
$sql01=$sql01." FROM PlaneacionProyectos A, Proyectos B, TipoCobroProy C " ;
$sql01=$sql01." WHERE A.id_proyecto = B.id_proyecto " ;
$sql01=$sql01." AND B.idTipoCobro *= C.idTipoCobro "  ;
$sql01=$sql01." AND A.unidad = " . $laUnidad ;
$sql01=$sql01." AND A.vigencia = " . $pAno ;
$sql01=$sql01." AND A.mes = " . $pMes ;
$sql01=$sql01." GROUP BY A.id_proyecto, A.unidad, A.vigencia, A.mes, B.nombre, B.codigo, B.cargo_defecto, B.idTipoCobro, C.nomTipoCobro " ;
...
$sql09="SELECT A.* , B.nombre, B.codigo, B.cargo_defecto, B.idTipoCobro, C.nomTipoCobro ";
$sql09=$sql09." FROM ProyectosSinPlaneacion A, Proyectos B, TipoCobroProy C " ;
$sql09=$sql09." WHERE A.id_proyecto = B.id_proyecto  " ;
$sql09=$sql09." AND B.idTipoCobro *= C.idTipoCobro "  ;
      ...
      <tr>
        <td bgcolor="#FFFFFF"><table width="100%"  border="0" cellspacing="1" cellpadding="0">
          ...
          <tr class="TxtTabla">
			...
			if ($tieneVBJefe != '1') { ?>
			<input name="Submit2" type="submit" class="Boton" onclick="MM_openBrWindow('addHTFacturacionProy.php?cualProyecto=<? echo $reg01['id_proyecto']; ?>&cualVigencia=<? echo $pAno; ?>&cualMes=<? echo $pMes; ?>&hayPlaneacion=1','wAFTP','scrollbars=yes,resizable=yes,width=1000,height=500')" value="Facturar" />
			<? } ?>
			</td>
          ...
          <table width="100%"  border="0" cellspacing="1" cellpadding="0">
            ...
            <tr class="TxtTabla">
              ...
              <td width="10%"><? echo $reg09['nomTipoCobro'] ?> </td>
              <td width="5%" align="center">
			  <? 
			//Si la Hoja de tiempo ya tiene VoBo no se puede facturar más.
			if ($tieneVBJefe != '1') { ?>
			  <input name="Submit2" type="submit" class="Boton" onclick="MM_openBrWindow('addHTFacturacionProy.php?cualProyecto=<? echo $reg09['id_proyecto']; ?>&cualVigencia=<? echo $pAno; ?>&cualMes=<? echo $pMes; ?>&hayPlaneacion=0','wAFTP','scrollbars=yes,resizable=yes,width=1000,height=500')" value="Facturar" />
			  <? } ?>
			  </td>
            </tr>
			<?
          ...
          <table width="100%"  border="0" cellspacing="0" cellpadding="0">
			...
			//Si la Hoja de tiempo ya tiene VoBo no se puede facturar más.
			if ($tieneVBJefe != '1') { ?>
			  <input name="Submit3" type="submit" class="Boton" onclick="MM_openBrWindow('addHTProySinPlan.php?cualVigencia=<? echo $pAno; ?>&cualMes=<? echo $pMes; ?>','WinPsp','scrollbars=yes,resizable=yes,width=500,height=600')" value="Proyectos sin planeaci&oacute;n" />
			  <? } ?>
			  </td>
            </tr>
          </table>
		  ...
		  ?>
            ...
            <tr >
			  ...
			  <? if ($tieneVBproy == '1') { 
					//No se podrá eliminar toda la hoja de tiempo si al menos uno de los proyectos ya tiene VoBo
					//Se suma 1 a la siguiente variable cada vez que haya un proyecto con VoBo.
			  		$cuantosVoBoProy=$cuantosVoBoProy+1;
			  ?>
...
//				}
			  ...
			  ?></td>
				  ...
				  if (($reg02['id_proyecto'] == 56) OR ($reg02['id_proyecto'] == 60) OR ($reg02['id_proyecto'] == 61) OR ($reg02['id_proyecto'] == 62) OR ($reg02['id_proyecto'] == 63) OR ($reg02['id_proyecto'] == 64) OR ($reg02['id_proyecto'] == 65) OR ($reg02['id_proyecto'] == 71) ) {
				  	$esProyectoNovedad="SI";
				  ?>
				  <img src="img/images/alertaAzul.gif" alt="ADP" width="15" height="16" style="cursor: hand" onclick="MM_openBrWindow('htVtnAdp.php?cualProyecto=<? echo $reg02['id_proyecto']; ?>&cualActiv=<? echo $reg02['id_actividad']; ?>&cualHorario=<? echo $reg02['IDhorario']; ?>&cualLocaliza=<? echo $reg02['localizacion']; ?>&cualClaseT=<? echo $reg02['clase_tiempo']; ?>&cualCargo=<? echo $reg02['cargo']; ?>&cualVigencia=<? echo $pAno; ?>&cualMes=<? echo $pMes; ?>','winvtnres1','scrollbars=yes,resizable=yes,width=400,height=600')" />
				  <?
				  }
				  ?>
				  </td>
			...
			?>			  
			<?
			//Los viáticos sólo aplican para los proyectos que No son especiales
				if (trim($esProyectoNovedad) == "NO" ) {
			?>
			  <img src="img/images/icoCuantia.gif" alt="Vi&aacute;ticos" width="16" height="16" style="cursor: hand" onclick="MM_openBrWindow('addHtViaticosP.php?cualProyecto=<? echo $reg02['id_proyecto']; ?>&cualActiv=<? echo $reg02['id_actividad']; ?>&cualHorario=<? echo $reg02['IDhorario']; ?>&cualLocaliza=<? echo $reg02['localizacion']; ?>&cualClaseT=<? echo $reg02['clase_tiempo']; ?>&cualCargo=<? echo $reg02['cargo']; ?>&cualVigencia=<? echo $pAno; ?>&cualMes=<? echo $pMes; ?>','winVtnViaticosP','scrollbars=yes,resizable=yes,width=500,height=580')" />
			<? } // if esProyectoNovedad ?>	
			<? } // if tieneVBJefe; ?>			  
			</td>
              <td width="1%" class="TxtTabla"><img src="img/images/ver.gif" alt="Resumen" width="16" height="16" style="cursor: hand" onclick="MM_openBrWindow('htVtnResumen.php?cualProyecto=<? echo $reg02['id_proyecto']; ?>&cualActiv=<? echo $reg02['id_actividad']; ?>&cualHorario=<? echo $reg02['IDhorario']; ?>&cualLocaliza=<? echo $reg02['localizacion']; ?>&cualClaseT=<? echo $reg02['clase_tiempo']; ?>&cualCargo=<? echo $reg02['cargo']; ?>&cualVigencia=<? echo $pAno; ?>&cualMes=<? echo $pMes; ?>','winvtnres1','scrollbars=yes,resizable=yes,width=400,height=600')" /></td>
              <td width="1%" class="TxtTabla">
			  <? 
			//Si la Hoja de tiempo ya tiene VoBo no se puede facturar más.
			if ($tieneVBJefe != '1') { ?>
			...
			if ($tieneVBproy != '1') { ?>
			  <img src="img/images/Del.gif" alt="Eliminar Facturaci&oacute;n" style="cursor: hand" width="14" height="13" onclick="MM_openBrWindow('delHTporReg.php?cualProyecto=<? echo $reg02['id_proyecto']; ?>&cualActiv=<? echo $reg02['id_actividad']; ?>&cualHorario=<? echo $reg02['IDhorario']; ?>&cualLocaliza=<? echo $reg02['localizacion']; ?>&cualClaseT=<? echo $reg02['clase_tiempo']; ?>&cualCargo=<? echo $reg02['cargo']; ?>&cualVigencia=<? echo $pAno; ?>&cualMes=<? echo $pMes; ?>','winDelReg','scrollbars=yes,resizable=yes,width=700,height=500')" />
			  <? } // VoBo Proyecto ?>			  
			  <? } // VoBo Jefe ?>			  
			  </td>
            </tr>
...
<tr >
			   <td colspan="6" class="TituloUsuario" > .:: RELACI&Oacute;N DE VIÁTICOS - D&Iacute;AS DE CAMPO </td>
			   <?
			  //25Jul2013
			  //PBM
			  //Genera los dís del mes 
...
//					echo $reg12['diaIniV'] . " - " . $reg12['diaFinV'] . "<br>";
			...
			<? 
			//No se pueden eliminar los viáticos si ya existe VoBo del viático
			if ($tieneVBviatico != '1') { ?>   
			   <img src="img/images/Del.gif" alt="Eliminar Vi&aacute;tcos" width="14" height="13" style="cursor: hand" onclick="MM_openBrWindow('delHTviaticosP.php?cualProyecto=<? echo $reg11['id_proyecto']; ?>&cualActiv=<? echo $reg11['id_actividad']; ?>&cualHorario=<? echo $reg11['IDhorario']; ?>&cualLocaliza=<? echo $reg11['localizacion']; ?>&cualClaseT=<? echo $reg11['clase_tiempo']; ?>&cualCargo=<? echo $reg11['cargo']; ?>&cualVigencia=<? echo $pAno; ?>&cualMes=<? echo $pMes; ?>&cualSitio=<? echo $reg11['IDsitio']; ?>&cualTipoViatico=<? echo $reg11['IDTipoViatico']; ?>','winDelReg','scrollbars=yes,resizable=yes,width=700,height=500')" />
			<? } //if tieneVBviatico ?>   
			<? } //if tieneVBJefe ?>			   
			   </td>
		    </tr>
	...
	?>			
          ...
          <table width="100%"  border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td align="right" class="TxtTabla">
			  <input name="Submit6" type="submit" class="Boton" onclick="MM_openBrWindow('pdfHtUsuario.php?laUnidad=<? echo $laUnidad; ?>&cualVigencia=<? echo $pAno; ?>&cualMes=<? echo $pMes; ?>','winPDFht','scrollbars=yes,resizable=yes,width=700,height=500')" value="Generar PDF" />
			  <?
			  //el siguiente botón es para generar el PDF de una hoja de tiempo de otra persona.
			  if ($MuestraElBoton == "NO") {
			  ?>
			  <input name="Submit62" type="submit" class="Boton" onclick="MM_openBrWindow('pdfHtUsuario-V2.php?cualUnidadUsu=15712&cualVigenciaUsu=<? echo $pAno; ?>&cualMesUsu=<? echo $pMes; ?>','winPDFht','scrollbars=yes,resizable=yes,width=700,height=500')" value="Generar PDF v2" />                
			  <? } // ?>
                <? 
			//Si la Hoja de tiempo ya tiene VoBo no se puede facturar más.
			if ($tieneVBJefe != '1') { ?>
                <input name="Submit7" type="submit" class="Boton" onclick="MM_openBrWindow('addHtEviaHTvoBoJefe.php?cualAno=<? echo $pAno ; ?>&cualMes=<? echo $pMes ; ?>','winEnviaJefe','scrollbars=yes,resizable=yes,width=400,height=200')" value="Enviar a VoBo Jefe inmediato - Revisión " />                
				<?
				//No se podrá eliminar toda la hoja de tiempo si al menos uno de los proyectos ya tiene VoBo
				//Se suma 1 a la siguiente variable cada vez que haya un proyecto con VoBo.
				if ($cuantosVoBoProy== 0) {
				?>  
			  	<input name="Submit5" type="submit" class="Boton" onclick="MM_openBrWindow('delHTtodaHT.php?pAno=<? echo $pAno; ?>&pMes=<? echo $pMes; ?>&unidad_u=<? echo $laUnidad; ?>&visible=0','winDelHTHT','scrollbars=yes,resizable=yes,width=1400,height=500')" value="Eliminar toda la Hoja de tiempo" />              
				<? } // if cuantosVoBoProy ?>	
			  <? } // if tieneVBJefe ?>			  
