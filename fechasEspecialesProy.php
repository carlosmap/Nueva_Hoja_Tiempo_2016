...
$sql4="select * from FechasEspecialesProy " ;
$sql4=$sql4." where IDhorario = " . $cualHorario ;
$sql4=$sql4." and id_proyecto = " . $cualProy ;
    ...
    <td bgcolor="#FFFFFF">
	    ...
	    <table width="100%"  border="0" cellspacing="1" cellpadding="0">
		  ...
		  <tr class="TxtTabla">
            ...
            <td width="1%">
			<?
			//Verifica si hay facturación en el proyecto para ese día especial
			//para identificar si se puede o no borrar la información del día especial
			$phayFacturacion = 0;
			$vFEsql="SELECT COUNT(*) hayFact FROM HojaDeTiempo.dbo.Horas ";
			$vFEsql=$vFEsql." WHERE id_proyecto = " . $cualProy ;
			$vFEsql=$vFEsql." and fecha = '". trim($miFecha)."' ";
			$cursorvFE = mssql_query($vFEsql);
			if ($RegvFE=mssql_fetch_array($cursorvFE)) {
				$phayFacturacion = $RegvFE[hayFact];
			}
			
			if ($phayFacturacion == 0) {
			?>
			<a href="#"><img src="img/images/Del.gif" alt="Eliminar" width="14" height="13" border="0" onClick="MM_openBrWindow('delHorarioFechasEspProy.php?cualHorario=<? echo $pIDhorario; ?>&cualFecha=<? echo $miFecha; ?>&cualProy=<? echo $cualProy; ?>','vdFE','scrollbars=yes,resizable=yes,width=500,height=350')"></a>
			<?
			}
			?>
			</td>
	    ...
	    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td align="right" class="TxtTabla"><input name="Submit" type="button" class="Boton" onClick="MM_openBrWindow('addHorarioFechasEspProy.php?cualHorario=<? echo $pIDhorario; ?>&cualProy=<? echo $cualProy; ?>','adFE','scrollbars=yes,resizable=yes,width=500,height=300')" value="Ingresar"></td>
          </tr>
        </table>
		</td>
  </tr>
