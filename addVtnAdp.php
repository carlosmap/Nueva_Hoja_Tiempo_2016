...
include "validaUsrBd.php";
		...
		if($recarga==2)
		{
			...
			#/*#	Valida que la fecha que se ingrese siempre sea mayor al ultimo registro
			...
			#*/
			#/*#	Verifica para cuando esta dentro de un rango de fechas 
			if($error==1)
			{
				$sqlFchEnt='SELECT * FROM AdpHT ';
				$sqlFchEnt=$sqlFchEnt.'WHERE id_proyecto = '.$cualProyecto;
				$sqlFchEnt=$sqlFchEnt.' AND unidad = '.$laUnidad;
				$sqlFchEnt=$sqlFchEnt.' AND vigencia = '.$cualVigencia;
				$sqlFchEnt=$sqlFchEnt.' AND mes = '.$cualMes;
				$qryFchEnt=mssql_query($sqlFchEnt);
				$error=1;
				$msg = 'La fecha ingresada no esta en uno de los rangos.\n';
				while($rwFchEnt=mssql_fetch_array($qryFchEnt))
				{
					#/*
					$fFin = date( 'd', strtotime( $rwFchEnt['fechafinal'] ) );
					$fcI = date( 'd', strtotime( $fInicia ) );
	
					if($fcI>$fFin)#if($fcI>$fIni&&$fcI>$fFin)
					{
						$sqlFchEnt='SELECT * FROM AdpHT ';
						$sqlFchEnt=$sqlFchEnt.'WHERE id_proyecto = '.$cualProyecto;
						$sqlFchEnt=$sqlFchEnt.' AND unidad = '.$laUnidad;
						$sqlFchEnt=$sqlFchEnt.' AND vigencia = '.$cualVigencia;
						$sqlFchEnt=$sqlFchEnt.' AND mes = '.$cualMes;
						$qryFchEnt2=mssql_query($sqlFchEnt);
						$error=1;
						$msg = 'La fecha ingresada no esta en uno de los rangos.\n';
