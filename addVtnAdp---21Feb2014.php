...
include "validaUsrBd.php";
		...
		if($recarga==2)
		{
			...
			if($error==0)
			{
				...
				if($hayAdp[hayAdp]==0)
				{
					#	fechaInicio, fechafinal		///	(O), MONTH(O), DAY()
					#	Llave primaria				
...
//echo $sql_update."<br>".mssql_get_last_message()."<br><br>";
			...
			*/
			...
			{
				#mssql_query(" ROL<strong></strong>LBACK TRANSACTION");
				echo "<script>alert('".$msg."')</script>";
			}
			/*
			echo "<script>
					window.close();
					MM_openBrWindow('htVtnAdp.php?pMes=".$cualMes."','winADP','toolbar=yes,scrollbars=yes,resizable=yes,width=960,height=700');
				  </script>";
			#*/
		}
...
<script language="javascript" type="text/javascript">
	...
	function valida()
	{
		...
		{
			msg = 'El formato de ADP no corresponde.\n';
		}
		*/
		vigencia = vigenciaMes( document.getElementById('fchInicial').value, document.getElementById('fchFinal').value );
		if(vigencia==false)
		{
			msg = msg + 'Si registrando una ADP que cubre 2 meses diferentes. Registre primero desde la fecha que empieza hasta el dia que finaliza ese mes.\n y empiece en el primer dia del mes siguiente hasta fecha que finaliza.';
		}
          ...
          <td colspan="5"  class="TxtTabla"><!-- readonly-->
            ...
            <!--
              ...
              <tr>
                ...
                <td align="left" class="TxtTabla"><input type="text" class="CajaTexto" name="adp" id="adp" onKeyPress="return acceptNum(event);" /></td>
                </tr>
