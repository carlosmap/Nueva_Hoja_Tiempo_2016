...
if(trim($recarga) == "2"){
	//CONSULTA LA CANTIDAD DE LOSTES DE TRABAJO, ASOCIADOS CON EL LOTE DE CONTROL
/*
	 $sql_lt=mssql_query("select MAX(erpLT) lt_total from Actividades where id_proyecto=".$cualProyecto." and nivel=2 and erpLC= ");

SELECT erpLC from Actividades where id_proyecto=".$cualProyecto." and nivel=1 and id_actividad=".$LC."
*/

	//CONSULTA EL ID MAXIMO DEL CAMPO erpLT, DE LAS ACTIVIDADES DEL LOTE DE CONTROL, QUE SE HAN MOVIDO EN LOS DIFRENTES LC
	//ADEMAS, CONSULTA EL erLC, DEL LOTE DE CONTROL EN DONDE SE VAN A REGISTRAR LOD LOTES DE TRABAJO
	 $sql_lt=mssql_query("select MAX(erpLT) lt_total,(SELECT erpLC from Actividades where id_proyecto=".$cualProyecto." and nivel=1 and id_actividad=".$LC.") erpLC from Actividades where id_proyecto=".$cualProyecto." and nivel=2 and erpLC=(
	 select erpLC from Actividades where id_proyecto=".$cualProyecto."  and nivel=1 and id_actividad=".$LC.")");
	 
	 $datos_lotes=mssql_fetch_array($sql_lt);

//echo $datos_lotes["erpLC"]." *************** "."select MAX(erpLT) lt_total,( SELECT erpLC from Actividades where id_proyecto=".$cualProyecto." and nivel=1 and id_actividad=".$cualLC." ) erpLC from Actividades where id_proyecto=".$cualProyecto." and nivel=2 and actPrincipal=".$LC."<br><br>";

	$can_lt=( (int) $datos_lotes["lt_total"] ) + $pCantReg;
	$num_lote_trabajo=( (int) $datos_lotes["lt_total"] ) +1;
	...
	//echo $sqlId."<br>".mssql_get_last_message()."<br>"; 				
			...
			if( ((int) $num_lote_trabajo )<10)
				$num_lote_trabajo ='0'.$num_lote_trabajo ;
...
//			$num_lote_control = substr($Lote_Control,2,strlen($Lote_Control));
	...
	//echo ${$elpJefe}."  ---<br>";
			...
			$sqlIn1=$sqlIn1."dependeDe, actPrincipal, tipoActividad, nivelesActiv, nivel, erpLC, erpLT ,usuarioCrea,fechaCrea ) ";
			...
			$sqlIn1 = $sqlIn1 . " '".$datos_lotes["erpLC"]."', ";
			$sqlIn1 = $sqlIn1 . " '".$num_lote_trabajo."', ";
...
//	echo $sqlIn1."<br>".mssql_get_last_message()."<br>"; 				
		...
		echo ("<script>window.close();MM_openBrWindow('htPlanProyectos01.php?cualProyecto=".$cualProyecto."&aa=".$LC."','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");

	}
	else //SI SE SUPERARON LA CANTIDAD DE LC, PERMITIDA (MAX 99)
	{
			$cant_permitidos=99-((int) $datos_lotes["lt_total"]);
			echo ("<script>alert('No se puede registrar mas de ".$cant_permitidos." lotes de trabajo en este lote de control.');</script>");
	}
	 ...
	 //CONSULTA LA CANTIDAD DE LOSTES DE TRABAJO, ASOCIADOS CON EL LOTE DE CONTROL
//	 $sql_lt="select MAX(erpLT) lt_total from Actividades where id_proyecto=".$cualProyecto." and nivel=2 and actPrincipal=".$cualLC;

	if(!isset($LC))
		$LC=$cualLC;

	 $sql_lt="select MAX(erpLT) lt_total
	 from Actividades where id_proyecto=".$cualProyecto." and nivel=2 and erpLC=(
	 select erpLC from Actividades where id_proyecto=".$cualProyecto."  and nivel=1 and id_actividad=".$LC.")";
	 $cur_lt=mssql_query($sql_lt);
	 $datos_lotes=mssql_fetch_array($cur_lt);
//echo $datos_lotes["lt_total"]." ******** ".$sql_lt;
	if($datos_lotes["lt_total"]==99)
	{
//mas de 99 lotes de control en el proyecto
				echo ("<script>alert('No se puede registrar mas lotes de trabajo en este lote de control.');</script>"); 
		
				echo ("<script>window.close();MM_openBrWindow('htPlanProyectos01.php?cualProyecto=".$cualProyecto."&aa=".$cualLC."','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");
	}
	else
		$cant_permitidos=99-((int) $datos_lotes["lt_total"]);
//echo "<br>".$cant_permitidos." ******************* ";
...
<?php
 ...
 ?>
        ...
        <tr>
          <td width="22%" class="TituloTabla">Cantidad de Lotes de Trabajo</td>
...
<?
		if($cant_permitidos<=10)
		{
?>
            <tr>
              <td colspan="2" width="22%" class="TxtTabla">Solo puede registrar <?=$cant_permitidos ?> lotes de trabajo, en el lote de control. </td>
            </tr>
<?
		}
?>
