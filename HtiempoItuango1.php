...
?>
<?php
if (($_SESSION["sesUnidadUsuario"] == 15712) OR ($_SESSION["sesUnidadUsuario"] == 12974) OR ($_SESSION["sesUnidadUsuario"] == 14080) OR ($_SESSION["sesUnidadUsuario"] == 13104) ) {
?>

<form action="HtiempoItuango1.php" method="post" name="frm">
...
echo "<table class='txtTabla'><tr ><td width=85%><h2>HOJA DE TIEMPO</h2></td><TD width=15%><img src='imagenes/LogoIngetecPNG2.png' heigth=50 width=170></TD></tr></table>";
...
echo "<tr ><td class='tituloTabla'><b>Proyecto:</td><td class='txtTabla'>Proyecto Ituango Plan Inversión 1%</td></tr>";
...
$sql="SELECT RTRIM(localizacion) + '-' + RTRIM(cargo) AS codigoproyecto, cargo,
		...
		WHERE     (fecha BETWEEN $cadfecha) AND (unidad = '$laUnidad') and id_proyecto = 1798  and (cargo='460' or cargo='468' or cargo='469' ) order by dia";
...
//IMPRIME EL ENCABEZADO
echo "<table border=1 WIDTH='100%'>
<tr class='tituloTabla'><td width=3%><b><center>DIA</td><td width=3%><b><center>HR</td><td width=3%><b><center>CT</center></b></td>
		...
		if($IndDia<$indice){
			for($j=$IndDia;$j<$indice;$j++){
				...
				if ($nombredia==0 or $nombredia==6){
					...
					echo "<tr class='txtTabla'><td bgcolor='$color'>$j</td><td> </td><td> </td><td> </td><TD> </TD><TD> </TD></tr>";
				}else{
					if ($festivos[$j]==1) {
						$color=COLORFESTIVO;
						echo "<tr class='txtTabla'><td  bgcolor='$color'>$j</td><td> </td><td> </td><td> </td><TD> </TD><TD> </TD></tr>";
					}else{
						echo "<tr class='txtTabla'><td>$j</td><td> </td><td> </td><td> </td><TD> </TD><TD> </TD></tr>";
					}
				}
			}
			echo "<tr class='txtTabla'><td>$indice</td><td>$horasdia[$indice]</td><td>$tTiempo</td><td>$mActiv[$indice]</td>
		...
		for($j=$indice;$j<=31;$j++){
			...
			if ($nombredia==0 or $nombredia==6){
				...
				if ($festivos[$j]==1) {
					...
					echo "<tr class='txtTabla'><td width=30>$j</td><td width=55> </td><td> </td><td width=200> </td><TD width=300> </TD><TD width=300> </TD></tr>";
				}
			}
		}
	}
...
$sqlViat=" SELECT  distinct   Horas.resumen_trabajo, ViaticosHoras.horallegada, ViaticosHoras.horasalida, Horas.unidad, Viaticos.sitio, Horas.fecha
            ...
            Horas.fecha = ViaticosHoras.fecha
			WHERE (Horas.fecha BETWEEN $cadfecha) AND (ViaticosHoras.unidad = '$laUnidad') AND (ViaticosHoras.id_proyecto = 1798) order by Horas.fecha";
