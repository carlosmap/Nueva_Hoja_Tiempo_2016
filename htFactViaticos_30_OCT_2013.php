...
if($recarga==2)
{
	...
	$sql_usuarios="select distinct(FacturacionProyectos.unidad), upper (Usuarios.nombre +' '+Usuarios.apellidos) nombre, Usuarios.fechaRetiro,retirado from FacturacionProyectos 
					...
					where id_proyecto=".$cualProyecto." and vigencia=".$vigencia." and esInterno='I' and mes=".$pMes;
	$ban_consul=1;
}
?>
...
?>
            ...
            <tr class="TituloTabla2">
              ...
              <td width="5%">Dia de regreso <img title="1=Dia Completo, 2=Dia de Regreso." src="../NuevaHojaTiempo/imagenes/icoDetalleInf.gif"/> </td>
              <td width="15%">Aprobado</td>
...
//cant_aprobaciones
?>
		<input name="Submit" type="submit" class="Boton" value="VoBo" onClick="MM_openBrWindow('htVoBoFactViaticos.php?pMes=<?=$pMes ?>&vigencia=<?=$vigencia ?>&cualProyecto=<?=$cualProyecto ?>&unidad_u=<?=$datos_usu["unidad"] ?>','winAddFV','scrollbars=yes,resizable=yes,width=1000,height=700')" />
<?
	}
?>
	</td>
...
?>
  <tr>
    <td align="right"><input name="Submit2" type="submit" class="Boton" onClick="MM_openBrWindow('htVoBoFactViaticos.php?pMes=<?=$pMes ?>&vigencia=<?=$vigencia ?>&cualProyecto=<?=$cualProyecto ?>','winAddFV','scrollbars=yes,resizable=yes,width=1400,height=400')" value="Aprobación de la facturación" /></td>
  </tr>
<?
	}
?>
