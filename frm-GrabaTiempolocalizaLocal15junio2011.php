...
//			if (($_SESSION["sesUnidadUsuario"] == 12974 ) OR ($_SESSION["sesUnidadUsuario"] == 15712 ) ) {
          ...
          <td valign="top"><FORM name='formulario' ACTION="frm-GrabaTiempolocaliza.php" METHOD="POST">
...
el horario no lo permite*/
	...
	echo "<script>alert('$permisoPorFechaEspecial')</script>";
...
//		echo $zSql1 . "<br>";
	...
	if ($sClase == 7) {
		if ($numHorasCT6 != 8) {
			echo "<script>alert('No es posible grabar más Clase de tiempo $sClase sin haber registrado previamente 8 horas en clase de tiempo 6. Por favor corrija la información')</script>";
			exit();
		}
	}
	...
	*/
	...
	if (trim($locFecha) == "") {
		...
		if (trim($locFecha) == $ldLocalizacion) {
			...
			echo "<script>alert('No es posible grabar la localización $ldLocalizacion en este día porque ya existe una licalización diferente previamente registrada. Por favor corrija la información')</script>";
			exit();
		}
	}
...
function localizaProyEsp($peFecha, $peUni, $peLocalizacion, $peClaseTiempo) {
	...
	$peTipoContrato="";
	$peMensaje="";
	$peSql01="SELECT * FROM usuarios ";
	$peSql01=$peSql01." WHERE unidad = " . $peUni ;
	$peCursor = mssql_query($peSql01);
	if ($peReg=mssql_fetch_array($peCursor)) {
		$peSitioContrato=strtoupper($ldReg[SitioContrato]);
		$peSitioTrabajo=strtoupper($ldReg[SitioTrabajo]);
		$peTipoContrato=strtoupper($ldReg[TipoContrato]);
	}
	
	//Valida que esté grabando correctamente la clase de tiempo
	if (trim($peTipoContrato) == "MT") AND ($peClaseTiempo <> 2) {
	...
	if (trim($peMensaje) == "") {
		...
		echo "<script>alert('No es posible grabar la novedad por: ".trim($peMensaje).". Por favor corrija la información')</script>";
		exit();
	}
}
*/
