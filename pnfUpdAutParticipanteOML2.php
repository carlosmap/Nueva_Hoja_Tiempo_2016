...
//10Jun2008
//Identificar si el usuario activo verá toda la información o sólo sus TMPActividadesHT2
$esDC = 0 ;
$esProgP = 0;
$esOrdG = 0 ;
$todo= 0 ;
...
if( $recarga == 2 ){
	...
	while( $reg < $cantUsuarios ){
		$estado = "aplicaUsuario".$reg;
		$act = "idAct".$reg;

#		if( ${$opc} == 1 ){
		$sqlQry = " Update ".$tabla." set 
						estado = ".${$estado}."
					Where id_actividad = ".${$act}." AND unidad = ".$und." AND id_proyecto = ".$pro;
#		}		
		$qryUpd = mssql_query( $sqlQry );
		if( !$qryUpd )
			$mal = 1;
		#echo $sqlQry."<br />";		
		$reg++;
	...
	if( $mal == 0 ){
		echo "<script type='text/javascript' language='javascript'>
				...
				window.open( 'htProgProyectos02OML2.php?cualProyecto=".$pro."', 'winHojaTiempo' );
				window.close();
			  </script>";
	}
	else{
...
<?
	...
	$sqlDrCr = "select cr.unidad undC, dr.unidad undD, cr.nombre nCr, cr.apellidos aCr, dr.nombre nDr, dr.apellidos aDr from Proyectos pr, Usuarios cr, Usuarios dr
...
?>
    ...
    <tr class="TxtTabla">
    <td width="25%" valign="top"><?= $qryDrCr[aDr]." ".$qryDrCr[nDr]." [".$qryDrCr[undD]."]" ?></td>
    <td width="25%" valign="top" ><?= $qryDrCr[aCr]." ".$qryDrCr[nCr]." [".$qryDrCr[undC]."]" ?></td>
    <td width="25%" >
    <?
	...
	<form name="form1" id="form1" method="post" action="">
        ...
        <td bgcolor="#FFFFFF">
            ...
            <table width="100%"  border="0" cellspacing="1" cellpadding="0">
              ...
              <tr class="TxtTabla">
                <td valign="top"><?= $row[unidad] ?>
                <input type="hidden" name="pro" value="<?= $cualProyecto ?>" />
                <input type="hidden" name="und" value="<?= $row[unidad] ?>" /></td>
                <td valign="top">
                <?= $row[NombresApellidos] ?></td>
            ...
            <table width="100%"  border="0" cellspacing="1" cellpadding="0">
                ...
                <tr class="TituloTabla2">
                  <td colspan="4" valign="top">Actividad</td>
                  <td valign="top">Inactivo</td>
                </tr>
                <?
                    $sqlAct = "SELECT act.id_actividad idAct, act.nombre nActividad, act.macroactividad, res.estado, usu.unidad		
                ...
                <tr class="TxtTabla">
                    <td colspan="4" valign="top">
                        ...
                        <input type="hidden" name="idAct<?= $r ?>" value="<?= $rw[idAct] ?>" /></td>
                    <td width="15%" valign="top">
						...
						?>
                        <input type="radio" name="aplicaUsuario<?= $r ?>" value="1" <?= $chk1 ?>/>Si
                        <input type="radio" name="aplicaUsuario<?= $r ?>" value="0"  <?= $chk2 ?>  />No
                    </td>
                </tr>
                <?	$r++;
                    }
...
<input name="Submit" type="submit" class="Boton" onclick="envia2()" value="Actualizar" /></td>
              </tr>
            </table>
            <table width="100%"  border="0" cellspacing="0" cellpadding="0">
                <tr>
