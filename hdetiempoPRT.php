...
//	echo "<head>";
		...
		header("Content-Disposition: attachment; filename=HojaTiempo".$laUnidad.".xls");
//	echo "</head>";
	$convertir="";
}

//echo $_SESSION["sesCERES"] ."<br>";
//echo $_SESSION["sesConvenio"] ."<br>";
?>
<?php
...
$qSql2=$qSql2." and unidad =" . $laUnidad;
...
if ($cantidadFilas > 0) {
	...
	if ($muestraBoton == 0) {
		//$mensajeHT = "Hoja de tiempo en revisión del jefe sin aprobación " . "<br>" . $ObsJefeAprueba ;
		$cualImagen = "<img src='img/images/No.gif' alt='$mensajeHT' border='0'>";
	}
	else {
		//$mensajeHT = "Hoja de tiempo Aprobada" . $ObsJefeAprueba ;
...
<body leftmargin="10" topmargin="3" rightmargin="10" bottommargin="0"  bgcolor="E6E6E6">

<!--<div style="position:absolute; left:0px; top:0px; height: 0px;">-->
<form action="hdetiempoPRT.php" name=reporte method="post">
<?
...
$tcSql=$tcSql." WHERE (unidad = ".$laUnidad. ")";
...
//	$enfermedad[1]="1";
	...
	$ADP = consultaADP($laUnidad,$cadfecha,'ENF0');
	if($ADP!=-1){
...
//	$acciddetrabajo[1]="1";
	...
	$ADP = consultaADP($laUnidad,$cadfecha,'ACC0');
	if($ADP!=-1){
...
//	$permisospacto[1]="1";
	...
	$ADP = consultaADP($laUnidad,$cadfecha,'PER0');
	if($ADP!=-1){
...
//	$licencias[1]="1";
	...
	$ADP = consultaADP($laUnidad,$cadfecha,'LIC0');
	if($ADP!=-1){
...
//	$sanciones[1]="1";
	...
	$ADP = consultaADP($laUnidad,$cadfecha,'SAN0');
	if($ADP!=-1){
...
//	echo "<table class='TxtTabla' border='1' bordercolor=black>";
/*
...
$eSql=$eSql." where unidad = " . $laUnidad;
...
/*********************************************IMPRIME LAS HORAS FACTURADAS***********************************************/
		...
		while($filas){
			...
			//8Nov2007
			//Si el proyecto es Gastos Generales Mostrar la imagen para especificar o cambiar el jefe 
			//que autoriza Gastos Generales del usuario activo
			if ($id_proy == 42) {
				//Consulta la unidad de la persona que está firmando
				
				$elUsuarioRevisaGG = "";
				//Verificar si el usuario ya existe para mostrar el jefe ya seleccionado
				$rSQLgg="Select R.* , U.nombre, U.apellidos ";
				$rSQLgg=$rSQLgg." from RevisaGastosGenerales R, Usuarios U ";
				$rSQLgg=$rSQLgg." where R.unidadRevisa = U.unidad ";
				if (trim($_SESSION["sesUnidadUsuario"]) != trim($laUnidad)  ) {
					$rSQLgg=$rSQLgg." and R.unidad = " . $laUnidad ;
				}
				else {
					$rSQLgg=$rSQLgg." and R.unidad = " . $_SESSION["sesUnidadUsuario"];
				}
				$rCursorGG = mssql_query($rSQLgg);
				if ($rRegGG=mssql_fetch_array($rCursorGG)) {
					$elUsuarioRevisaGG = strtoupper($rRegGG[nombre] . " " . $rRegGG[apellidos]);
				}
				
				$imgMuestra= "<a href='#'><img src='img/images/actualizar.jpg' alt='$elUsuarioRevisaGG (Grabar/Actualizar)' width='19' height='17' border='0' onclick=\"MM_openBrWindow('addAutorizaGG.php','vGG','scrollbars=yes,resizable=yes,width=400,height=150')\" /></a>" ;
			}
			else {
				$imgMuestra= "" ;
			}
			echo "<td nowrap><font face='Arial' size=1>$codproy [$act] $nProyecto</font>$imgMuestra</td>";
			...
			for($i=1;$i<=31;$i++){
				...
				if($horasdia[$i]==0){
					...
					//Pone el link del resumen del trabajo 
			...
			$qSqlvb=$qSqlvb . " and A.unidad = $laUnidad " ;
...
//			echo "<td align=center> $qSqlvb  </td>";			
...
/*			$sql="SELECT rtrim(localizacion)+ '-' +rtrim(cargo) AS codigo, DAY(fechaIni) AS diaIni, DAY(fechaFin) AS diaFin, IDTipoViatico, id_proyecto, IDSitio, viaticoCompleto , id_actividad ";
			$sql1="FROM ViaticosProyecto WHERE  (fechaIni BETWEEN $cadfecha) AND (fechaFin BETWEEN $cadfecha) AND(unidad = '$laUnidad') ";
...
*/
			...
			$sql1=" and (V.fechaIni BETWEEN $cadfecha) AND (V.fechaFin BETWEEN $cadfecha) AND(V.unidad = '$laUnidad')";
				...
				if(mssql_num_rows($resultado)>0){
					...
					while($filas) {
						...
						while(($idActividad==$filas[id_actividad]) AND ($codigo == $filas[codigo] ) AND ($tipoViatico == $filas[IDTipoViatico] ) ) // ****2014 Adicion de AND ($tipoViatico == $filas[IDTipoViatico] )
						   {    //20140325
						   		/****2014: Comentariado las consultas para hallar nombre Viatico, Proyecto y Sitio ***************
								//Nuevo
...
<!--<div id="Layer4">-->
<table width="100%" border="0" cellpadding="0" cellspacing="0" class='TxtTabla' >
<? 
...
<tr><td width="15%">
  Unidad 
      <input type=text size=6 name=unidadvar value='<? echo $laUnidad; ?>' readonly></td><td>
<?	include("fechahoja.php"); ?>
<input name="verhoja" type="submit" class="Boton" value="Consultar" />

</td>
      <td align="right"><a href="hdetiempoPRT.php?convertir=1">Grabar a EXCEL</a>
      <input name="Submit2" type="submit" class="Boton" onclick="MM_goToURL('parent','frm-GrabaTiempo.php');return document.MM_returnValue" value="P&aacute;gina principal Hoja  de tiempo" />        <?
//El botón sólo aparec si la Hoja de tiempo no ha sido aprobada por el jefe
if ($muestraBoton == 0) {
?>
<input name="Submit" type="button" class="Boton" onclick="MM_openBrWindow('addAutorizaHT.php?cualAno=<? echo $Flano ; ?>&cualMes=<? echo $Flmes ; ?>','vAut','scrollbars=yes,resizable=yes,width=400,height=150')" value="Especificar Jefe Inmediato-Revisión" />
<? } ?></td>
</tr>
</table>
<!--</div>-->
...
<?php
	...
	//Imprime
	echo "<table class='TxtTabla' width=100% border = 1>";
