...
//****************************PERFILAMIENTOS DE lc, lt, dIVISIONES Y aCTIVIADES
//02FEB2013
...
//************************************
	...
	*/
	...
	//echo "Act=" . trim($actividad) . "<br>" ;
	if (trim($actividad) == "") {
		$miActividad = "" ;
		//Si la actividad está vacia, se rearga con el id_actividad de la división
		$vSqlB="select id_actividad  from HojaDeTiempo.dbo.Actividades ";
		$vSqlB=$vSqlB." Where id_proyecto =" . $cualProyecto;
		$vSqlB=$vSqlB." and dependeDe = " . $lt;
		$vSqlB=$vSqlB." and nivel = 3 ";
		$vSqlB=$vSqlB." and id_division = " . $pfDivision ;
		//echo "vSqlB=" . trim($vSqlB) . "<br>" ;
		$vCursorB = mssql_query($vSqlB);
		if ($vRegB=mssql_fetch_array($vCursorB)) {
			$miActividad = $vRegB[id_actividad] ;
		}
	}
	else {
	...
	$sql3 = $sql3 . " (" ;
	$sql3 = $sql3 . " SELECT DISTINCT unidad " ;
	$sql3 = $sql3 . " From ParticipantesActividad " ;
	$sql3 = $sql3 . " where id_proyecto =" . $cualProyecto ;
	$sql3 = $sql3 . " and id_actividad = " . $miActividad ;
	$sql3 = $sql3 . " )" ;
...
//	echo $sql3 . "<br>";
	...
	*****************/
	if($recarga == 2){
		...
		//Si la actividad está vacia, se rearga con el id_actividad de la división
		$vSqlA="select id_actividad  from HojaDeTiempo.dbo.Actividades";
		$vSqlA=$vSqlA." Where id_proyecto =" . $cualProyecto;
		$vSqlA=$vSqlA." and dependeDe = " . $lt;
		$vSqlA=$vSqlA." and nivel = 3";
		$vSqlA=$vSqlA." and id_division = " . $pfDivision ;
		$vCursorA = mssql_query($vSqlA);
		if ($vRegA=mssql_fetch_array($vCursorA)) {
			$actividad = $vRegA[id_actividad] ;
		}
...
function envia2(){
	...
	v1 = 1
	v2 = v3 = 0;
	m1 = "Debe seleccionar al menos una persona.\n";
	m2 = m3 = msg = "";
	...
	if( registros == -1 ){
		v2 = 1; 
		m2 = "Debe hacer el filtro para elegir una persona. \n";
	}
	if( document.getElementById('pfDivision').value == "" ){
		v3 = 1;
		m3 = "Debe seleccionar una División.\n";
	}
	/*
	//Se bloque porque no es indispensable una actividad
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="TituloUsuario">.:: Participantes del proyecto </td>
  </tr>
</table>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	//si $verBoton="SI" es Director, coordinador, ordenador de gasto o programador del proyecto y ve TODO
		if ($verBoton=="SI") { 
			...
			//Los LC asociados a una persona que no es Director, cooordinado, ordenador de gasto o programador de proyecto
			$sqlLc = "select * from HojaDeTiempo.dbo.Actividades Where id_proyecto = ".$cualProyecto." AND nivel = 1 AND id_encargado = ".$_SESSION['sesUnidadUsuario'];
			$sqlLc = "select *  ";
			$sqlLc = $sqlLc . " from HojaDeTiempo.dbo.Actividades ";
			$sqlLc = $sqlLc . " Where id_proyecto = " . $cualProyecto ;
			$sqlLc = $sqlLc . " AND nivel = 1  ";
			$sqlLc = $sqlLc . " AND id_actividad in ( ";
			$sqlLc = $sqlLc . "     SELECT id_actividad ";
			$sqlLc = $sqlLc . " 	FROM Actividades ";
			$sqlLc = $sqlLc . " 	WHERE id_proyecto = " . $cualProyecto ;
			$sqlLc = $sqlLc . " 	and id_encargado =" . $laUnidad;
			$sqlLc = $sqlLc . " 	UNION ";
			$sqlLc = $sqlLc . " 	SELECT id_actividad ";
			$sqlLc = $sqlLc . " 	FROM ResponsablesActividad ";
			$sqlLc = $sqlLc . " 	WHERE id_proyecto = " . $cualProyecto ;
			$sqlLc = $sqlLc . " 	and unidad = " . $laUnidad;
			$sqlLc = $sqlLc . " ) ";
		}
  ...
  <tr>
    ...
    <td align="left" class="TxtTabla"><?
			if ($verBoton=="SI") { 
				...
				$sqlLt = $sqlLt . " AND id_actividad in ( ";
				$sqlLt = $sqlLt . "     SELECT id_actividad ";
				$sqlLt = $sqlLt . " 	FROM Actividades ";
				$sqlLt = $sqlLt . " 	WHERE id_proyecto = " . $cualProyecto ;
				$sqlLt = $sqlLt . " 	and id_encargado =" . $laUnidad;
				$sqlLt = $sqlLt . " 	UNION ";
				$sqlLt = $sqlLt . " 	SELECT id_actividad ";
				$sqlLt = $sqlLt . " 	FROM ResponsablesActividad ";
				$sqlLt = $sqlLt . " 	WHERE id_proyecto = " . $cualProyecto ;
				$sqlLt = $sqlLt . " 	and unidad = " . $laUnidad;
				$sqlLt = $sqlLt . " ) ";
			}
  ...
  <tr>
    ...
    <td align="left" class="TxtTabla"><?
		if ($verBoton=="SI") { 
			...
			$sqlDv = $sqlDv . " AND id_actividad in ( ";
			$sqlDv = $sqlDv . "     SELECT id_actividad ";
			$sqlDv = $sqlDv . " 	FROM Actividades ";
			$sqlDv = $sqlDv . " 	WHERE id_proyecto = " . $cualProyecto ;
			$sqlDv = $sqlDv . " 	and id_encargado =" . $laUnidad;
			$sqlDv = $sqlDv . " 	UNION ";
			$sqlDv = $sqlDv . " 	SELECT id_actividad ";
			$sqlDv = $sqlDv . " 	FROM ResponsablesActividad ";
			$sqlDv = $sqlDv . " 	WHERE id_proyecto = " . $cualProyecto ;
			$sqlDv = $sqlDv . " 	and unidad = " . $laUnidad;
			$sqlDv = $sqlDv . " ) ";
		}
      ...
      <?
		  if ($verBoton=="SI") { 
				...
				$sqlActividad = "SELECT macroactividad, id_actividad, nombre, id_proyecto, id_encargado FROM Actividades 
								 WHERE dependeDe in( Select id_actividad from Actividades where id_division = ".$pfDivision." AND id_proyecto = ".$cualProyecto." ) 
								 AND id_proyecto = ".$cualProyecto;
				$sqlActividad = $sqlActividad . " AND id_actividad in ( ";
				$sqlActividad = $sqlActividad . "     SELECT id_actividad ";
				$sqlActividad = $sqlActividad . " 	FROM Actividades ";
				$sqlActividad = $sqlActividad . " 	WHERE id_proyecto = " . $cualProyecto ;
				$sqlActividad = $sqlActividad . " 	and id_encargado =" . $laUnidad;
				$sqlActividad = $sqlActividad . " 	UNION ";
				$sqlActividad = $sqlActividad . " 	SELECT id_actividad ";
				$sqlActividad = $sqlActividad . " 	FROM ResponsablesActividad ";
				$sqlActividad = $sqlActividad . " 	WHERE id_proyecto = " . $cualProyecto ;
				$sqlActividad = $sqlActividad . " 	and unidad = " . $laUnidad;
				$sqlActividad = $sqlActividad . " ) ";
		}						 
    ...
    <td colspan="2"><table width="100%"  cellspacing="1">
	  ...
	  if($miDpto != 0 || trim($nombre) != "" || trim($unidad) != ""){
		  ...
		  while($reg3 = mssql_fetch_array($cursor3)){
			  $sqlRA = "select COUNT(*) registrado from HojaDeTiempo.dbo.ParticipantesActividad where id_proyecto = ".$cualProyecto." and unidad = ".$reg3['unidad']." and id_actividad = ".$actividad;
