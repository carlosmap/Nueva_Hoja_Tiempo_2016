	...
	function infoHTUsuario($unidad, $mes, $vigencia)
	{
		...
		{	
			...
			$tabla = $tabla .'<tr>
				...
				$tabla = $tabla .'<tr>
								<td width="17%" align="left" > 
								'.utf8_encode($reg02['localizacion'].'-'.$reg02['codigo'].''.$reg02['cargo_defecto'].'['.$reg02['macroactividad'].'] '.substr( utf8_encode($reg02['nombre']), 0, 15)).'
								</td>						
								<td width="2%" align="center" >'.$reg02['clase_tiempo'].'</td>';
				//25Jul2013
				//PBM
		...
		$col = 5 + $totalDiasMes;
		$tabla = $tabla .'
		<tr class="TituloTabla2"><th colspan="'.$col.'"> RELACI&Oacute;N DE VIÁTICOS - DÍAS DE CAMPO</th></tr>';
			...
			while ($reg11 = mssql_fetch_array($cursor11)) {
			   ...
			   $sql14=$sql14." AND A.IDTipoViatico = " . $reg11['IDTipoViatico'] ;
				...
				$tabla = $tabla .'<th width="12%">&nbsp;<strong>'.utf8_encode($reg11['NomTipoViatico']).'</strong>';
				if($reg11['NomSitio']!='')
				{
	...
	function detalleHt($unidad, $mes, $vigencia)
	{
		...
		{							
			...
			if($mostrar==1)
			{
				...
				{
					$detalle = $detalle . '&nbsp;<strong>*</strong>'.trim($row['resumen']).'.';
				}
				$detalle = $detalle .'</td>
					</tr>';
			}
