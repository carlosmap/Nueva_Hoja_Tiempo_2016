...
<form action="hdetiempoTMP.php" name=reporte method="post">
<?
...
//	$vacaciones[1]="1";
	...
	$sql="SELECT * FROM HorasTMP WHERE cargo = 'vac0' AND unidad = $verUnidadDe AND fecha between $cadfecha";
...
//	$enfermedad[1]="1";
	...
	$sql="SELECT * FROM HorasTMP WHERE cargo = 'enf0' AND unidad = $verUnidadDe AND fecha between $cadfecha";
...
//	$acciddetrabajo[1]="1";
	...
	$sql="SELECT * FROM HorasTMP WHERE cargo = 'acc0' AND unidad = $verUnidadDe AND fecha between $cadfecha";
...
//	$permisospacto[1]="1";
	...
	$sql="SELECT * FROM HorasTMP WHERE cargo = 'per0' AND unidad = $verUnidadDe AND fecha between $cadfecha";
...
//	$licencias[1]="1";
	...
	$sql="SELECT * FROM HorasTMP WHERE cargo = 'lic0' AND unidad = $verUnidadDe AND fecha between $cadfecha";
...
//	$sanciones[1]="1";
	...
	$sql="SELECT * FROM HorasTMP WHERE cargo = 'san0' AND unidad = $verUnidadDe AND fecha between $cadfecha";
...
//	$ausencias[1]="1";
	...
	$sql="SELECT * FROM HorasTMP WHERE cargo = 'aus0' AND unidad = $verUnidadDe AND fecha between $cadfecha";
...
//echo $encabSql . "<br>" ;
	...
	$sql="SELECT estadoAprobDivision, comentariosDivision, revisadoPorDivision, estadoAprobProyecto, comentariosProyecto,
		revisadoPorProyecto, RTRIM(localizacion) + '-' + RTRIM(cargo) AS codigoproyecto,
		 horas_registradas, clase_tiempo, resumen_trabajo, day(fecha) as dia, id_actividad, id_proyecto FROM HorasTMP
...
/*********************************************IMPRIME LAS HORAS FACTURADAS***********************************************/
		...
		while($filas){
			...
			while(($codproy==$filas[codigoproyecto]) and ($ttiempo==$filas[clase_tiempo]) and ($mactividad==$filas[id_actividad])){
					...
					//10Feb2009
					//Trae el nombre del proyecto porque no aparece cuando se reemplazó el proyecto
					$sqlwProy="select * from proyectosTMP where id_proyecto = $id_proy";
					$curwProy=mssql_query($sqlwProy);
					$regwProy=mssql_fetch_array($curwProy);
					$nProyecto =  ucwords(substr($regwProy[nombre],0,14));
			...
			if ($id_proy == 42) {
				...
				if (trim($_SESSION["sesUnidadUsuario"]) != trim($verUnidadDe)  ) {
					$rSQLgg=$rSQLgg." and R.unidad = " . $verUnidadDe ;
				}
				else {
...
<tr><td width="15%">
  Unidad 
      <input type=text size=6 name=unidadvar value='<? echo $verUnidadDe; ?>' readonly></td><td>
...
<?php
	...
	$sql = "SELECT DISTINCT cargo AS Cargo FROM HorasTMP
	...
	$sql="SELECT     distinct HorasTMP.cargo, Proyectos.nombre AS NombreProyecto,  HorasTMP.fecha,
	HorasTMP.id_proyecto,HorasTMP.resumen_trabajo
	FROM HorasTMP INNER JOIN Proyectos ON HorasTMP.id_proyecto = Proyectos.id_proyecto
	WHERE     (HorasTMP.unidad = $verUnidadDe) AND (HorasTMP.fecha BETWEEN $cadfecha)
	ORDER BY HorasTMP.fecha asc";
