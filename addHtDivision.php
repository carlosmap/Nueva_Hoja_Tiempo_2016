...
//echo $val_asignar."=".$val_asig_div." - ".$val_div_lt."   (".$val_asignar." / ".$val_asig_div." / ".$val_div_lt." ) <br>";
	...
	{	
		...
		while ($s <= $pCantReg) {
			...
			$sql_nom_div="select nombre,id_division,secuenciaDAF from Divisiones where  estadoDiv='A' and id_division =".${$ladivision};
			$cursor_nom_div=mssql_query($sql_nom_div);
			if($datos_nom_div=mssql_fetch_array($cursor_nom_div))
			{
				...
				$DAF_div=$datos_nom_div["secuenciaDAF"];
			}
			...
			$sqlIn1 = $sqlIn1 . " '3', '".$DAF_div."' ,'".$erp_lc."' ,'".$erp_lt."'  ,".$_SESSION["sesUnidadUsuario"].",getdate()  ";
			if(trim(${$valor})!="")
			{
...
*/
		...
		$sql2=$sql2." and U.retirado is null order by apellidos ";
