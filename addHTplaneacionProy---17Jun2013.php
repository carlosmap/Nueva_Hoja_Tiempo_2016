...
$sql04="SELECT * FROM ( ";
$sql04=$sql04." SELECT nombre, apellidos, unidad, 'I' tipoUsuario ";
...
$sql04=$sql04." 	) ";
$sql04=$sql04." ) U ";
$sql04=$sql04." WHERE unidad NOT IN ";
$sql04=$sql04." 	(";
$sql04=$sql04." 	select distinct unidad from PlaneacionProyectos ";
$sql04=$sql04." 	WHERE id_proyecto = " . $cualProyecto;
$sql04=$sql04." 	AND id_actividad = ". $cualAct; 
$sql04=$sql04." 	AND vigencia =" . $lstVigencia;
$sql04=$sql04."		) ";
			...
			*/
			...
			if ( (trim(${$eltxtPlan}) != "") AND (${$eltxtPlan} != 0) ) {
				...
				if  (trim($cursorIn1) != "")  {
					...
					$msgGraba=$msgGraba."[".${$ellstUnidadP}." - " . $d . " - " .  ${$eltxtPlan}. "] \\r" ;
				}
				else {
...
*/			
	...
	//if  (trim($cursorIn1) != "")  {
	if  (trim($msgNOGraba) != "")  {
		echo ("<script>alert('No se realizó planeación para: $msgNOGraba ');</script>"); 
	} 
	
	if  (trim($msgGraba) != "")  {
		echo ("<script>alert('Se grabó la planeación para las siguientes Unidades - Mes - Dedicación: \\r$msgGraba ');</script>"); 
	} 
	else {
		echo ("<script>alert('Error durante la grabación');</script>");
	};
	echo ("<script>window.close();MM_openBrWindow('htPlanProyectos03.php?cualProyecto=$miProyecto&lstLC=$fldLoteC&lstLT=$fldLoteT&lstDiv=$fldLoteDiv&opcID=$miActividad&lstVigencia=lstVigencia','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");
}
...
//Verifica que el valor total no supere el valor de la actividad
if( (totalizaFac()) == 1 ) {
	v1='n';
	msg1 = 'La planeación supera el valor total asignado al recurso. \n';
}

//Verifica si se ha realizado planeación al menos para una persona.
if ((trim(document.Form1.txtTotalPlaneado.value) == '') || (parseFloat(document.Form1.txtTotalPlaneado.value) == 0))  {
	v2='n';
	msg2 = 'Debe planear al menos una unidad para llevar a cabo la grabación. \n';
}
