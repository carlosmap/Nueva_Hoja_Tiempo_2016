...
if(trim($recarga) == "2")
{
	...
	$sql_del_prog="delete  from AutorizaEDT where id_proyecto=".$cualProyecto;
...
//echo mssql_get_last_message(); 
	...
	if($error=="no")
	{
		$sql_del="delete from  HojaDeTiempo.dbo.PlaneacionProyectos   where id_proyecto=".$cualProyecto;
...
//echo mssql_get_last_message(); 
	...
	if($error=="no")
	{
		$sql_del="delete from  HojaDeTiempo.dbo.ActividadesRecursos where id_proyecto=".$cualProyecto;
		$cur_del_val_act=mssql_query($sql_del);
		if  (trim($cur_del_val_act) == "")  
		{
			$error="si";
		}
	}
//echo mssql_get_last_message(); 
	...
	if($error=="no")
	{
		$sql_del="delete from  HojaDeTiempo.dbo.ParticipantesActividad where id_proyecto=".$cualProyecto;
		$cur_del_par=mssql_query($sql_del);
		if  (trim($cur_del_par) == "")  
		{
			$error="si";
		}
...
-->
      <table width="100%"  border="0" cellspacing="1" cellpadding="0">
		...
		<tr>
          ...
          <td  align="center" class="TxtTabla">Nota: Al eliminar la EDT, se eliminar&aacute;n  todos sus Lotes de Control, Lotes de Trabajo, Divisiones,  Actividades, Fechas, Planeaci&oacute;n y Participantes Esta operaci&oacute;n es irreversible.<br/><br />
