...
<tr class='TituloUsuario'><td colspan='6' bgcolor='#999999' ></td></tr>";
	...
	--SE IMPLEMENTO DISTINCT, PARA EVITAR, QUE LA CONSULTA ARROJE REGISTROS DUPLICADOS
					...
					select distinct unidad,id_proyecto,nombre,nombre_depto,nombre_div,vigencia, '1' tipo from (	select Usuarios.unidad,id_proyecto, Upper(Usuarios.nombre+' '+ Usuarios.apellidos) nombre ,total_facturacion, hombresMes, upper(Departamentos.nombre) nombre_depto ,upper(Divisiones.nombre) nombre_div,vigencia  from (
	...
	--SE IMPLEMENTO DISTINCT, PARA EVITAR, QUE LA CONSULTA ARROJE REGISTROS DUPLICADOS
					...
					select distinct unidad,id_proyecto,nombre,nombre_depto,nombre_div,vigencia, '2' tipo from (	select Usuarios.unidad,id_proyecto, Upper(Usuarios.nombre+' '+ Usuarios.apellidos) nombre ,total_facturacion, hombresMes, upper(Departamentos.nombre) nombre_depto ,upper(Divisiones.nombre) nombre_div,vigencia  from (
					select id_proyecto,id_actividad,unidad,hombresMes,mes,vigencia,(
...
//echo "Ingresooooo 2 <br><br>".$sql_fact."<br><br>";
			...
			{ 
			 	$sql_fact=$sql_fact."select distinct(FacturacionProyectos.unidad), FacturacionProyectos.id_proyecto, Upper(Usuarios.nombre+' '+ Usuarios.apellidos) nombre , upper(Departamentos.nombre) nombre_depto ,upper(Divisiones.nombre) nombre_div, vigencia, '3' tipo  from  FacturacionProyectos
