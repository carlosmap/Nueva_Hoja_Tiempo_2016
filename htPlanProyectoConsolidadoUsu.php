...
<body bgcolor="#EAEAEA" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0">
 ...
 <td width="15%" height="20" class="FichaInAct" > <a href="htPlanProyectoConsolidadoProy.php?division=<?=$division; ?>&departamento=<?=$departamento ?>&categoria=<?=$categoria ?>&empleado=<?=$empleado ?>" class="FichaInAct1" >Usuarios por Proyecto</a></td>
    	    ...
    	    <td width="15%"    class="FichaInAct"><a href="htReporFactHT.php?pDivision=<?=$division; ?>&pDepto=<?=$departamento ?>&pCategoria=<?=$categoria ?>&lstVigencia=<?=$lstVigencia ?>" class="FichaInAct1" >Facturaci&oacute;n por Divisi&oacute;n</a></td>
          ...
          <tr class="TxtTabla" >
            ...
            <td width="40%" rowspan="2"   align="center">
              <table width="100%" border="0" cellpadding="0" cellspacing="1"  bgcolor="#FFFFFF">
                <tr>
                <td class="TituloUsuario" colspan="2">Criterios de consulta </td>
                </tr>
				<tr>
...
<td width="15%"   class="TituloTabla">Departamento</td>
            <td width="10%"  align="left" class="TxtTabla"><select name="departamento" class="CajaTexto" id="departamento"  onchange="document.Form1.submit();">
...
?>
                ...
                <tr class="TxtTabla" >
                  ...
                  <td width="10%"  align="left"><select name="categoria" class="CajaTexto" id="categoria" onchange="document.Form1.submit();" >
                    <option value="">::Seleccione Categoria::</option>
                    <?
						$sql_div="select * from Categorias where estadoCat='A' order by nombre ";
...
?>
  ...
  <input type="hidden" name="ba" value="<?=$ba; ?>" id="ba" />   
		<?php
			if((trim($division)!="") and (trim($lstVigencia)!="") and ($recarga==1) )
			{
		?>
		            <input onclick="MM_openBrWindow('consolidado_total_div_xls.php?division=<?=$division;?>&amp;departamento=<?=$departamento;?>&amp;categoria=<?=$categoria;?>&proyecto=<?=$proyecto; ?>&lstVigencia=<?=$lstVigencia; ?>&empleado=<?=$empleado; ?>','wRPT1','scrollbars=yes,resizable=yes,width=500,height=400')" type="button" class="Boton" value="Descargar en XLS" />
...
//----------------------------------------------------------------
			...
			?>
              ...
              <tr>
                <td width="2%" class="TituloTabla2">F</td>
<?php
						$sql_total_factu="select (select  SUM(hombresMesF) as total_H_M from FacturacionProyectos where vigencia=".$lstVigencia." and  unidad=".$datos_usuarios_planeados["unidad"].") as total_H_M , SUM(hombresMesF)  as hombresMes ,mes from FacturacionProyectos where  vigencia=".$lstVigencia." and  unidad=".$datos_usuarios_planeados["unidad"]." group by(mes) ORDER BY(mes)";
						$cur_total_factu=mssql_query($sql_total_factu);

					$ban=1;  //permite saber que mes del año se esta dibujando(1,2,3....,12)
					$total=0;
//----------------------------------------------------------------
						while($datos_total=mssql_fetch_array($cur_total_factu))
						{
		...
		//									$total_perso-=0.75;
											$total_perso=0;
											$ima="../portal/imagenes/eje100.gif";
										}
		...
		//									$total_perso-=0.5;
											$total_perso=0;
											$ima="../portal/imagenes/eje75.gif";
										}
										else if ( ($total_perso<=0.5) && (0.25<$total_perso) ) // si el valor esta entre 0.5 y 0.25
										{
		//									$total_perso-=0.25;
											$total_perso=0;
											$ima="../portal/imagenes/eje50.gif";
										}
										else if ( ($total_perso<=0.25) && (0<$total_perso) ) // si el valor esta entre 0.25 y 0.01
										{
		//									$total_perso-=0.25;
											$total_perso=0;
											$ima="../portal/imagenes/eje25.gif";
										}
										else
										{ $ima="imagenes/eje5.gif"; } 
										
		
									}
									else //si el valor es mayor a 1
									{
										$ima="../portal/imagenes/eje1.gif";
										$total_perso-=1;
									}
		?>
				<td width="4%" align="left" background="<?  echo $ima; ?>" class="TxtTabla" ><?php echo $datos_total["hombresMes"]; ?></td>                        
