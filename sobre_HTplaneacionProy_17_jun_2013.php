...
function sobre_planeacion($minimoMes,$maximoMes,$vigencia,$unidades)
		{
			...
			<td colspan="2" >
				...
				foreach($unidades as $unid)
				{
					...
					$sql_planea_usu="select top(1) SUM (hombresMes) as total_hombre_mes ,PlaneacionProyectos.unidad ,mes 
									,PlaneacionProyectos.id_proyecto, PlaneacionProyectos.unidad, Usuarios.nombre, 
					...
					$sql_planea_usu=$sql_planea_usu."0) and PlaneacionProyectos.unidad=".$unid." 
									group by  PlaneacionProyectos.id_proyecto,PlaneacionProyectos.unidad,mes,Usuarios.nombre, Usuarios.apellidos,Divisiones.nombre ,Departamentos.nombre 
...
echo $sql_planea_usu." ---- <br>".mssql_get_last_message()." *** ".mssql_num_rows($cur_planea_usu)."<br>";
					while($datos__planea_usu=mssql_fetch_array($cur_planea_usu))
					{
						...
						$sql_uu=$sql_uu."0) and unidad in(".$unid.") group by unidad, mes , PlaneacionProyectos.id_proyecto,Proyectos.nombre,Proyectos.codigo,Proyectos.cargo_defecto  )   aa ";
// HAVING (SUM (hombresMes)	>1	
...
//echo "<br><BR>---//**************--------------".$sql_uu." ".mssql_get_last_message()."<br>".$cant_proy."<br>";
				...
				//echo $sql_proy_planea." ---- <br>".mssql_get_last_message()."<br><br>";
						...
						while($datos_uu=mssql_fetch_array($cur_uu))
						{
							...
							$sql_pro=$sql_pro." 0)group by PlaneacionProyectos.id_proyecto ,PlaneacionProyectos.unidad ,mes order by (mes) ";
// HAVING (SUM (hombresMes))>1
							...
							{	
								for ($m;$m<=$maximoMes; $m++) 
								{
									if($datos_proy_planea["mes"]==$m)
									{
										...
										$pTema = $pTema.'<td class="Estilo2">'.((float) $datos_proy_planea["total_hombre_mes"] ).'</td>';
