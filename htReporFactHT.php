...
<script language="JavaScript">
	...
	function envia0()
	{
		...
		if(document.form1.lstVigencia.value=="0")
...
<td width="15%" height="20" class="FichaInAct" > <a href="htPlanProyectoConsolidadoProy.php?division=<?=$division; ?>&departamento=<?=$departamento ?>&categoria=<?=$categoria ?>&empleado=<?=$empleado ?>" class="FichaInAct1" >Usuarios por Proyecto</a></td>
    	    ...
    	    <td width="15%"    class="FichaAct">Facturaci&oacute;n por Divisi&oacute;n</td>

    	    <td width="15%"    class="FichaInAct"><a href="htRptHTDivision.php?pDivision=<?=$division; ?>&miDpto=<?=$departamento ?>&pFiltro=<?=$categoria ?>&pAno=<?=$lstVigencia ?>" class="FichaInAct1" >Hoja de tiempo por División</a></td>
			 <td width="55%" height="20" class="TxtTabla">	</td>
          </tr>
      <tr>
...
?>
      ...
      <tr class="TxtTabla" >
        ...
        <td align="left">
<?
//	echo date('Y')."---- ".$lstVigencia." --***-- ".(!isset($lstVigencia))."<br>";
?>
...
<table width="100%"  border="0" cellpadding="0" cellspacing="1"  bgcolor="#FFFFFF" >
  ...
  <tr>
    <td colspan="3" class="TituloTabla">Usuarios</td>
  </tr>
<?php
	$filtros="";
	///FILTROS DE BUSQUEDA
...
?></td>
    <td rowspan="9" align="center" class="TxtTabla" width="5%" ><input name="button2" type="button" class="Boton" id="button2" value="Detalle" onClick="MM_openBrWindow('htReporFactHTDet.php?pMes=<?=$mes ?>&vigencia=<?=$lstVigencia ?>&cualProyecto=<?=$datos_proys["id_proyecto"] ?>&pDivision=<?=$pDivision ?>&pDepto=<?=$pDepto ?>&pCategoria=<?=$pCategoria ?>','winFacturacionHT','scrollbars=yes,resizable=yes,width=1400,height=400')" ></td>
  </tr>
  <tr>
...
*/
        ...
        inner join Departamentos on Usuarios.id_departamento=Departamentos.id_departamento
        inner join Divisiones on Departamentos.id_division=Divisiones.id_division
        where FacturacionProyectos.id_proyecto not in (61,65,71,60,63,62,64,56) and FacturacionProyectos.vigencia=".$lstVigencia." and FacturacionProyectos.mes=".$mes." and FacturacionProyectos.esInterno='I'  and Divisiones.id_division=".$pDivision;
...
*/
		...
		inner join Departamentos on Usuarios.id_departamento=Departamentos.id_departamento
		inner join Divisiones on Departamentos.id_division=Divisiones.id_division
		where FacturacionProyectos.id_proyecto not in (61,65,71,60,63,62,64,56) and vigencia=".$lstVigencia." and mes=".$mes." and FacturacionProyectos.esInterno='I' and Divisiones.id_division=".$pDivision." and FacturacionProyectos.unidad not in (
...
?>
  ...
  <tr>
    <td class="TxtTabla">Valor planeado total <img title="Valor planeado para la divisi&oacute;n en todos los proyectos." src="../NuevaHojaTiempo/imagenes/icoDetalleInf.gif"></td>
    <td class="TxtTabla"><?
		$sql_planeado=" select SUM(PlaneacionProyectos.valorPlaneado) val_planea 
		from PlaneacionProyectos
		inner join Usuarios on PlaneacionProyectos.unidad=Usuarios.unidad
		inner join Actividades on PlaneacionProyectos.id_proyecto=Actividades.id_proyecto and PlaneacionProyectos.id_actividad=Actividades.id_actividad
		where Actividades.id_division=".$pDivision." ";
/*
		if(trim($pDepto)!="")
		{
...
?></td>
  ...
  <tr>
    <td class="TxtTabla">Valor facturado total <img title="Valor facturado para la divisi&oacute;n en todos los proyectos." src="../NuevaHojaTiempo/imagenes/icoDetalleInf.gif"></td>
    <td class="TxtTabla"><?
...
?></td>
  ...
  <tr>
    <td class="TxtTabla">Valor planeado <img title="Valor planeado para la divisi&oacute;n en todos los proyectos, durante la vigencia y mes seleccionados." src="../NuevaHojaTiempo/imagenes/icoDetalleInf.gif"></td>
    <td class="TxtTabla">
<?
		$sql_planeado=" select SUM(PlaneacionProyectos.valorPlaneado) val_planea 
		from PlaneacionProyectos
...
?>
  ...
  <tr>
    <td class="TxtTabla">Valor facturado <img title="Valor facturado para la divisi&oacute;n en todos los proyectos, durante la vigencia y mes seleccionados." src="../NuevaHojaTiempo/imagenes/icoDetalleInf.gif"></td>
    <td class="TxtTabla">
<?
		$sql_factu="select SUM(FacturacionProyectos.valorFacturado) val_fac
		from FacturacionProyectos
...
<table width="100%" border="0"  bgcolor="#FFFFFF" >
  ...
  <tr>
    <td width="10%" class="TxtTabla">Cantidad de proyecto <img title="Proyectos en donde la divis&oacute;n, tiene recurso asignado." src="../NuevaHojaTiempo/imagenes/icoDetalleInf.gif"></td>
    <td width="86%" class="TxtTabla">
<?
...
?>
  ...
  <tr>
    <td class="TxtTabla">Con VoBo <img title="Proyectos cuyas actividades asociadas a la divisón, tienen toda la facturacion aprobada." src="../NuevaHojaTiempo/imagenes/icoDetalleInf.gif"></td>
    <td class="TxtTabla">
<?
	//CANTIDAD DE PROYECTOS, QUE TIENEN TODA LA FACTURACION DE LA DIVISION APROBADA, EN TODOS LOS PROYECTOS
	$sql_vobo="
	SELECT count(*) vobo FROM
	(
		SELECT *, 
		isnull((
			---CANTIDAD DE USUARIOS Y ACTIVIDADES ASOCIADAS A LA DIVISION.  CUYA FACTURACION, FUE APROBADA				 
			SELECT  COUNT(*) con_vobo  FROM (
			select  distinct VoBoFactuacionProyHT.id_proyecto,VoBoFactuacionProyHT.id_actividad,unidad  from VoBoFactuacionProyHT
			inner join Actividades on Actividades.id_proyecto=T3.id_proyecto and Actividades.id_actividad=VoBoFactuacionProyHT.id_actividad
			where VoBoFactuacionProyHT.vigencia=".$lstVigencia."
			and VoBoFactuacionProyHT.validaEncargado=1 
			and VoBoFactuacionProyHT.mes=".$mes."
			and VoBoFactuacionProyHT.esInterno='I'  and Actividades.id_division=".$pDivision."	
			AND VoBoFactuacionProyHT.id_proyecto=T3.id_proyecto
			) T2
			group by (id_proyecto)
		),0) con_vobo
		from (
			--CANTIDAD DE USUARIOS Y ACTIVIDADES ASOCIADAS A LAS DIVISION. DONDE LOS USUARIOS REGISTRARON FACTURACION EN LOS DIFERENTES PROYECTOS
			select id_proyecto,COUNT(*) facturados from (
				select distinct FacturacionProyectos.id_proyecto,FacturacionProyectos.id_actividad,unidad from FacturacionProyectos
				...
				where FacturacionProyectos.vigencia=".$lstVigencia."
				and FacturacionProyectos.mes=".$mes."
				and FacturacionProyectos.esInterno='I'  and Actividades.id_division=".$pDivision."
			) T1		
			group by (id_proyecto)
		) T3
	)T4
	WHERE facturados=con_vobo";
...
?>
  ...
  <tr>
    <td class="TxtTabla">Sin VoBo <img title="Proyectos cuyas actividades asociadas a la divisón, NO tienen toda la facturacion aprobada." src="../NuevaHojaTiempo/imagenes/icoDetalleInf.gif"></td>
    <td class="TxtTabla"><?
...
//			echo  number_format($val_disp,2,'.','')."<br>".$val_disp;
        ...
        <td align="center" rowspan="<? ?>" >
          <input name="button" type="button" class="Boton" id="button" value="Detalle" onClick="MM_openBrWindow('htReporFactHTDet2.php?pMes=<?=$mes ?>&vigencia=<?=$lstVigencia ?>&cualProyecto=<?=$datos_proys["id_proyecto"] ?>&pDivision=<?=$pDivision ?><? if($activi!=""){	echo "&activi=".$activi; } ?>&pCategoria=<?=$pCategoria ?>','winAddFV','scrollbars=yes,resizable=yes,width=1400,height=400')" >
		</td>
      </tr>
<?
	}
...
<p style="color:red; font-weight:bold; " >
    Nota: En este reporte se presenta la informaci&oacute;n de los proyectos, teniendo en cuenta la asignaci&oacute;n de la divisi&oacute;n 
    dentro de los proyectos.<br>
    Es necesario, mostrar la situaci&oacute;n de los recursos (Personal de la divisi&oacute;n) frente a los proyectos, independientemente la divisi&oacute;n no tenga relaci&oacute;n.
</p>
