...
//Traer la fecha de ingreso y retiro de una persona
$miFechaIngreso= "";
$miFechaRetiro= "";
$firSql="SELECT * FROM Usuarios ";
$firSql=$firSql." WHERE unidad = " . $verUnidadDe;
$firCursor = mssql_query($firSql);
if ($firReg=mssql_fetch_array($firCursor)) {
	if (trim($firReg[fechaIngreso]) != "") {
	 	$miFechaIngreso = "F.I." . date("M d Y ", strtotime($firReg[fechaIngreso])) ;
	}
	if (trim($firReg[fechaRetiro]) != "") {
	 	$miFechaRetiro = "F.R." . date("M d Y ", strtotime($firReg[fechaRetiro])) ;
	}
}

//11Ene2012
//Trae la informaciòn de las personas que participaron en la elaboraciòn de la Hoja de tiempo por suplnataciòn en caso de tratarse de un usuario retirado
$supSql="select distinct A.unidadElabora , U.nombre, U.apellidos ";
$supSql=$supSql." from Horas A, Usuarios U ";
$supSql=$supSql." where A.unidadElabora = U.unidad  ";
$supSql=$supSql." and A.unidad = " . $verUnidadDe;
//Si se carga por primera vez la hoja de tiempo trae el mes y año actual
if (trim($Flmes) == "") {
	$supSql=$supSql." and YEAR(A.fecha) = year(getdate()) ";
	$supSql=$supSql." and MONTH(A.fecha) = month(getdate()) ";
}
// si no, trae la información correspondiente al mes y año seleccionados en la lista
else {
	$supSql=$supSql." and YEAR(A.fecha) = " . $Flano ;
	$supSql=$supSql." and MONTH(A.fecha) = " .$Flmes;
}
$supCursor = mssql_query($supSql);

$supMensaje="";
while ($supReg=mssql_fetch_array($supCursor)) {
	if (trim($supMensaje) == "") {
		$supMensaje="Elaborado Por: ";
	}
	$supMensaje=$supMensaje." [".$supReg[nombre] . " " . $supReg[apellidos] . "] - ";
}
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    &nbsp;<input name="Submit4" type="submit" class="Boton" onclick="MM_goToURL('parent','ContratosHT.php?pMes=<? echo $Flmes; ?>&pAno=<? echo $Flano; ?>');return document.MM_returnValue" value="Revisi&oacute;n Contratos" />
...
//										echo "<td>$viaticos[$i]</td>";
		...
		<TD COLSPAN=6> $kfechaEnvio <br>  $miFechaIngreso <br> $miFechaRetiro <br> $supMensaje  </TD>
