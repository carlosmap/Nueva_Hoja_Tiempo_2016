...
if( $recarga == 2 ){
	...
	while( $reg < $cantAct ){
		...
		$opc = "idAct".$reg;
		$act = "d".$reg;
		$sqlDel = "Update ResponsablesActividad SET 
						estado = ".${$act}."
	...
	if( $error == "" ){
		echo "<script> 
				...
				window.close();
				MM_openBrWindow( 'htProgProyectos02OML.php', 'winHojaTiempo', 'scrollbars=yes,resizable=yes,width=620,height=400');
			  </script>";
	}
	else{
		echo "<script> 
...
<!--		->
	function envia2(){
		...
		var mensaje = '';
		var v1, v2, m1, m2, msg;
		var registros = document.getElementById('cantAct').value;
		var i = 1;
	
		v1 = v2 = 1;
		m1 = "Debe seleccionar almenos una actividad.\n";

		for( i = 1; i <= registros; i++ ){				
			if( document.getElementById('d' + i ).checked != false ){
				v1 = 0;
				m1 = "";
			}
		}
      ...
      <tr>
        ...
        <?
			...
			$sqlActResponsable = "SELECT distinct
									usu.unidad, ( usu.nombre + ' ' + usu.apellidos ) NombresApellidos, dep.nombre nDepartamento, div.nombre nDivision, cat.nombre Categoria
								  FROM
									HojaDeTiempo.dbo.ResponsablesActividad res, HojaDeTiempo.dbo.Actividades act, HojaDeTiempo.dbo.Usuarios usu,
									HojaDeTiempo.dbo.Categorias cat, HojaDeTiempo.dbo.Departamentos dep, HojaDeTiempo.dbo.Divisiones div, HojaDeTiempo.dbo.Programadores prog									
								  WHERE
        ...
        <table width="100%"  border="0" cellspacing="1" cellpadding="0">
          ...
          <?
			$sqlAct = "SELECT act.id_actividad idAct, act.nombre nActividad, act.macroactividad, res.estado 	
					   FROM HojaDeTiempo.dbo.ResponsablesActividad res, HojaDeTiempo.dbo.Actividades act, HojaDeTiempo.dbo.Usuarios usu	
					   WHERE 
