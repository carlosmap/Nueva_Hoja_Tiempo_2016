...
*/
		...
		while ($rD <= $totalDiasDinamicos) {
			...
			//Solo realiza la grabación si el día tiene horas registradas
			if (trim(${$elregDia}) != "") {
				$grabar = "SI";
				//--------
...
//	alert(rDhasta);
	...
	//Parte 1 Si es Actividad
	//Si la actividad tiene valor valida contra ese valor
//alert('  '+nuevaFacturacionFila+' ---- '+vlrrActividadF+'');
	if (parseFloat(vlrrActividadF) > 0) {
		nuevaFacturacionFila = parseFloat(vlrFactAct) + parseFloat(facturacionFila);
alert(vlrFactAct+' --- '+facturacionFila);
		//alert('nuevaFacturacionFila='+nuevaFacturacionFila);
		if ( parseFloat(nuevaFacturacionFila) > parseFloat(vlrrActividadF) ) {
			document.Form1.elements[d].value = '';
			alert(' '+nuevaFacturacionFila+' --1-- '+vlrrActividadF+'  ATENCIÓN. Con esta facturación se supera el valor asignado para la actividad. Por favor contacte al Director, Coordinador , Ordenadores de gasto o encargado del tema en la División del proyecto');
			return;
		}
	}
	else {
		...
		if ( parseFloat(nuevaFacturacionFila) > parseFloat(vlrDependeDeF) ) {
			document.Form1.elements[d].value = '';
			alert('  '+nuevaFacturacionFila+' --2-- '+vlrrActividadF+' ATENCIÓN. Con esta facturación se supera el valor asignado para la División en el Lote de control. Por favor contacte al Director, Coordinador , Ordenadores de gasto o encargado del tema en la División del proyecto');
			return;
		}
	}
	//CME 
...
//					}
            ...
            <table width="100%"  border="0" cellspacing="1" cellpadding="0">
			  ...
			  ?>
              <tr class="TxtTabla">
                <td width="15%"><select name="lstActiv<? echo $r; ?>" class="CajaTexto" id="lstActiv<? echo $r; ?>" style='width:200px; ' onChange="limpiaHorasFila(<? echo $r; ?>); envia1()">
...
//echo "sql25=" . $sql25 . "<br>";
				...
				if ($reg25b=mssql_fetch_array($cursor25b)) {
					$pvalActFacturadoFila=$reg25b[valActFacturado];
				}
//quitarEcho				echo "vfA=" . $pvalActFacturadoFila . "<br>";
				...
				if ($reg25c=mssql_fetch_array($cursor25c)) {
					$pvalDependeDeFacturadoFila=$reg25c[valDependeDeFacturado];
				}
//quitarEcho				echo "vfAx=" . $pvalDependeDeFacturadoFila . "<br>"; 
				...
				?>
				<input name="<? echo $d2; ?>regDia<? echo $r; ?>" type="text" class="CajaTexto" id="<? echo $d2; ?>regDia<? echo $r; ?>" onBlur="validaFila(<? echo $r;?>, <? echo $horasAvalidarDia; ?>, <? echo $d2; ?>, <? echo $esDia; ?>, <? echo $horasFactCT_1_2_3; ?>, <? echo $horasFactCT_1o2o11; ?>, <? echo $maxCantHrHorario; ?>, <? echo $pFechaFacturacionTime; ?>, <? echo $esFestivo; ?>, <? echo $numHorasCT6; ?>, <? echo $pLocalizaDia; ?>, <? echo $pvalorActividadFila; ?>, <? echo $pvalorDivDependeDeFila; ?>, <? echo $pvalActFacturadoFila; ?>, <? echo $pvalDependeDeFacturadoFila; ?> )" onKeyPress="return acceptNum(event)" value="<? echo ${$mitxtregDia}; ?>" size="1">
				<? 
