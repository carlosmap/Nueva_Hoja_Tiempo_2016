...
$vSql02=$vSql02." FROM ";
$vSql02=$vSql02." ( ";
$vSql02=$vSql02." 	Select MIN(fechaFacturacion) minFechaFac, MAX(fechaFacturacion) maxFechaFac  ";
$vSql02=$vSql02."	from HojaDeTiempo.dbo.FacturacionProyectos  ";
if (trim($_SESSION["sesUnidadUsuario"]) != trim($laUnidad)  ) {
...
$vSql02=$vSql02." and mes =".$mesAut;
$vSql02=$vSql02." and vigencia =".$anoAut;
$vSql02=$vSql02." ) A ";
...
if (($pmesIngreso == $mesAut) AND ($panoIngreso == $anoAut)) {
	...
	if ($pdiaMinFact < $pdiaIngreso) {
		$validaIngreso="SI";
		$MensajeIngreso="ATENCIÓN. No puede facturar antes de su fecha de ingreso ".date("d-M-Y ", strtotime($pfechaIngreso)).". Por favor corrija la información. ";
	}
	else {
		$MensajeIngreso="";
	}
...
if (($pmesRetiro == $mesAut) AND ($panoRetiro == $anoAut)) {
	...
	if ($pdiaMaxFact > $pdiaRetiro) {
		$validaRetiro="SI";
		$MensajeRetiro="ATENCIÓN. No puede facturar después de su fecha de retiro ".date("d-M-Y ", strtotime($pfechaRetiro))." . Por favor corrija la información. ";
	}
	else {
		$MensajeRetiro="";
	}
...
//-----------------
	...
	//Elimina la información de la unidad, mes y año indicados de la tabla ValidaFactVerticalHT
	//donde se está almacenando la facturación de las personas.
	$vSqlv01="DELETE FROM HojaDeTiempo.dbo.ValidaFactVerticalHT ";
	if (trim($_SESSION["sesUnidadUsuario"]) != trim($laUnidad)  ) {
...
//	$vSqlv01=$vSqlv01." and YEAR(fecha) =" . $elAno;
	$vSqlv01=$vSqlv01." and mes=" . $elMes ;
	$vSqlv01=$vSqlv01." and vigencia =" . $elAno;
	$vCursorv01 = mssql_query($vSqlv01);
//	echo "vSqlv01 <br>" . $vSqlv01 . "<br>";
//	echo "vCursorv01 <br>" . $vCursorv01 . "<br>";

	//Busca la facturación de la persona, la estructura para hacer la validación vertical y la inserta en ValidaFactVerticalHT
	$vSqlv02="
		INSERT INTO HojaDeTiempo.dbo.ValidaFactVerticalHT 
					(unidad, IDhorario, id_proyecto, id_actividad, clase_tiempo, localizacion, cargoFact, fecha, vigencia, mes, esInterno, 
					horas_registradas, horasProyecto, horasGenerico, 
					diaSemana, horasLunes, horasMartes, horasMiercoles, horasJueves, horasViernes, horasSabado, horasDomingo, 
					hmFact, codigoProy, cargoAsig, fechaCrea, unidadCrea)
		
		SELECT DISTINCT A.unidad, A.IDhorario, A.id_proyecto, A.id_actividad, A.clase_tiempo, A.localizacion, A.cargo, A.fechaFacturacion fecha, A.vigencia, A.mes, A.esInterno,
		A.horasMesF, 
		(select CuantasHoras from HojaDeTiempo.dbo.FechasEspecialesProy
		where id_proyecto =A.id_proyecto
		and IDhorario = A.IDhorario
		and Fecha = A.fechaFacturacion ) horasProyecto
		,
		(select CuantasHoras from HojaDeTiempo.dbo.FechasEspeciales
		where IDhorario = A.IDhorario
		and Fecha = A.fechaFacturacion ) horasGenerico
		,
		DATEPART(dw, A.fechaFacturacion) diaSemana
		,
		...
		'0', B.codigo, B.cargo_defecto, GETDATE(), 15712 
		
		FROM HojaDeTiempo.dbo.FacturacionProyectos A, HojaDeTiempo.dbo.Proyectos B
	...
	$vSqlv02=$vSqlv02." AND A.mes = " . $elMes ;
	$vSqlv02=$vSqlv02." AND A.vigencia = " . $elAno;
...
//exit;
	...
	$vSqlv03="SELECT * FROM HojaDeTiempo.dbo.ValidaFactVerticalHT ";
	if (trim($_SESSION["sesUnidadUsuario"]) != trim($laUnidad)  ) {
	...
	$vSqlv03=$vSqlv03." and mes =" . $elMes ;
	$vSqlv03=$vSqlv03." and vigencia =" . $elAno;
...
//	echo "vCursorv03 <br>" . $vCursorv03 . "<br>";
	...
	*/
		...
		$vSqlv04=$vSqlv04." and vigencia =". $vRegv03[vigencia];
		$vSqlv04=$vSqlv04." and mes =". $vRegv03[mes];
		$vSqlv04=$vSqlv04." and esInterno = '".$vRegv03[esInterno]."' ";
		$vCursorv04 = mssql_query($vSqlv04);
		//echo "vSqlv04 <br>" . $vSqlv04 . "<br>";
//			echo "El Horario=" . $vRegv03[IDhorario] ;
	}
...
//			$sVsql="SELECT ROUND(SUM(hmFact), 1) totalHM ";
			$sVsql="SELECT ROUND(SUM(hmFact), 4) totalHM ";
			$sVsql=$sVsql." FROM HojaDeTiempo.dbo.ValidaFactVerticalHT ";
			if (trim($_SESSION["sesUnidadUsuario"]) != trim($laUnidad)  ) {
...
//						echo "El día " . $d . " YA se había retirado. <br>";
				...
				//06Mar2012
				//PBM
				//Sa redondeo porque no permite pasar la validación cuando hay varios horarios.
//				if ($totalDia!=1) {
//				if (round($totalDia, 0) != 1) {
//echo "totalDia=" . $totalDia . "<br>";

				if (($totalDia<0.98) OR ($totalDia>1.09)) {
//				if (($totalDia<0.98) OR ($totalDia>1.02)) {
					...
					if (($esFestivo=="SI") OR ($numeroDia == 7) OR ($numeroDia == 1) OR ($yaIngreso == "NO") OR ($seRetiro == "SI") ) {
						//echo "No tener en cuenta el día " . $d . " sábado, domingo o festivo o No había ingresado o Ya se retiró //n";
						//$queGraba = $totalDia;
					}
					else {
						$mensajeDia=$mensajeDia." El dia " . $d . " no cumple la validación de tiempo total. " . $totalDia . "\\n" ;
						$estaMal=$estaMal+1;
						$queGraba = 'M';
					}
				}
				...
				//echo "No tener en cuenta el día " . $d . "//n";
				$queGraba = 'X';
			}
...
//-----------------
		...
		if ($cuantosHay == 0) {
			...
			$query = "INSERT INTO VoBoFirmasHT(vigencia, mes, unidad, unidadJefe, fechaEnvio, usuarioCrea, fechaCrea)  " ;
			...
			if (trim($_SESSION["sesUnidadUsuario"]) != trim($laUnidad)  ) {
				...
				$query = $query . " usuarioMod = " . $_SESSION["sesUnidadUsuario"] . " , ";	
			}
...
//	exit;
		...
		if  (trim($cursor) != "") {
			echo ("<script>alert('El envío de la Hoja de Tiempo se realizó con éxito.');</script>");
		} 
		else {
			echo ("<script>alert('Error durante la grabación');</script>");
		};
		echo ("<script>window.close();MM_openBrWindow('htFacturacion.php?pAno=$elAno&pMes=$elMes','winFacturacionHT','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");		
	}
}
...
<html>
<head>
<title>.:: Hoja de tiempo - Enviar a VoBo Jefe inmediato - Revisi&oacute;n</title>
