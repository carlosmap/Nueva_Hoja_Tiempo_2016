...
if ($pAprob != "") {
	...
	$query = $query . " validaDirector = '".$pAprob . "', ";	
	$query = $query . " unidadDirector=".$laUnidad . ", ";	
	$query = $query . " comentaDirector = '". $Observaciones. "', " ;	
	$query = $query . " requiereFirma = '".$pFirma . "', ";	
	if ($pFirma == '1') {
		$query = $query . " unidadJefe=" .  $pJefeAut2 ;
	}
	else {
		$query = $query . " unidadJefe=NULL " ;
	}
	...
	if  (trim($cursor) != "") {
		...
		if (trim($pFirma == '1')) {
			...
			$AsuntoEM = "Portal Ingetec S.A - Solicitud Programación";
			
			$msgEM= "<table width='100%'  border='0' cellspacing='0' cellpadding='0'>" ;
			$msgEM=$msgEM." <tr> " ;
			$msgEM=$msgEM." 	<td>Usted tiene una nueva de solicitud de programación para autorizar. Por favor consulte el Portal <a href='http://www.ingetec.com.co/portal/'>www.ingetec.com.co/portal</a> opción Hoja de tiempo/Solicitud Programación. </td> " ;
			$msgEM=$msgEM."   </tr> ";
			$msgEM=$msgEM."   <tr> ";
			$msgEM=$msgEM." 	<td> </td> " ;
			$msgEM=$msgEM."   </tr> ";
...
//echo $cualMailEM . "<br>";
	...
	echo ("<script>window.close();MM_openBrWindow('ProgProyectosSolHoras.php?pMes=$elMes&pAno=$elAno','winHojaTiempoSHoras','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");	
}
...
//		$sql2="Select * from Proyectos order by nombre " ;
  ...
  <tr>
	...
	?>
	<input name="pAprob" type="radio" value="1" <? echo $selSi; ?> >
      Si&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <input name="pAprob" type="radio" value="0" <? echo $selNo; ?> >
        No</td>
  </tr>
  <tr>
    <td class="TituloTabla">Observaciones</td>
    <td class="TxtTabla"><textarea name="Observaciones" cols="50" class="CajaTexto" id="Observaciones"> <? echo $mcomentaDirector; ?></textarea></td>
  </tr>
  
  <tr>
    <td class="TituloTabla">Requiere VoBo?</td>
    <td class="TxtTabla">
	<?
	if (trim($mrequiereFirma) == "1") {
		$selFSi = "checked";
		$selFNo = "";
	}
	else {
	...
	?>
	<input name="pFirma" type="radio" value="1" <? echo $selFSi; ?> >
      Si
        &nbsp;&nbsp;&nbsp;&nbsp;
        <input name="pFirma" type="radio" value="0" <? echo $selFNo; ?> > 
        No </td>
  </tr>
  <tr>
    <td class="TituloTabla">Solicitar VoBo a </td>
    <td class="TxtTabla">
	<?
...
if ($reg=mssql_fetch_array($cursor)) {
	...
	<select name="pJefeAut2" class="CajaTexto" id="pJefeAut2" >
		...
		while ($reg2=mssql_fetch_array($cursor2)) {
			...
			if (trim($munidadJefe) == $reg2[unidad]) {
				$selUsu="selected";
			}
			else {
				$selUsu="";
