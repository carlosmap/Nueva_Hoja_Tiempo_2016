...
if($recarga==1)
{
		...
		$sql3 = " SELECT *,Usuarios.nombre as nom_usu,(select id_encargado from Actividades  where id_proyecto=".$cualProyecto." and id_actividad =".$id_ac.") encargado FROM HojaDeTiempo.dbo.Usuarios";
...
//		$mail = "mail" . $i;
		...
		*/
		if(($$aplicaUsuario == 1)and($okGuardar == "Si"))
		{
			...
			$sql_planea="select * from  PlaneacionProyectos  where id_proyecto=".$cualProyecto." and id_actividad=".$id_ac." and unidad=".$$usuario." and esInterno='I' ";
...
//echo "<br>".$sqlDel;
	...
	//exit;
	echo ("<script>window.close();MM_openBrWindow('htPlanProyectos02.php?cualProyecto=".$cualProyecto."&participante=0','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=960,height=700');</script>");	
}
...
function envia0()
{
	...
	{
		error = 's';
		mensaje = mensaje + 'Seleccione una actividad. \n';
	}
	if(error=='s')
	{
		alert(mensaje);
...
//alert (document.Form1.cantUsuarios.value);
...
<? 
			...
			if( ($reg3['encargado'])!=($reg3['unidad']) ) { 
?>
        <td align="center">
				<input name="aplicaUsuario<? echo $i; ?>" type="radio" value="1"  >
		</td>

        <td align="center">

			<input name="aplicaUsuario<? echo $i; ?>" type="radio" value="0" checked > 
...
<?
			...
			{
			//SI SE EL USUARIO CONSULTADO ES EL RESPONSABLE DE LA ACTIVIDAD, NO SE PUEDE DESASOCIAR DE FORMA DIRECTA, HASTA QUE NO SE ESTE DEJE DE SER EL RESPONSABLE
?>
        <td align="center" colspan="2">
				<img src="../NuevaHojaTiempo/imagenes/icoDetalleInf.gif" title="No se puede desasociar al responsable de la actividad.">
				<? $i--;
				//echo $i; ?>
		</td>
<?
 ...
 ?>
  ...
  <tr>
	...
	<input name="cantUsuarios" type="hidden" id="cantUsuarios" value="<? echo $i; ?>">	<input name="recarga" type="hidden" id="recarga" value="0">
