...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td>
    <input name="BotonReg" type="submit" class="Boton" id="BotonReg" onclick="MM_callJS('history.back()')" value="Listado de usuarios" />&nbsp;&nbsp; </td>
  </tr>
</table>
...
//--Trae la información del usuario con nombre del departamento y nombre de la categoria
	...
	$sql="SELECT estadoAprobDivision, comentariosDivision, revisadoPorDivision, estadoAprobProyecto, comentariosProyecto,
		revisadoPorProyecto, RTRIM(localizacion) + '-' + RTRIM(cargo) AS codigoproyecto,
		 horas_registradas, clase_tiempo, resumen_trabajo, day(fecha) as dia, id_actividad, id_proyecto, localizacion, cargo FROM horasTMP
...
/*********************************************IMPRIME LAS HORAS FACTURADAS***********************************************/
		...
		while($filas){
			$klocalizacion=$filas[localizacion];
			$kcargo=$filas[cargo];
...
//					$nProyecto =  ucwords(substr($filasAct[proyecto],0,14));
			...
			for($i=1;$i<=31;$i++){
				...
				if($horasdia[$i]==0){
					...
					echo "<td bgcolor='$color' style='text-align:center;'><a href='#' onClick=MM_openBrWindow('upHTTMP.php?cualProyecto=$id_proy&cualActividad=$mactividad&cualUnidad=$verUnidadDe&cualFecha=$Flano-$Flmes-$i&cualLocaliza=$klocalizacion&cualCargo=$kcargo&cualClase=$ttiempo&Flano=$Flano&Flmes=$Flmes','upTT','scrollbars=yes,resizable=yes,width=500,height=200') >$hora</a></td>\r";
					$horasdia[$i]=" ";
				}
			}
			$color='';
...
*/
		...
		echo "<tr><td colspan=4>CLASE DE TIEMPO</td><TD colspan=4>HORARIOS</TD>
		<td colspan=9>CLASE DE TIEMPO</td>
		<TD COLSPAN=4>HORARIOS</TD><TD COLSPAN=6>FIRMA DEL EMPLEADO</TD>
