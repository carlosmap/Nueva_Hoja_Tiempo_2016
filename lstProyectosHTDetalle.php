  ...
  <tr>
    ...
    <table width="100%"  border="0" cellspacing="0" cellpadding="0">
      ...
      <tr class="TituloTabla2">
        ...
        <td>Comentarios</td>
        <td width="8%">Fecha de aprobaci&oacute;n </td>
        <td width="1%">&nbsp;</td>
        <td width="1%">&nbsp;</td>
      </tr>
       <?
	  ...
	  <tr class="TxtTabla">
		...
		if ($regA=mssql_fetch_array($cursorA)) {
			...
			$laFechaDeAprob= $regA[fechaAprEnc] ;
		}
        ...
        <td width="8%"><? 
		if (trim($laFechaDeAprob) != "") {
			echo date("M d Y ", strtotime($laFechaDeAprob)); 
		}
		?></td>
        <td width="1%">
		<?
		$tieneVoCoContratos=0;
		$sqlVC00="select * ";
		$sqlVC00=$sqlVC00." from HojaDeTiempo.dbo.AutorizacionesHT   ";
		$sqlVC00=$sqlVC00." where unidad = " . $reg[unidad] ;
		$sqlVC00=$sqlVC00." and mes =" . $pMes;
		$sqlVC00=$sqlVC00." and vigencia =" . $pAno;
		$cursorVC00 = mssql_query($sqlVC00);
		if ($regVC00=mssql_fetch_array($cursorVC00)) {
			$tieneVoCoContratos=$regVC00[validaContratos] ; 
		}
		
		if ($tieneVoCoContratos== 0) {
		?>
		<a href="#"><img src="img/images/actualizar.jpg" alt="Modificar" width="19" height="17" border="0" onclick="MM_openBrWindow('addlstProyectosHTDetalle2.php?cualProyecto=<? echo $cualProyecto; ?>&pMes=<? echo $mesEnvio ?>&pAno=<? echo $anoEnvio ?>&cualUnidad=<? echo $reg[unidad]; ?>','verHT','scrollbars=yes,resizable=yes,width=700,height=400')" /></a>
		<? } ?>
		</td>
