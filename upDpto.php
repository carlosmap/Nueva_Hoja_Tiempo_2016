...
//10Abr2008
//Trae el listado de Departamentos
...
if($Nombre != "") {
	...
	//Valida que el nombre del departamento no exista
	$sqlDep = "select * from Departamentos where nombre = '". $Nombre . "' ";
	$sqlDep = $sqlDep . " and id_division = " . $miDivision ;
	$sqlDep = $sqlDep . " and id_departamento <> " . $miDepartamento ;	
	
	$cursorDep = mssql_query($sqlDep) ;
	if(mssql_num_rows($cursorDep) > 0) {
		echo "<script>alert('Este nombre de departamento ya existe en esta división. Por favor corríjalo')</script>";
	...
	$insDep=$insDep." WHERE id_departamento = " . $miDepartamento ;
	...
	echo ("<script>window.close();MM_openBrWindow('ProgMantenimiento.php?cualDependencia=$miDependencia&cualDivision=$miDivision','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=960,height=700');</script>");	
}
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="TituloUsuario">Actualizaci&oacute;n del Departamento</td>
  </tr>
</table>
<table width="100%" border="0" cellspacing="1" cellpadding="0">
  ...
  <tr>
    ...
    <td width="68%" class="TxtTabla">
      ...
      <input name="miDependencia" type="hidden" id="miDependencia" value="<? echo $cualDependencia; ?>">
      <input name="miDepartamento" type="hidden" id="miDepartamento" value="<? echo $pid_departamento ; ?>"></td>
  </tr>
  <tr>
