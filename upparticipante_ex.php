...
//echo "<br>".$sqlDel."<br>".mssql_get_last_message()."<br><br>";
			...
			else
			{
				...
				$sqlDel = "UPDATE  HojaDeTiempo.dbo.ParticipantesExternos SET estado='I' ,usuarioMod=".$_SESSION["sesUnidadUsuario"]." ,fechaMod=getdate() 
						   Where id_actividad = ".${$opc}." and consecutivo = ".$und." AND id_proyecto = ".$cualProyecto;
				$qry = mssql_query( $sqlDel );
				if( !$qry )
					$error = "no";
...
*/
		...
		if ( (${$act}=='A') &&($error!="no") )
		{
				$sqlDel = "UPDATE  HojaDeTiempo.dbo.ParticipantesExternos SET estado='A' ,usuarioMod=".$_SESSION["sesUnidadUsuario"]." ,fechaMod=getdate()  
						   Where id_actividad = ".${$opc}." and consecutivo = ".$und." AND id_proyecto = ".$cualProyecto;
				$qry = mssql_query( $sqlDel );
				if( !$qry )
					$error = "no";
      ...
      <table width="100%"  border="0" cellspacing="0" cellpadding="0">
         <tr class="TxtTabla">
                ...
                <table width="100%" border="0">
                  <tr>
                    <td class="FichaAct">Activar-Desactivar Actividades</td>
                    <td class="FichaInAct"><a href="addActiviUExterno.php?cualProyecto=683&conse=<?=$conse; ?>&aa=<?=$aa; ?>" class="FichaInAct1">Asociar Actividades</a></td>
                    <td width="60%" ></td>
                  </tr>
                </table></td>
                </tr>
