...
$sql5=$sql5." A.Jueves, A.Viernes, A.Sabado, A.Domingo, A.localiza " ;
...
//11May2011
//Trae el primer horario
$laLocalizacion = "";
if (trim($pHorario) == "") {
	$cursorph = mssql_query($sql5);
	if ($phReg=mssql_fetch_array($cursorph)) { 
		$primeHorario = $phReg[IDhorario];
		 if (trim($phReg[ubicacion]) != "") {
			$laLocalizacion = trim($phReg[ubicacion]);
		 }
		 else {
			$laLocalizacion = trim($phReg[localiza]);
		 }
	}
}
...
if (trim($pMes) != "") {
	...
	$sqlHM=$sqlHM." where unidad = " . $pUsuario ;
	...
	$sqlHM=$sqlHM." where unidad = " . $pUsuario ;
	$sqlHM=$sqlHM." and (clase_tiempo = 1 or clase_tiempo = 2) ";
	$sqlHM=$sqlHM." and id_proyecto = " . $miProyecto ;
	$sqlHM=$sqlHM." and id_actividad = " . $miActividad ;
	$sqlHM=$sqlHM." and month(fecha_inicial) = " . $mesHM ;
	$sqlHM=$sqlHM." and year(fecha_inicial)=" . $AnoHM;
	$cursorHM = mssql_query($sqlHM);
	if ($regHM=mssql_fetch_array($cursorHM)) { 
...
//11May2011
//Encontrar cuál es la localización a partir del horario seleccionado
if (trim($pHorario) != "") {
	//Consulta para traer la localización del horario genérico o del horario del proyecto
	$sqlL=$sql5." AND H.IDhorario = " . $pHorario ;

	$cursorL = mssql_query($sqlL);
	if ($LReg=mssql_fetch_array($cursorL)) { 
		$primeHorario = $LReg[IDhorario];
		 if (trim($LReg[ubicacion]) != "") {
			$laLocalizacion = trim($LReg[ubicacion]);
		 }
		 else {
			$laLocalizacion = trim($LReg[localiza]);
		 }
	}
}

//echo "<br>" . "Localiza Horario " . $laLocalizacion; 
//Cierra 11May2011
...
//09Oct22008
//Valida que no se supere la cantidad de horas del mes con lo que se esta asignando de programación al proyecto y actividad
		$nuevoTotalHoras = $totHorasMesProy + $HorasAsignadas;
...
//		echo "<br> Horas Mesr=" .  $horasLabMes;
		...
		$hSql=$hSql." from horarios where idHorario =" . $pHorario ;
...
//		if ($nuevoTotalHoras > $horasLabMes) {/
//		if ($nuevoTotalHoras > $horasLocaliza) {
//28Oct2008
//verificar si se trata de clase de tiempo 1 o 2, de lo contrario la validación NO aplica
//echo $pClase ;
//		if (($nuevoTotalHoras > $horasLocaliza) AND (($pClase == 1) OR ($pClase == 2)) ) {
		if (($nuevoTotalHoras > $horasLocaliza) AND (($pClase == 1) OR ($pClase == 2)) AND ($phorasHorario <= 44 ) ) {
	...
	//cierra 09Oct2008
			...
			if (($pClase == 1) OR ($pClase == 2) OR ($pClase == 10) OR ($pClase == 11)) {
				$recursoCalculado=($HorasAsignadas/$horasLaborales)*$esteSalario*$cualFactor;
				...
				if ($recursoCalculado > $esteSalario) {
					$recursoCalculado=$esteSalario;
				}
			}
			else {
				$recursoCalculado=($HorasAsignadas/240)*$esteSalario*$cualFactor;
			}
			
			//01Jun2011
			//PBM
			//Por solicitud de Enrique Piñeros
			//Verifica el tipo de contrato y realiza la siguiente validación:
			//Si la persona es TC y se seleccionó clase de tiempo = 2 entonces se le asigna automáticamente clase de tiempo 1
			//Si la persona es MT y se seleccionó clase de tiempo = 1 entonces se le asigna automáticamente clase de tiempo 2
			//NOTA: 15jUN2011 PBM
			//Se eliminó la obligatoriedad porque algunas personas cambian el tipo de contrato a mitad de mes.
			//Por tanto sólo puede sugerirse pero NO forzarse.
			//Revisión PBM y Sonia Hoyos.
			/*
			$tipoContratoUsuario = "";
			$uSqlTC="SELECT * FROM usuarios ";
			$uSqlTC=$uSqlTC." WHERE unidad = " . $pUsuario;
			$uCursorTC = mssql_query($uSqlTC);
			if ($uRegTC=mssql_fetch_array($uCursorTC)) {
				$tipoContratoUsuario = $uRegTC[TipoContrato];
			}
			if ((trim($tipoContratoUsuario) == "TC") AND ($pClase == 2))  {
				$pClase = 1;
			}
			if ((trim($tipoContratoUsuario) == "MT") AND ($pClase == 1))  {
				$pClase = 2;
			}
			*/
			...
			if (trim($queHacer) == "Continuar") {
				echo ("<script>MM_openBrWindow('ProgProyectosActiv.php?cualProyecto=$miProyecto&cualActividad=$miActividad&pfDivision=$pfDivision&miDpto=$miDpto&pFiltro=$pFiltro','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");	
			}
			else {
				echo ("<script>window.close();MM_openBrWindow('ProgProyectosActiv.php?cualProyecto=$miProyecto&cualActividad=$miActividad&pfDivision=$pfDivision&miDpto=$miDpto&pFiltro=$pFiltro','winHojaTiempo','toolbar=yes,scrollbars=yes,resizable=yes,width=950,height=600');</script>");	
			}
		}
	}
}
...
//	echo $cualSalario;
  ...
  <tr>
    <td class="TituloTabla">Clase de tiempo <br></td>
    <td class="TxtTabla">
	...
	<br>
	<strong>NOTA</strong>: Tener en cuenta para la programaci&oacute;n el cambio del Tipo de contrato. <br>
	Para Tiempo completo la Clase de tiempo debe ser 1. <br>
	Para Medio tiempo la clase de tiempo debe ser 2. <br>
	Ejemplo: Juan Perez tiene tipo de contrato igual a Tiempo completo entre el 1 y el 15 del mes. En adelante es medio tiempo. La programaci&oacute;n debe estar los primeros quince d&iacute;as asociados a clase de tiempo 1 y los restantes en clase de tiempo 2. </td>
  </tr>
  <tr>
	...
	<?
		...
		<input name="pLocaliza" type="hidden" id="pLocaliza" value="3">
	<?
	}
	?>
    	</td>
  ...
  <tr>
    ...
    <td class="TxtTabla">
	<select name="pHorario" class="CajaTexto" id="pHorario" onChange="envia1()">
...
<?
  ...
  <tr>
    ...
    <td class="TxtTabla"><? echo $phorasMesAsignadas; ?>
      <input name="totHorasMes" type="hidden" id="totHorasMes" value="<? echo $phorasMesAsignadas; ?>">
  ...
  <tr>
	...
	<input name="btnGraba" type="button" class="Boton" onClick="envia2()" value="Grabar y Cerrar" >
	<? } ?>	</td>
  </tr>
  <tr>
    <td colspan="2" class="TxtTabla"><strong>NOTA</strong>: <br>
      Por favor tener en cuenta que el personal contratado a Medio tiempo (MT) DEBE REPORTAR su tiempo en Clase de tiempo 2 - ordinario (medio tiempo)<br>      
