      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
        ...
        <tr>
          <td width="22%" valign="top"><table width="100%" border="0" cellspacing="5" cellpadding="0">
			...
			<? } ?>
			<?php
			//Se activa el link para Adriana Bautista y América Cortés
				if(($laUnidad == 12974) OR ($laUnidad == 15712) OR ($laUnidad == 14386) OR ($laUnidad == 14987)) {
			?>
            <tr>
              <td class="menu"><img src="img/images/mnuFlecha.gif" width="12" height="13" /></td>
              <td class="menu"><a href="HtiempoCerrejon.php">Hoja de tiempo Cerrrej&oacute;n </a> </td>
            </tr>
			<? } ?>			
			<?php
			//Se activa el link del proyecto Ituango 1% [46.0] para Elizabeth Gandara  e Ivan silva
				if(($laUnidad == 12974) OR ($laUnidad == 15712) OR ($laUnidad == 14080) OR ($laUnidad == 13104)) {
			?>
            <tr>
              <td class="menu"><img src="img/images/mnuFlecha.gif" width="12" height="13" /></td>
              <td class="menu"><a href="HtiempoItuango1.php">Hoja de tiempo Ituango Plan Inversión 1% </a> </td>
            </tr>
			<? } ?>		
			<?php
			//Se activa el link para Teófilo Noriega, Fabio Amaya, Martha Isabel Perez Sandoval
				if(($laUnidad == 12974) OR ($laUnidad == 15712) OR ($laUnidad == 15431) OR ($laUnidad == 1517)  OR ($laUnidad == 18091)  ) {
			?>
            <tr>
              <td class="menu"><img src="img/images/mnuFlecha.gif" width="12" height="13" /></td>
              <td class="menu"><a href="HtiempoTipiel.php">Hoja de tiempo Tipiel </a> </td>
            </tr>
			<? } ?>						
            <tr> 
			  <td class="menu"><img src="img/images/mnuFlecha.gif" width="12" height="13" /></td>
			...
			//26Jul2012
			//Gonzalo Rodriguez, Patricia Barón, Camilo Marulanda, Fernando Manjarrés, Fabio Sánchez, Katherine Pinilla, Mary Guevara, María Angelica Cárdenas, Juliana Gonzalez
			if ( ($linkRHT > 0) OR ($laUnidad == 12974) OR ($laUnidad == 15712) OR ($laUnidad == 14384) OR ($laUnidad == 11733) OR ($laUnidad == 13829) OR ($laUnidad == 900384) OR ($laUnidad == 18164) OR ($laUnidad == 17063) OR ($laUnidad == 800262)) { 
			?>
			<tr>
			  <td class="menu"><img src="img/images/mnuFlecha.gif" width="12" height="13" /></td>
            ...
            <? 
			//Para que el link solo aparezca en este momento a Gonzalo rodríguez, Patricia Barón y Fabio sánchez y Andrés Marulanda 14234 y Lina Arroyave (900582)
			if ( ($laUnidad == 12974) OR ($laUnidad == 15712) OR ($laUnidad == 13829) OR ($laUnidad == 14234) OR ($laUnidad == 900582) ) { ?>
			<tr>
			  <td class="menu"><img src="img/images/mnuFlecha.gif" width="12" height="13" /></td>
			  <td class="menu"><a href="rptGP01.php">Reportes Gerencia de Proyectos</a>  </td>
			</tr>
			<?  } ?>
            			<?
...
//			if (($_SESSION["sesUnidadUsuario"] == 12974 ) OR ($_SESSION["sesUnidadUsuario"] == 15712 ) ) {
			...
			//13Nov2007
			//Si la unidad activa corresponde a Heidy Zambrano muestra el link para entrar a 
			//las Hojas de tiempo de las personas que facturaron a Ituango Interventoría (44.0)
			//05Nov2013 PBM agregar a Sandra Suarez [14438] según lo indicado por Heisy Zambrano
			if (($laUnidad == 13991) OR ($laUnidad == 17091) OR ($laUnidad == 14438)) {			?>
            <tr>
              <td class="menu"><img src="img/images/mnuFlecha.gif" width="12" height="13" /></td>
              <td class="menu"><a href="RevisaHTItuango.php"> Ver Hoja de tiempo Ituango Interventoría</a> </td>
            </tr>
			<? } ?>			
			<? 
...
//	echo $zSql0 ;
	...
	///02Ago2013
	//PBM
	//Se implementa Filtro para verificar que la fehca especialpara el proyecto también sea tenida en cuenta
	//****Desde Aqui
	//tAMBIEN GENERÓ CAMBIOS EN LINEAS 1516 (if (($miDiaEspecial == 0) AND ($miDiaEspecialb == 0)) {) Y 
	$zSql0b="select count(*) esDiaEspecial , coalesce(sum(CuantasHoras), 0) cuantasHoras from fechasEspecialesProy  ";
	$zSql0b=$zSql0b." where IDhorario =" . $miHorario ;
	$zSql0b=$zSql0b." and fecha = '" . $laFecha . "' " ;
//	$zSql0b=$zSql0b." and CuantasHoras > 0 " ;
//	echo $zSql0b ;
	$cursorzSql0b = mssql_query($zSql0b);
	if ($regzSql0b=mssql_fetch_array($cursorzSql0b)) {
		$miDiaEspecialb= $regzSql0b[esDiaEspecial];
		
		//10Jul2008
		//Verifica el máximo valor en cantidad de horas
		$miHorasDiaEspecialb=$regzSql0b[cuantasHoras];
		//Cierra 10Jul2005
	}
...
//	if ($miDiaEspecial == 0) {
	if (($miDiaEspecial == 0) AND ($miDiaEspecialb == 0)) {
...
//		echo $zSql1 . "<br>";
		...
		//09Ago2012
		//PBM
		//Se elimina porque se requiere validar si el horario permite grabar un tiempo especial en ese día
		/*
		if ($esDiaFestivo == "1") {
		...
		*/
		if ($esDiaFestivo == "1") {
			...
			//09Ago2012
			//PBM
			//Si la fecha es festivo y se incluye en los horarios de un proyecto especial lo permite pasar
			$z2Sql="select COUNT(*) esFechaProyecto  ";
			$z2Sql=$z2Sql." from FechasEspecialesProy ";
			$z2Sql=$z2Sql." where id_proyecto = " . $pProy ;
			$z2Sql=$z2Sql." and Fecha = '".$laFecha."' ";
			$z2Cursorz2Sql = mssql_query($z2Sql);
			if ($regz2Sql=mssql_fetch_array($z2Cursorz2Sql)) {
				$esDiaFestivoDelProyecto= $regz2Sql[esFechaProyecto];
			}
/*echo $z2Sql . "<br>";
		echo "<script>alert('".$esDiaFestivoDelProyecto."')</script>"; */
			if ($esDiaFestivoDelProyecto == 0) {
				echo "<script>alert('No puede grabar Clase de tiempo 1 en un día festivo. Por favor corrija la información--')</script>";
				exit();
			}
		}
...
//		if ($miHorasDiaEspecial == 0) {
		if (($miHorasDiaEspecial == 0) AND ($miHorasDiaEspecialb == 0)) {
...
function localizaProyEsp($peFecha, $peUni, $peLocalizacion, $peClaseTiempo) {
	...
	*/
	...
	if ($peReg=mssql_fetch_array($peCursor)) {
		...
		$pecodUbicacion=$peReg[codUbicacion]; 
	}
	...
	*/
	...
	if (($peCategoria >= 53) AND ($peCategoria <=62)) {
		...
		if ( ( (trim($peSitioContrato)=="MEDELLIN") AND (trim($peSitioTrabajo)=="OF MEDELLIN") ) OR ((trim($peSitioContrato) == trim($peSitioTrabajo)) AND (trim($peSitioContrato)=="MEDELLIN") AND (trim($pecodUbicacion) == "0510000400")) OR ((trim($peSitioContrato) == trim($peSitioTrabajo)) AND (trim($peSitioContrato)=="BOGOTA")) OR ((trim($peSitioTrabajo)=="BOGOTA") AND (trim($peSitioContrato)!="BOGOTA")) ) {
...
function validaProySinProg($spFecha, $spUni, $spLocalizacion, $spClaseTiempo) {
	...
	*/
	...
	if ($spReg=mssql_fetch_array($spCursor)) {
		...
		$spcodUbicacion=$spReg[codUbicacion];
	}
...
//		if ( ((trim($spSitioContrato) == trim($spSitioTrabajo)) AND (trim($spSitioContrato)=="MEDELLIN") AND (trim($spcodUbicacion) == "0510000400")) OR ((trim($spSitioContrato) == trim($spSitioTrabajo)) AND (trim($spSitioContrato)=="BOGOTA")) OR ((trim($spSitioTrabajo)=="BOGOTA") AND (trim($spSitioContrato)!="BOGOTA")) ) {
		...
		//Se agregó la validación para que las personas que estan trabajando en la oficina de Medellín (Ej. Sergio Esteban Rosales [18076]) reporten su tiempo a localización 1
		//Por esta razón el sitio de trabajo debe ser = OF MEDELLIN 
		if ( ( (trim($spSitioContrato)=="MEDELLIN") AND (trim($spSitioTrabajo)=="OF MEDELLIN") ) OR ((trim($spSitioContrato) == trim($spSitioTrabajo)) AND (trim($spSitioContrato)=="MEDELLIN") AND (trim($spcodUbicacion) == "0510000400")) OR ((trim($spSitioContrato) == trim($spSitioTrabajo)) AND (trim($spSitioContrato)=="BOGOTA")) OR ((trim($spSitioTrabajo)=="BOGOTA") AND (trim($spSitioContrato)!="BOGOTA")) ) {		
			//La localización debe ser 1
			if ($spLocalizacion != 1) {
...
//--and (month(a.fecha_inicial) = 1 and year(a.fecha_inicial) = 2009) 
	...
	if($pProyID!="42" AND $pProyID!="48" AND $pProyID!="71" AND $pProyID!="65" AND $pProyID!="61" AND $pProyID!="60" AND $pProyID!="63" AND $pProyID!="62" AND $pProyID!="64" AND $pProyID!="56" ){
		//06Mar2012
		//PBMla siguiente linea en comentario porque no permite grabar la cantidad de horas cuando hay varios horarios en ese día
		//$tSql2=$tSql2." and a.IDhorario = " . $pHorarioDia ;

		//06Mar2012
		//PBM puse la siguiente consulta para poder encontrar los horarios en que ya ha registrado horas la persona en el día que está tratando de grabar
		//Para validar la cantidad de horas máxima que debe dejar grabar
		//Si no hay nada grabado en el día pone el id del horario que está tomando como parámetro que corresponde al horario del proyecto que está grabado.
		$losHorariosDia="";
		$tSql3="SELECT B.IDhorario ";
		$tSql3=$tSql3." FROM Horas A, Asignaciones B, Proyectos C ";
		$tSql3=$tSql3." WHERE A.id_proyecto = B.id_proyecto ";
		$tSql3=$tSql3." and A.id_actividad = B.id_actividad ";
		$tSql3=$tSql3." and A.unidad = B.unidad ";
		$tSql3=$tSql3." AND A.clase_tiempo = B.clase_tiempo ";
		$tSql3=$tSql3." and A.localizacion = B.localizacion ";
		$tSql3=$tSql3." and A.cargo = C.codigo+B.cargo ";
		$tSql3=$tSql3." and A.id_proyecto = C.id_proyecto ";
		$tSql3=$tSql3." and A.unidad = " . $pUni;
		$tSql3=$tSql3." and A.fecha = '".$laFecha."' ";
		$tSql3=$tSql3." AND MONTH(B.fecha_inicial) = " . $arreglo[0] ;
		$tSql3=$tSql3." AND YEAR(B.fecha_inicial) = " . $arreglo[2] ;
		$cursortSql3 = mssql_query($tSql3);
		while ($regtSql3=mssql_fetch_array($cursortSql3)) {
			$losHorariosDia = $losHorariosDia . $regtSql3[IDhorario] . ", ";
		}
		if (trim($losHorariosDia) != "") {
			//$losHorariosDia = substr($losHorariosDia, 0, (strlen($losHorariosDia) - 2));
			$losHorariosDia = $losHorariosDia . $pHorarioDia  ;
			$tSql2=$tSql2." and a.IDhorario IN (" . $losHorariosDia . ") ";
		}
		else {
			$tSql2=$tSql2." and a.IDhorario = " . $pHorarioDia ;
		}
...
//			echo $maxHorasDia . "<br>";
		...
		if ($nuevoTotal > $maxHorasDia) {
			echo "<script>alert('La cantidad de horas máxima que puede registrar para este día es ".$maxHorasDia.". Por favor corrija la información.z')</script>";
			exit;
		}
		*/
		if ($nuevoTotal > $maxHorasDia) {
			...
			//09Ago2012
			//PBM
			//Verifica la máxima cantidad de horas del proyecto que puede registrar en la HT 
			 $v1Sql1="select COUNT(*) esFechaProyecto  ";
			 $v1Sql1=$v1Sql1." from FechasEspecialesProy ";
			 $v1Sql1=$v1Sql1." where id_proyecto = " . $pProyID ;
			 $v1Sql1=$v1Sql1." and Fecha = '". $laFecha ."'";
			 $v1Cursorv1Sql1 = mssql_query($v1Sql1);
			 if ($regv1Sql1=mssql_fetch_array($v1Cursorv1Sql1)) {
				$maxDiaFestivoDelProyecto= $regv1Sql1[esFechaProyecto];
			 }
			 
			 if ($maxDiaFestivoDelProyecto == 0) {
				echo "<script>alert('La cantidad de horas máxima que puede registrar para este día es ".$maxHorasDia.". Por favor corrija la información.z')</script>";
				exit;
			}
		}
