...
<tr class='TituloUsuario'><td colspan='6' bgcolor='#999999' ></td></tr>";
	...
	if($recarga==1)
	{
			...
			if(($facturacion==2) || ($facturacion==1))
			{ 
				$sql_fact=" 
	...
	--SE IMPLEMENTO DISTINCT, PARA EVITAR, QUE LA CONSULTA ARROJE REGISTROS DUPLICADOS
					...
					select distinct unidad,id_proyecto,nombre,nombre_depto,nombre_div,vigencia from (	select Usuarios.unidad,id_proyecto, Upper(Usuarios.nombre+' '+ Usuarios.apellidos) nombre ,total_facturacion, hombresMes, upper(Departamentos.nombre) nombre_depto ,upper(Divisiones.nombre) nombre_div,vigencia  from (
			...
			//	FACTURACION QUE ES INFERIOR A LA PLANEACION 
			if(($facturacion==2))
			{ 
				 $sql_fact=$sql_fact." and  hombresMes>total_facturacion )T2	 ";
			}
